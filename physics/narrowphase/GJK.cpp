...
static Vec3 Support(const RigidBody& body, const Vec3& d) {
  ...
  if (body.collider->type == ColliderType::Box) {
    ...
    Vec3 local{Vec3::Dot(d, rot.GetColumn(0)) > 0 ? box->halfExtents.x : -box->halfExtents.x,
               Vec3::Dot(d, rot.GetColumn(1)) > 0 ? box->halfExtents.y : -box->halfExtents.y,
               Vec3::Dot(d, rot.GetColumn(2)) > 0 ? box->halfExtents.z : -box->halfExtents.z};
    return body.position + rot.GetColumn(0) * local.x + rot.GetColumn(1) * local.y +
           rot.GetColumn(2) * local.z;
  }
  return body.position;
}

// Minkowski difference support
static Vec3 MinkowskiSupport(const RigidBody& a, const RigidBody& b, const Vec3& d) {
