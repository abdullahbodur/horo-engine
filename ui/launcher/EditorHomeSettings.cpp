...
namespace Horo::Launcher {
    ...
    EditorHomeDocument LoadEditorHomeDocument() {
        ...
        if (const json recentJson = homeJson.value("recentProjects", json::array());
            recentJson.is_array()) {
            for (const json &entry: recentJson) {
                if (!entry.is_string())
                    continue;
                const std::string value =
                        NormalizeRecentProjectPath(entry.get<std::string>());
                if (value.empty())
                    continue;
                out.state.recentProjects.push_back(value);
            }
        }
        out.state.lastProjectPath = NormalizeRecentProjectPath(
            homeJson.value("lastProjectPath", out.state.lastProjectPath));
        return out;
    }
