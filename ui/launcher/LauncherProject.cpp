...
namespace Horo::Launcher {
    ...
    namespace {
        ...
        std::string ExpandTokens(std::string_view value, const fs::path &projectRoot,
            ...
            expanded =
                    ReplaceAll(expanded, "${projectDir}", projectRoot.generic_string());
    ...
    LoadProjectManifestDocument(const fs::path &projectRoot) {
        ...
        out.manifest.schemaVersion =
                projectJson.value("schemaVersion", out.manifest.schemaVersion);
        out.manifest.projectId =
                projectJson.value("projectId", out.manifest.projectId);
        out.manifest.projectName =
                projectJson.value("projectName", out.manifest.projectName);
        out.manifest.defaultScene =
                projectJson.value("defaultScene", out.manifest.defaultScene);

        if (const json commandsJson = projectJson.value("commands", json::object());
            commandsJson.is_object()) {
            out.manifest.configureCommand =
                    ParseCommand(commandsJson.value("configure", json::object()));
            out.manifest.buildCommand =
                    ParseCommand(commandsJson.value("build", json::object()));
            out.manifest.runCommand =
                    ParseCommand(commandsJson.value("run", json::object()));
        }
    ...
    bool SaveProjectManifestDocument(const fs::path &projectRoot,
        ...
        std::error_code ec;
        fs::create_directories(ResolveProjectManifestPath(projectRoot).parent_path(),
                               ec);
        if (ec) {
            if (outError)
                *outError = ec.message();
    ...
    bool ResolveLauncherCommand(const LauncherProjectCommand &command,
        ...
        const std::string workingDirRaw =
                command.workingDirectory.empty()
