...
namespace Monolith {
  ...
  if (ec) {
        LogWarn("UI temp home creation error at '{}': {}",
                homePath.string(), ec.message());
        ec.clear();
    }
  ...
  if (ec) {
    ...
    void ConfigureUiVideoCapture(UiAutomationRunState *state,
        ...
        LogDebug("UI video encoder candidate path: '{}'",
                 encoderPath.string());
    ...
    void LogUiHeartbeat(const UiHeartbeatSnapshot &snapshot) {
        ...
        LogDebug("UI automation heartbeat: frame={}/{} elapsed={:.2f}s frame_dt={:.4f}s "
    ...
    void LogUiEnvVar(const char *name) {
        ...
        LogDebug("UI automation env: {}={}", name,
                 value.empty() ? "<unset>" : value);
    }
    ...
    void UiAutomationRunner::PrepareEnvironmentBeforeAppStart(
        ...
        LogInfo("UI automation pre-start: cwd='{}', temp_root='{}', home_root='{}'",
                fs::current_path().string(), tempRoot.string(),
                homeRoot.string());
    ...
    void UiAutomationRunner::StartIfRequested(
        ...
        LogDebug("UI engine IO config: capture={} run_speed={} fixed_dt={:.4f} "
                 ...
                 testIo.ConfigFixedDeltaTime, testIo.ActionDelayShort,
    ...
    void UiAutomationRunner::PostRenderFrame(
        GLFWwindow *nativeWindowHandle) const {
#ifdef MONOLITH_STANDALONE_UI_AUTOMATION
