...
std::string BuildTemplateMainCpp(const std::string &projectName) {
  return "#include <memory>\n"
         ...
         "  if (!shader) {\n"
         "    shader = "
         "std::make_shared<Shader>(Shader::FromFiles(\"shaders/basic.vert\", "
         "\"shaders/basic.frag\"));\n"
         "  }\n"
         "  return shader;\n"
         "}\n"
         ...
         "  static std::unordered_map<std::string, std::shared_ptr<Mesh>> "
         "cache;\n"
         ...
         "}\n"
         "\n"
         "std::shared_ptr<Texture> LoadTextureIfPresent(const std::string& "
         "path) {\n"
         "  if (path.empty())\n"
         ...
         "    return it->second;\n"
         "  auto texture = "
         "std::make_shared<Texture>(Texture::FromFile(path));\n"
         "  cache[path] = texture;\n"
         "  return texture;\n"
         "}\n"
         "\n"
         ...
         "\n"
         "    m_scene.AddRenderSystem(std::make_unique<RenderSystem>(m_camera, "
         "m_renderAlpha));\n"
         "    m_referenceRuntime = "
         "std::make_unique<SceneReferenceRuntime>(&m_scene);\n"
         "    m_referenceRuntime->SetPropEntityCreatedCallback(\n"
         "        [this](const RuntimeSceneProp& prop, Entity entity, Scene& "
         "sceneRef) {\n"
         ...
         "          sceneRef.GetRegistry().Add<MeshComponent>(entity, "
         "std::move(component));\n"
         "        });\n"
         "  }\n"
         "\n"
         ...
         "        {.requested = RenderBackendId::OpenGL, .nativeWindowHandle = "
         ...
         "      const auto doc = "
         "Editor::SceneSerializer::LoadFromFile(GetDefaultSceneFilePath());\n"
         "      const SceneRuntimeOperationResult result = "
         "m_referenceRuntime->LoadDocument(doc);\n"
         "      if (!result.ok)\n"
         "        throw std::runtime_error(\"Failed to load default scene: \" "
         "+ result.error);\n"
         "      if (m_referenceRuntime->GetSceneCamera().has_value()) {\n"
         "        const RuntimeSceneCamera& camera = "
...
bool CreateLauncherProjectTemplate(
  ...
  if (!WriteTextFile(
          projectRoot / "shaders" / "basic.vert",
          "#version 410 core\n"
  ...
  if (!WriteTextFile(projectRoot / "shaders" / "basic.frag",
                     ...
                     "void main() {\n"
                     "  vec4 base = (u_hasTexture == 1) ? texture(u_albedoMap, "
                     "v_uv) : u_color;\n"
                     "  FragColor = base;\n"
                     "}\n",
                     outError))
