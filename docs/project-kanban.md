...
### TST-002 — Catch2 ile birleşik Dear ImGui UI automation

- **Durum:** İncelenecek
- **Öncelik:** P0
- **Özet:** Catch2 discovery/raporlama otoritesini Dear ImGui Test Engine widget
  otomasyonuyla birleştir; headless ve deterministik editor interaction
  regresyon katmanı oluştur.
- **Kabul:** Test dependency yalnız açık UI-test composition'ında bulunur; tek
  instrumented ImGui implementation kullanılır; input ImGui IO'dan merkezi
  router'a tek akışla taşınır; frame budget, cancellation, exception ve
  lifecycle yolları doğrulanır; aynı test senaryosu varsayılan headless yüzeyde
  veya opt-in SDL/OpenGL gözlem yüzeyinde çalışabilir.
- **Kanıt:** `HoroEditorUiAutomationTests`; native `RunChildTest()` tabanlı
  `UiScenarioPipe`, setup/test child ayrımı, gerçek screen/modal/panel/project
  service composition'ını kullanan `FullEditorUiTestHost`, tekrar kullanılabilir
  `CreateProjectAndOpenWorkspace` setup pipeline'ı, reusable hierarchy/viewport
  action blokları, temel
  Button/Checkbox/Slider/Input/Menu harness contract testi ve full-editor
  project creation → hierarchy create/cross-panel → viewport projection bağlı
  yolculuğu `en-US` ile `tr-TR` altında çalışır. Settings, splitter ve Welcome
  akışlarının full-editor dönüşümü açık kalır.
- **Mimari:** 
- **Son doğrulama:** 2026-07-20 — macOS/AppleClang test-enabled OpenGL
  composition'ında 11 UI/harness Catch2 case geçti. Full-editor journey; Welcome
  → Project Creation → async Project Loading → Workspace yolunu named setup
  children ile bir kez tamamladı, ardından hierarchy ve viewport action'larını
  aynı canlı session üzerinde doğruladı. Tüm executable görünür SDL/OpenGL
  modunda iki locale ile geçti; harness contract fixture'ları zorunlu headless
  kaldı. Linux/GCC, Windows/MSVC, Metal-only
  tekrar doğrulaması ve full matrix kapıları bekleniyor.
...
- **Son doğrulama:** 2026-07-23. AST-001A (registry/provider baseline) Tamamlandı, AST-001B (runtime-scene) Tamamlandı,
  AST-001C (headless-null cooking/cache) Tamamlandı. 63/63 asset tests pass. E2E integration: fixture OBJ+sidecar →
  registry → cook → cache → generation → provider.
...
- **Son doğrulama:** 2026-07-23. `ctest -R "HoroAsset|HoroCook|HoroHeadless|AssetCookInt"` → 63/63 pass. End-to-end
  integration: fixture OBJ+sidecar → registry → cook (cache hit on 2nd run) → generation → FilesystemAssetProvider →
  artifact decode verificatio n. C ABI adapter deferred to Phase B.
