...
They follow ADR-027 pending/ready/released state, non-wrapping generation,
dependency pins and deferred retirement. They are not native addresses, GPU Scene
slots, asset IDs or serializable values. Native device addresses/descriptors stay
inside the backend realization and are invalidated by recreation.
...
Geometry declares opacity/any-hit policy, transform source, material/hit-group
semantic ID and build preference. Buffer descriptors must include the admitted AS
input/device-address usages; bounds and checked byte ranges must fit. Mixed
triangle/AABB support, index absence, transform buffers and update eligibility are
validated explicitly. No adapter reads mesh memory based on a guessed vertex
layout or source C++ struct.

`RayInstanceDescriptor` references a ready BLAS plus a stable GPU Scene instance,
finite origin-relative transform, mask, front-face/culling policy and logical hit-
group contribution. TLAS generations carry exact GPU Scene, origin, mesh/resource
table and material/hit-layout generations. Removed or changed instances cannot
remain reachable in newly published TLAS work, while old frames retain old leases.
...
and effect plans publish together at a render safe point. Failure/cancellation
