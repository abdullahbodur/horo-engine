...
| `HoroEngine::NetworkApi` | Partial | Foundation-only target owns stable replication identities, bounded versioned descriptors, transactional immutable registry snapshots, canonical schema-set fingerprints, and exact transport delivery capability negotiation/admission; transport handles, sessions and runtime remain planned. |
