...
| `HoroEngine::AnimationApi` | Partial | `HoroAnimationApi` owns typed persistent asset identities, stable component/runtime identities, bounded generation-safe handles, stable validation outcomes, and inert authoring/runtime component projections. Registries, evaluation, pose storage, presentation publication, and backend integration remain planned. |
