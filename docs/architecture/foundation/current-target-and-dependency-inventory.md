...
| `HoroEngine::AudioApi` | Partial | Target owns typed identities, generation-safe handles, stable errors, device discovery/default selection, format/layout metadata, requested/effective negotiation admission and backend capability probes; runtime commands and processing execution remain planned. |
