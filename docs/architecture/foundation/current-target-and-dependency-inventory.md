...
| `HoroEngine::NetworkApi` | Partial | Foundation-only target owns stable replication identities, bounded versioned descriptors, transactional immutable registry snapshots and canonical schema-set fingerprints; transport/session/runtime contracts remain planned. |
