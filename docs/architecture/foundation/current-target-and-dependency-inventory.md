...
| `HoroAudioCommands` (`HoroEngine::AudioCommands`) | Always | Owns `Audio/AudioCommands.h` and `Audio/AudioCommandBuffer.h`: typed intents, normalization/coalescing policy and bounded SPSC publication. MPSC ingress remains separate work. | AudioMemory (public) |
...
| `HoroEngine::AudioCommands` | Partial | Typed command values, normalization and bounded SPSC transport; MPSC ingress and end-to-end saturation regressions remain in progress. |
