...
| `HoroAnimationApi` (`HoroEngine::AnimationApi`) | Always | Owns backend-neutral persistent animation asset identities, generation-safe runtime handles, stable validation errors, inert component contracts, and immutable bounded skeleton assets with deterministic hierarchy validation under `Animation/**`. Runtime storage, evaluation, presentation publication, and backend integration remain outside this target. | Foundation, Assets (public) |
...
| `HoroEngine::AnimationApi` | Partial | `HoroAnimationApi` owns typed persistent asset identities, stable component/runtime identities, bounded generation-safe handles, stable validation outcomes, inert component projections, and immutable skeleton assets with canonical parent-before-child order, bind/reference-pose checks, typed mirror/retarget metadata and sockets. Registries, evaluation, pose storage, presentation publication, and backend integration remain planned. |
