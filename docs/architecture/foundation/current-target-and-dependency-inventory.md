...
| `HoroEngine::Physics` | Partial | Foundation/Assets target owns world-scoped identities, stable errors, world/capability preflight, scale-free poses, analytic and exact cooked-shape references, common body policy and fixed/distance constraint descriptors. Optional private pinned Jolt composition verifies native ABI without activation; omitted builds retain the descriptor API. Public SDK isolation and notice installation have regression tests. Full cook-target encoding/envelope verification, additional joint policies, registries and native world execution remain planned. |
