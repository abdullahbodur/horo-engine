...
| `HoroRuntimeUi` (`HoroEngine::RuntimeUi`) | Always | Owns backend-neutral identities, authored/cooked document contracts, canvas modes and deterministic logical-to-screen resolution, canvas references, mutable instance lifecycle, bounded retained element trees, deferred structural transactions, immutable per-view render extraction, dependency enumeration, and typed stale-value rejection under `Runtime/Ui/**`. Renderer realization, output/DPI/safe-area policy, platform, editor, and gameplay ownership remain outside this target. | Foundation, Assets (public) |
