...
| `HoroRuntimeUi` (`HoroEngine::RuntimeUi`) | Always | Owns backend-neutral identities, authored/cooked document contracts, canvas references, mutable instance lifecycle, bounded retained element trees, deferred structural transactions, immutable per-view render extraction, dependency enumeration, and typed stale-value rejection under `Runtime/Ui/**`. Renderer realization, platform, editor, and gameplay ownership remain outside this target. | Foundation, Assets (public) |
...
| `HoroAnimationApi` (`HoroEngine::AnimationApi`) | Always | Owns backend-neutral persistent animation asset identities, generation-safe runtime handles, stable validation errors, inert component contracts, immutable bounded skeleton assets, and generation-bound skeletal-mesh skinning snapshots with deterministic remap, section, palette, and influence validation under `Animation/**`. Runtime storage, evaluation, mutable pose palettes, renderer resources, presentation publication, and backend integration remain outside this target. | Foundation, Assets (public) |
...
| `HoroEngine::RuntimeUi` | Partial | `HoroRuntimeUi` owns typed identities plus distinct authored, cooked, scene-reference, mutable-instance, bounded retained-tree, atomic structural-command, and immutable render-extraction contracts. Service composition, layout, and interaction remain planned. |
...
| `HoroEngine::AnimationApi` | Partial | `HoroAnimationApi` owns typed persistent asset identities, stable component/runtime identities, bounded generation-safe handles, stable validation outcomes, inert component projections, immutable skeleton assets with canonical hierarchy validation, and immutable skeletal-mesh skinning data with generation-safe skeleton binding, bounded palettes, and canonical normalized influences. Registries, evaluation, pose storage, render-resource realization, presentation publication, and backend integration remain planned. |
