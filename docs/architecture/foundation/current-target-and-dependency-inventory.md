...
| `HoroRuntimeUi` (`HoroEngine::RuntimeUi`) | Always | Owns backend-neutral identities, authored/cooked document contracts, canvas references, mutable instance lifecycle, bounded retained element trees, deferred structural transactions, dependency enumeration, and typed stale-value rejection under `Runtime/Ui/**`. Rendering, platform, editor, and gameplay ownership remain outside this target. | Foundation, Assets (public) |
...
| `HoroEngine::RuntimeUi` | Partial | `HoroRuntimeUi` owns typed identities plus distinct authored, cooked, scene-reference, mutable-instance, bounded retained-tree, and atomic structural-command contracts. Service composition, layout, interaction, and extraction remain planned. |
