...
| `HoroPackages` (`HoroEngine::Packages`) | Always | Owns canonical package paths and the typed file inventory under `Packages/**`. JSON and Unicode normalization remain private; archive verification, install/trust/semantic package-manifest services are separate follow-up stages. | Foundation (public) |
