...
| `HoroAnimationApi` (`HoroEngine::AnimationApi`) | Always | Owns backend-neutral persistent animation asset identities, generation-safe runtime handles, stable validation errors, inert component contracts, immutable bounded skeleton and skinning assets, plus preallocated frame pose storage with deterministic hierarchy evaluation and move-only immutable leases under `Animation/**`. Renderer resources, presentation palette publication, graph evaluation, and backend integration remain outside this target. | Foundation, Assets (public) |
