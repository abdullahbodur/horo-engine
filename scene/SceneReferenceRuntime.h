...
class SceneReferenceRuntime {
 public:
  ...
  using PropEntityCreatedCallback = std::function<void(const RuntimeSceneProp& prop,
                                                       Entity entity,
                                                       Scene& scene)>;
  ...
  const SceneRuntimeCoordinator& GetCoordinator() const { return m_coordinator; }
  const std::vector<Light>& GetLights() const { return m_lights; }
  const std::optional<RuntimeSceneCamera>& GetSceneCamera() const { return m_sceneCamera; }
 ...
 private:
  ...
  bool ApplyRuntimeDefinition(const RuntimeSceneDefinition& definition, std::string* error);
