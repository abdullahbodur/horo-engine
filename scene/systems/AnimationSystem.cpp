...
void AnimationSystem::OnUpdate(Registry& registry, float dt) {
  for (Entity e : registry.GetEntities<AnimationComponent>()) {
    ...
    if (ac.loop) {
      ...
      // fmod can return negative values when ac.time is negative (speed < 0 edge case).
      if (ac.time < 0.0f)
        ac.time += duration;
    } else {
    ...
    for (int i = 0; i < boneCount; ++i)
      ac.boneMatrices[i] = globalTransforms[i] * ac.skeleton->GetBone(i).inverseBindMatrix;
  }
}
