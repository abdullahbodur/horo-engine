...
void SkinnedRenderSystem::OnUpdate(Registry& registry, float dt) {
  ...
  // A static empty palette is passed to SubmitSkinned when no AnimationComponent
  // is present (or boneMatrices is not yet populated).  The skinning shader must
  // handle this gracefully — typically by using vertex bone weights of zero so
  // all influence falls back to bone 0 identity, or by an explicit fallback.
  static const std::vector<Mat4> kEmptyBones;

  for (Entity e : registry.GetEntities<SkinnedMeshComponent>()) {
    auto& mc = registry.Get<SkinnedMeshComponent>(e);
