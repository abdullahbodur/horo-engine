...
namespace Monolith {
    ...
    public:
        ...
        Load(const RuntimeSceneDefinition &definition, ApplyFn &&applyCallback) {
        ...
        template<typename ApplyFn>
        SceneRuntimeOperationResult
        Reload(const RuntimeSceneDefinition &definition, ApplyFn &&applyCallback) {
    ...
    private:
        std::unique_ptr<SceneLifecycle> m_lifecycle = std::make_unique<SceneLifecycle>();
