...
namespace Monolith {
    namespace {
        void ValidateSceneAssets(const SceneProjectModel &model,
        ...
        void ValidateSceneNodeIds(const SceneProjectModel &model,
        ...
        void ValidateSceneNodeReferences(
            ...
            const std::unordered_set<std::string, StringHash, std::equal_to<> > &assetIds,
            SceneProjectValidationResult &result) {
            auto addIssue = [&result](SceneProjectValidationIssue::Severity severity,
                                      std::string path, std::string message) {
                result.issues.emplace_back(severity, std::move(path), std::move(message));
