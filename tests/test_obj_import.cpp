...
static bool ParseVec3String(const std::string& s, float& x, float& y, float& z)
{
...
TEST_CASE("ObjLoader::Load parses a valid triangle and returns CPU mesh data", "[objloader][load]")
{
...
TEST_CASE("AssetIdFromImportedPath extracts stem", "[editor][import]")
{
    ...
    REQUIRE(AssetIdFromImportedPath("/deep/nested/dir/door_south.obj") == "door_south");
}
...
{
    // Unix-style path — only the filename is kept and placed under assets/models/.
    REQUIRE(MeshTagFromImportedPath("/some/path/model.obj") == "assets/models/model.obj");
}
...
TEST_CASE("MeshTagFromImportedPath handles nested Unix path", "[editor][import]")
{
    ...
    REQUIRE(MeshTagFromImportedPath("/a/b/c/d/e/mymodel.obj") == "assets/models/mymodel.obj");
}
...
TEST_CASE("SuggestRenderScale returns 1,1,1 for nonexistent file", "[editor][import]")
{
    ...
    const std::string result = SuggestRenderScale("assets/models/no_such_file.obj", 2.0f);
