//  test_math_coverage.cpp
//  Targets previously uncovered paths in math module:
//    Vec2/Vec3/Vec4/Mat3/Mat4 — ToString, Zero, operator*, edge cases
//    Quaternion — ToEuler, LookRotation, Lerp, Dot, Conjugate, Inverse, ToMat3
//    Mat3 — operator==, operator!=, singular Inverse fallback
//    Mat4 — Zero, singular Inverse fallback, operator*, Transposed chain
...
// ============================================================
// Vec2 — ToString + Normalized zero-length
// ============================================================
...
// ============================================================
// Vec3 — ToString + Normalized zero-length
// ============================================================
...
TEST_CASE("Vec3 compound assignment operators", "[math][vec3]")
{
    Vec3 v{1, 2, 3};
    v += Vec3{1, 1, 1};
    REQUIRE(v.x == Approx(2)); REQUIRE(v.y == Approx(3)); REQUIRE(v.z == Approx(4));

    v -= Vec3{1, 1, 1};
    REQUIRE(v.x == Approx(1)); REQUIRE(v.y == Approx(2)); REQUIRE(v.z == Approx(3));

    v *= 2.0f;
    REQUIRE(v.x == Approx(2)); REQUIRE(v.y == Approx(4)); REQUIRE(v.z == Approx(6));
...
TEST_CASE("Vec4 compound arithmetic", "[math][vec4]")
{
    ...
    Vec4 b{4, 3, 2, 1};
    Vec4 sum = a + b;
    REQUIRE(sum.x == Approx(5)); REQUIRE(sum.w == Approx(5));
    Vec4 diff = b - a;
    REQUIRE(diff.x == Approx(3)); REQUIRE(diff.w == Approx(-3));
    Vec4 scaled = a * 2.0f;
    REQUIRE(scaled.z == Approx(6));
}

// ============================================================
// Mat3 — operator==, operator!=, singular inverse, ToString
// ============================================================
...
// ============================================================
// Mat4 — Zero, singular inverse, ToString, Transposed chain
// ============================================================
...
{
    // A perspective matrix produces w != 1, exercising the w-division branch
    Mat4 P = Mat4::Perspective(ToRadians(90.0f), 1.0f, 1.0f, 100.0f);
    // Any point behind the near plane — just check no crash and w-division runs
...
TEST_CASE("Quaternion Lerp produces intermediate orientation", "[math][quat]")
{
    ...
    // Mid should be normalized and not equal to either endpoint
...
{
    Quaternion q = Quaternion::FromAxisAngle({1, 0, 0}, ToRadians(60.0f)).Normalized();
    Quaternion c = q.Conjugate();
    // q * conj(q) should give (0,0,0,1) for unit quaternion
...
TEST_CASE("Quaternion FromEuler yaw creates Z rotation", "[math][quat]")
{
    ...
    Quaternion q = Quaternion::FromEuler(0.0f, angle, 0.0f); // yaw param → Z rotation in this engine
    Vec3 v{1, 0, 0};
    Vec3 r = q * v;
    // 90-deg Z rotation of +X → +Y
...
TEST_CASE("Transform rotation affects direction vectors", "[math][transform]")
{
    ...
    Vec3 fwd = t.Forward();
    // After 90° Y rotation, Forward (-Z) should point to -X
    REQUIRE(fwd.x == Approx(-1.0f).epsilon(1e-4f));
