...
TEST_CASE("Width budget keeps higher priority status items", "[unit][editor][status-bar]")
{
    ...
    Expect(registry.Register(Descriptor("plugin.medium", 50, EditorStatusBarAlignment::Right, 0), Content("CPU")),
           "register medium");
    Expect(
        registry.Register(Descriptor("plugin.low", 1, EditorStatusBarAlignment::Left, 10), Content("Verbose metric")),
        "register low");
    ...
    Expect(std::ranges::any_of(layout.items,
                               [](const EditorStatusItem *item) { return item->descriptor.id == "core.essential"; }),
...
TEST_CASE("Status layout enforces the maximum visible item count", "[unit][editor][status-bar]")
{
    ...
    {
        Expect(registry.Register(Descriptor("vendor.visible." + std::to_string(index), 100 - static_cast<int>(index)),
                                 Content("Item")),
