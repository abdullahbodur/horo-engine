...
namespace
{
        ...
        layout.root = LayoutNode{
            SplitNode{
                ...
                .first = std::make_unique<LayoutNode>(
                    LayoutNode{
                        TabStackNode{.id = "left.stack", .tabs = {"horo.hierarchy"}, .activeTab = "horo.hierarchy"}
                    }),
                .second = std::make_unique<LayoutNode>(
                    LayoutNode{
                        TabStackNode{.id = "right.stack", .tabs = {"horo.inspector"}, .activeTab = "horo.inspector"}
                    }),
            }
        };
    ...
    TEST_CASE("Rejects Moving An Unknown Tab Without Mutation", "[unit][editor]")
    {
        WorkspaceLayout layout;
        layout.root = LayoutNode{
            TabStackNode{.id = "root.stack", .tabs = {"horo.viewport"}, .activeTab = "horo.viewport"}
        };
