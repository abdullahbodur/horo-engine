...
namespace {
    ...
    TEST_CASE("Preserves Actionable Renderer Resolution States", "[unit][editor]") {
        ...
        const auto preflightFor = [&project](const RendererAvailabilityState state) {
            const RendererAvailabilitySnapshot availability{{RendererBackendAvailability{"vulkan", "Vulkan", state, "Action required."}},
                                                            "metal"};
            return PreflightProjectOpen(project.Root(), availability).status;
        };

        REQUIRE((preflightFor(RendererAvailabilityState::RepairRequired) == ProjectOpenPreflightStatus::RendererRepairRequired));
        REQUIRE((preflightFor(RendererAvailabilityState::UpdateRequired) == ProjectOpenPreflightStatus::RendererUpdateRequired));
        REQUIRE((preflightFor(RendererAvailabilityState::AbiMismatch) == ProjectOpenPreflightStatus::RendererCapabilityMismatch));
    }

    TEST_CASE("Rejects Malformed Nested And Duplicate Metadata", "[unit][editor]") {
        TemporaryProject project;
        project.WriteRawMetadata(R"({"formatVersion":1.5,"projectId":"p","name":"n","settings":{"renderBackend":"metal"}})");
    ...
    TEST_CASE("Accepts Unicode Escapes And Rejects Oversized Metadata", "[unit][editor]") {
        TemporaryProject project;
        project.WriteMetadata("metal");
        const Horo::Result<Horo::Application::ProjectMetadata> unicodeMetadata = Horo::Application::LoadProjectMetadata(project.Root());
