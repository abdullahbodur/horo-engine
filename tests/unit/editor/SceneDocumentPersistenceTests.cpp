...
TEST_CASE("Project Scene Save Reopens The Same Authored State", "[unit][editor][persistence]")
{
    ...
    auto saved =
        SaveProjectScene(project.Root(), project.ScenePath(), authored, expected.Value(), false, mutations, files);
    ...
    const std::string loadError =
        loaded.HasError() ? loaded.ErrorValue().code.Value() + ": " + loaded.ErrorValue().message : std::string{};
    INFO(loadError);
...
TEST_CASE("Scene Comparison Classifies Typed Added Removed And Modified Objects",
          "[unit][editor][persistence][compare]")
{
...
TEST_CASE("Failed Atomic Scene Replace Preserves Canonical Bytes", "[unit][editor][persistence]")
{
    ...
    auto expected = InspectProjectSceneFingerprint(project.Root(), project.ScenePath());
    REQUIRE((expected.HasValue()));
    REQUIRE((SaveProjectScene(project.Root(), project.ScenePath(), AuthoredScene(), expected.Value(), false, mutations,
                              files)
                 .HasError()));
...
TEST_CASE("Project Scene Resolver Rejects A Missing Configured Default Scene",
          "[unit][editor][persistence]")
{
    TemporaryProject project;
    project.WriteMetadata();
    REQUIRE((LoadProjectDefaultScene(project.Root()).HasError()));
...
TEST_CASE("Scene Save Detects External Byte Changes Before Atomic Replacement", "[unit][editor][persistence][conflict]")
{
    ...
    auto conflict = SaveProjectScene(project.Root(), project.ScenePath(), AuthoredScene(), expected.Value(), false,
                                     mutations, files);
...
TEST_CASE("Explicit Scene Conflict Overwrite Returns The New Canonical Identity",
          "[unit][editor][persistence][conflict]")
{
    ...
    auto saved = SaveProjectScene(project.Root(), project.ScenePath(), AuthoredScene(), expected.Value(), true,
                                  mutations, files);
    REQUIRE((saved.HasValue()));
    REQUIRE((saved.Value().status == ProjectSceneSaveStatus::Saved));
...
TEST_CASE("Scene Destination Save Rejects Invalid Paths And Detects Races",
          "[unit][editor][persistence][save-as][conflict]")
{
    TemporaryProject project;
    project.WriteMetadata();
    project.WriteScene("{\"schemaVersion\":1,\"objects\":[]}\n");
    ...
    REQUIRE((SaveProjectSceneToPath(project.Root(), std::filesystem::path{"relative.horo"}, AuthoredScene(), true,
                                    nativeMutations, nativeFiles)
                 .HasError()));
    REQUIRE((SaveProjectSceneToPath(project.Root(), project.Root() / "assets/scenes/copy.txt", AuthoredScene(), true,
                                    nativeMutations, nativeFiles)
                 .HasError()));
    REQUIRE((SaveProjectSceneToPath(project.Root(), project.Root().parent_path() / "outside.horo", AuthoredScene(),
                                    true, nativeMutations, nativeFiles)
                 .HasError()));

    const std::filesystem::path outsideDirectory =
        project.Root().parent_path() / (project.Root().filename().string() + "-outside");
    ...
    if (!symlinkError)
    {
        REQUIRE((SaveProjectSceneToPath(project.Root(), linkedDirectory / "escaped.horo", AuthoredScene(), true,
                                        nativeMutations, nativeFiles)
                     .HasError()));
    }
    ...
    auto conflict = SaveProjectSceneToPath(project.Root(), destination, AuthoredScene(), true, interferingMutations,
                                           interferingFiles);
    REQUIRE((conflict.HasValue()));
...
TEST_CASE("Scene Save Rechecks External Identity Immediately Before Replacement",
          "[unit][editor][persistence][conflict]")
{
    TemporaryProject project;
    project.WriteMetadata();
...
TEST_CASE("Scene Recovery Round Trips Without Mutating Canonical Scene", "[unit][editor][persistence][recovery]")
{
    ...
    ProjectMutationCoordinator mutations(files);
    const SceneDocumentSnapshot authored = AuthoredScene();
    REQUIRE((WriteProjectSceneRecovery(project.Root(), project.ScenePath(), authored, DocumentRevision{},
                                       DocumentStateId{1}, mutations, files)
                 .HasValue()));

    std::ifstream canonicalInput(project.ScenePath(), std::ios::binary);
    const std::string canonicalAfterAutosave{std::istreambuf_iterator<char>{canonicalInput},
...
TEST_CASE("Failed Atomic Recovery Replace Leaves No Partial Recovery", "[unit][editor][persistence][recovery]")
{
    ...
    ReplaceFailingFileSystem files;
    ProjectMutationCoordinator mutations(files);
    REQUIRE((WriteProjectSceneRecovery(project.Root(), project.ScenePath(), AuthoredScene(), DocumentRevision{},
                                       DocumentStateId{1}, mutations, files)
                 .HasError()));
    REQUIRE((!std::filesystem::exists(project.RecoveryPath())));
