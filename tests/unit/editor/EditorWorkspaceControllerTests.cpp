...
class SyncFailingFilesystem final : public DurableFileSystem {
  public:
    [[nodiscard]] Result<ExclusiveFileLock> TryAcquireExclusive(
        const std::filesystem::path &path, const std::string_view ownerMetadata) override {
    ...
    [[nodiscard]] Result<void> WriteDurable(const std::filesystem::path &path,
                                            const std::span<const std::byte> bytes) override {
        return native_.WriteDurable(path, bytes);
    }

    [[nodiscard]] Result<void> CopyDurable(const std::filesystem::path &source,
                                           const std::filesystem::path &destination) override {
        return native_.CopyDurable(source, destination);
    }

    [[nodiscard]] Result<void> AtomicReplace(const std::filesystem::path &prepared,
                                             const std::filesystem::path &destination) override {
        ++atomicReplaceCalls;
        if (failReplace)
...
class TestWorkspaceController final {
  ...
  private:
    void PumpLifecycleCommit() {
        ...
        REQUIRE(
            (runtimeScene_.OnPhase(Runtime::RuntimePhase::CommitDeferredLifecycleChanges, context)
                 .HasValue()));
...
TEST_CASE("Workspace Save Persists And Reopens The Default Scene", "[unit][editor][persistence]") {
    ...
    {
        EditorWorkspaceController controller{projectRoot.string(), runtimeScene, {},     nullptr,
                                             &mutations,           &files,       nullptr};
        ...
        const std::filesystem::path canonicalScenePath =
    ...
    {
        EditorWorkspaceController reopened{projectRoot.string(), runtimeScene, {},     nullptr,
                                           &mutations,           &files,       nullptr};
...
TEST_CASE("Workspace Save As Changes Identity While Save Copy As Does Not",
          "[unit][editor][persistence][save-as]") {
    const std::filesystem::path projectRoot =
        std::filesystem::temp_directory_path() /
        ("horo-workspace-save-as-" +
         std::to_string(std::chrono::steady_clock::now().time_since_epoch().count()));
    const std::filesystem::path scenePath = projectRoot / "assets/scenes/main.horo";
    ...
    REQUIRE(
        (controller.CurrentScenePath().value() == std::filesystem::weakly_canonical(scenePath)));
    ...
    REQUIRE((!controller.ViewModel().isDirty));
    REQUIRE(
        (controller.CurrentScenePath().value() == std::filesystem::weakly_canonical(saveAsPath)));
    ...
    {
        std::ofstream external(saveAsPath,
    ...
    REQUIRE((controller.ViewModel().objects.empty()));
    REQUIRE((controller.CurrentScenePath().value() ==
             std::filesystem::weakly_canonical(saveAsPath)));

    std::error_code cleanupError;
    std::filesystem::remove_all(projectRoot, cleanupError);
}

TEST_CASE("Workspace Classifies And Explicitly Restores Scene Recovery",
          "[unit][editor][persistence][recovery]") {
    const std::filesystem::path projectRoot =
        std::filesystem::temp_directory_path() /
        ("horo-workspace-scene-recovery-" +
         std::to_string(std::chrono::steady_clock::now().time_since_epoch().count()));
    const std::filesystem::path scenePath = projectRoot / "assets/scenes/main.horo";
    std::filesystem::create_directories(projectRoot / ".horo");
    std::filesystem::create_directories(scenePath.parent_path());
...

    {
        ...
        controller.ProcessCommand(create);
        controller.UpdateAutosave(60.0F, 1);
        REQUIRE((std::filesystem::exists(projectRoot /
                                         ".horo/local/recovery/default-scene.hororecovery")));
    }
...
TEST_CASE("Workspace Autosave Coalesces Unchanged State And Retries With Backoff",
          "[unit][editor][persistence][recovery]") {
    const std::filesystem::path projectRoot =
        std::filesystem::temp_directory_path() /
        ("horo-workspace-autosave-" +
         std::to_string(std::chrono::steady_clock::now().time_since_epoch().count()));
    const std::filesystem::path scenePath = projectRoot / "assets/scenes/main.horo";
    std::filesystem::create_directories(projectRoot / ".horo");
    std::filesystem::create_directories(scenePath.parent_path());
    ...
    auto recovery = InspectProjectSceneRecovery(std::filesystem::weakly_canonical(projectRoot),
                                                std::filesystem::weakly_canonical(scenePath));
    REQUIRE((recovery.HasValue()));
    REQUIRE((recovery.Value().has_value()));
...
TEST_CASE("Workspace Blocks Conflicting Save Until Reload Or Explicit Overwrite",
          "[unit][editor][persistence][conflict]") {
    const std::filesystem::path projectRoot =
        std::filesystem::temp_directory_path() /
        ("horo-workspace-scene-conflict-" +
         std::to_string(std::chrono::steady_clock::now().time_since_epoch().count()));
    const std::filesystem::path scenePath = projectRoot / "assets/scenes/main.horo";
    std::filesystem::create_directories(projectRoot / ".horo");
    std::filesystem::create_directories(scenePath.parent_path());
    ...
    REQUIRE((controller.ViewModel().objects.empty()));
    REQUIRE((
        !std::filesystem::exists(projectRoot / ".horo/local/recovery/default-scene.hororecovery")));
...
TEST_CASE("Workspace Reports Clean Scene Changes From The Background Watch",
          "[unit][editor][persistence][watch]") {
    const std::filesystem::path projectRoot =
        std::filesystem::temp_directory_path() /
        ("horo-workspace-scene-watch-" +
         std::to_string(std::chrono::steady_clock::now().time_since_epoch().count()));
    const std::filesystem::path scenePath = projectRoot / "assets/scenes/main.horo";
    std::filesystem::create_directories(projectRoot / ".horo");
    std::filesystem::create_directories(scenePath.parent_path());
    ...
    JobSystem jobs(JobSystemConfig{.workerCount = 1, .maxQueuedJobs = 8});
    {
        ...
        for (std::size_t attempt = 0;
             attempt < 100'000 && !controller.ViewModel().sceneExternalConflict; ++attempt) {
...
TEST_CASE("Published Asset Registry Revisions Refresh The Content Browser Projection",
          "[unit][editor]") {
    const std::filesystem::path projectRoot =
        std::filesystem::temp_directory_path() /
        ("horo-workspace-assets-" +
         std::to_string(std::chrono::steady_clock::now().time_since_epoch().count()));
    ...
    REQUIRE((std::filesystem::path{controller.ViewModel().contentBrowser.entries[0].absolutePath}
                 .is_absolute()));
...
TEST_CASE("Content Browser Refresh Exposes Loading And Reconciles Deleted Navigation Targets",
          "[unit][editor]") {
    const std::filesystem::path projectRoot =
        std::filesystem::temp_directory_path() /
        ("horo-workspace-refresh-" +
         std::to_string(std::chrono::steady_clock::now().time_since_epoch().count()));
...
TEST_CASE("Content Browser Rename And Delete Preserve Registry Consistency", "[unit][editor]") {
  ...
  "assetType": "core.mesh"
    ...
    const std::filesystem::path trashEntry = std::filesystem::directory_iterator {
        trashRoot
    } -> path();
    REQUIRE((std::filesystem::is_regular_file(trashEntry / renamed.filename())));
    REQUIRE(
        (std::filesystem::is_regular_file(trashEntry / (renamed.filename().string() + ".horo"))));
...
TEST_CASE("Content Browser Copy And Move Preserve Asset Identity Contracts", "[unit][editor]") {
    const std::filesystem::path projectRoot =
        std::filesystem::temp_directory_path() /
        ("horo-workspace-asset-copy-move-" +
         std::to_string(std::chrono::steady_clock::now().time_since_epoch().count()));
  ...
  "assetType": "core.mesh"
    ...
    REQUIRE(
        (controller.ViewModel().contentBrowserClipboard.mode == ContentBrowserClipboardMode::Copy));
    ...
    REQUIRE(
        (controller.ViewModel().contentBrowserClipboard.mode == ContentBrowserClipboardMode::Move));
    ...
    REQUIRE(
        (controller.ViewModel().contentBrowserClipboard.mode == ContentBrowserClipboardMode::None));
    REQUIRE((controller.ViewModel().contentBrowserOperationError.empty()));
    ...
    const Assets::AssetRecord *dragCopyRecord =
        afterDragCopy.FindByPath("assets/DragCopy/crate.horoasset");
    ...
    const Assets::AssetRecord *dragMoveRecord =
        afterDragMove.FindByPath("assets/DragMove/crate (1).horoasset");
...
TEST_CASE("Content Browser Rejects Unsafe Portable Paths And Companion Sets", "[unit][editor]") {
    ...
    const std::filesystem::path outsideDirectory =
        projectRoot.parent_path() / (projectRoot.filename().string() + "-outside");
    std::filesystem::create_directories(sourceDirectory);
    std::filesystem::create_directories(targetDirectory);
...
TEST_CASE("Content Browser Rename And Delete Roll Back Degraded Registry Publication",
          "[unit][editor]") {
    const std::filesystem::path projectRoot =
        std::filesystem::temp_directory_path() /
...
TEST_CASE("Moving An Active Panel Across Areas Updates Its Runtime Placement", "[unit][editor]") {
    ...
    REQUIRE((viewModel.activityBarLayout.ItemAt(ActivityBarRail::Left, 2, 0) ==
             "horo.global_dock"));
}
...
TEST_CASE("Dropping Into Bottom Right Splits The Full Bottom Dock", "[unit][editor]") {
    ...
    REQUIRE((viewModel.activityBarLayout.FindSlot("horo.inspector") ==
...
TEST_CASE("Dropping A Left Rail Icon Into Bottom Right Moves It To The Right Rail",
          "[unit][editor]") {
    TestWorkspaceController controller;
    ...
    REQUIRE((viewModel.activityBarLayout.FindSlot("horo.hierarchy") ==
             ActivityBarSlot{ActivityBarRail::Right, 2, 0}));
}
...
TEST_CASE("Reordering An Active Icon Moves Its Panel And Activates A Source Fallback",
          "[unit][editor]") {
    TestWorkspaceController controller;
...
TEST_CASE("Dropping On A Side Merge Target Expands The Panel Without Moving Its Icon",
          "[unit][editor]") {
    TestWorkspaceController controller;
...
TEST_CASE("Dropping On The Bottom Merge Target Preserves Its Activity Group", "[unit][editor]") {
    ...
    REQUIRE((viewModel.activeBottomLeftPanelId.empty()));
    REQUIRE((viewModel.activeBottomRightPanelId.empty()));
    REQUIRE((viewModel.activityBarLayout.FindSlot("horo.global_dock") ==
             ActivityBarSlot{ActivityBarRail::Left, 2, 0}));
    REQUIRE((viewModel.activityBarLayout.FindSlot("horo.inspector") ==
             ActivityBarSlot{ActivityBarRail::Right, 2, 0}));
}

TEST_CASE("Reordering An Active Bottom Panel To The Left Does Not Render It Twice",
          "[unit][editor]") {
    TestWorkspaceController controller;
...
TEST_CASE("Scene Commands Publish Committed Events And Drive Undo Redo State", "[unit][editor]") {
    ...
    auto subscription = controller.DataBus().Subscribe<SceneDocumentChangedEvent>(
        [&events](const SceneDocumentChangedEvent &event) {
...
TEST_CASE("Stable Selection Drives Inspector Projection And Reconciles After Delete",
          "[unit][editor]") {
    TestWorkspaceController controller;
    ...
    auto subscription = controller.DataBus().Subscribe<SelectionChangedEvent>(
        [&events](const SelectionChangedEvent &event) {
        events.push_back(event);
    });
...
TEST_CASE("Viewport Picking Uses The Authoritative Selection Model", "[unit][editor]") {
    ...
    hit.viewportPickPayload =
        ViewportPickRequest{.normalizedX = 0.5F, .normalizedY = 0.5F, .aspect = 1.0F};
    ...
    miss.viewportPickPayload =
        ViewportPickRequest{.normalizedX = 0.0F, .normalizedY = 0.0F, .aspect = 1.0F};
...
TEST_CASE("Transform Commands Update The Document Projection Viewport And History",
          "[unit][editor]") {
    TestWorkspaceController controller;
    const SceneObjectId object = controller.ViewModel().objects.front().id;
...
TEST_CASE("Viewport Navigation Updates Only The Editor Camera Authority", "[unit][editor]") {
    ...
    auto subscription = controller.DataBus().Subscribe<ViewportChangedEvent>(
        [&events](const ViewportChangedEvent &event) {
        events.push_back(event);
    });
...
TEST_CASE("Gizmo Preview Is Transient And Commit Creates One Undoable Document Change",
          "[unit][editor]") {
    TestWorkspaceController controller;
    const SceneObjectId object = controller.ViewModel().objects.front().id;
    ...
    REQUIRE((Math::TransformPoint(
                 *controller.ViewModel().primarySelectionPreviewWorldTransform, {}) ==
             edited.translation));
    REQUIRE((documentEvents.empty()));
    REQUIRE((viewportEvents.size() == 1 &&
             viewportEvents.front().kind == ViewportChangeKind::ScenePreviewChanged));
    ...
    REQUIRE((controller.ViewModel().isDirty && controller.ViewModel().canUndo));
    REQUIRE((documentEvents.size() == 1 &&
             documentEvents.front().kind == DocumentChangeKind::TransformChanged));
    REQUIRE((!controller.ViewModel().primarySelectionPreviewWorldTransform.has_value()));
    REQUIRE((controller.CurrentViewportRevision() == ViewportRevision{2}));
    REQUIRE((viewportEvents.size() == 2 &&
             viewportEvents.back().kind == ViewportChangeKind::ScenePreviewChanged));
...
TEST_CASE("Inspector Camera Update Uses Typed Document History", "[unit][editor]") {
    ...
    const auto updated = std::ranges::find(
        controller.ViewModel().objects, camera.id, &SceneObject::id);
...
TEST_CASE("Cancelling Gizmo Preview Restores The Exact Committed Projection", "[unit][editor]") {
    TestWorkspaceController controller;
    const SceneObjectId object = controller.ViewModel().objects.front().id;
    const Math::Transform committedTransform =
        controller.ViewModel().objects.front().localTransform;
...
TEST_CASE("Selection Change Cancels A Transient Inspector Transform Preview",
          "[unit][editor]") {
    TestWorkspaceController controller;
    ...
    const auto firstInstance = std::ranges::find(
        controller.ViewportScene().instanceObjects, first.id);
    REQUIRE((firstInstance != controller.ViewportScene().instanceObjects.end()));
    const std::size_t firstIndex = static_cast<std::size_t>(
        std::distance(controller.ViewportScene().instanceObjects.begin(), firstInstance));
    REQUIRE((Math::TransformPoint(
        controller.ViewportScene().instances[firstIndex].localToWorld, {}) ==
    ...
    REQUIRE((Math::TransformPoint(
        controller.ViewportScene().instances[firstIndex].localToWorld, {}) ==
        first.localTransform.translation));
    REQUIRE((controller.ViewModel().documentRevision == committedRevision));
