...
TEST_CASE("Mirrors The Workspace Design Menu Hierarchy", "[unit][editor]") {
    ...
    constexpr std::array expectedTopLevelKeys = {
        "web_workspace.menu.file",      "web_workspace.menu.edit",
        "web_workspace.menu.assets",    "web_workspace.menu.game_object",
        "web_workspace.menu.component", "web_workspace.menu.window",
    ...
    const EditorMenuItem *assetEditors =
        FindChild(model.menus[5], "web_workspace.menu.asset_editors");
...
TEST_CASE("Distinguishes Implemented And Placeholder Commands", "[unit][editor]") {
    ...
    const EditorMenuItem *saveCopyAs =
        FindAction(model.menus[0], EditorMenuAction::SaveSceneCopyAs);
...
TEST_CASE("Builds The Shared Catalog Create Tree", "[unit][editor]") {
    ...
    for (std::size_t index = 0; index < expectedGroups.size(); ++index) {
        ...
        for (const EditorMenuItem &primitive : items[index + 1].children) {
            ...
            REQUIRE(
                (primitive.primitive->value.find("primitive.collider.") == std::string_view::npos));
        }
    }
