...
namespace {
    ...
    [[nodiscard]] std::uint32_t ReadU32(const std::span<const std::uint8_t> bytes, const std::size_t offset) {
    ...
    [[nodiscard]] const AssetImporterContribution &FbxContribution(AssetImporterCatalog &catalog,
...
TEST_CASE("FBX importer combines transformed scene meshes into a shaded preview", "[native]") {
    ...
    const std::vector<std::uint8_t> source = ReadFixture("blender_279_nested_meshes_7400_binary.fbx");
