...
namespace Horo::WorldStreaming {
    namespace {
        ...
        StreamingSourceOwnerToken Owner(const std::uint32_t generation = 1, const PartitionEpoch epoch = IdentityFrom<PartitionEpoch>(1)) {
            return {.partition = World(), .epoch = epoch, .slot = 3, .generation = generation};
        }

        StreamingSourceDescriptor Descriptor(const StreamingSourceRevision revision = IdentityFrom<StreamingSourceRevision>(1)) {
        ...
        StreamingSourceAdmissionContext Context() {
