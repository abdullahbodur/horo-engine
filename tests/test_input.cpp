...
//
// Coverage:
//   - FPSCameraController: construction defaults, SetYaw/GetYaw, SetPitch/GetPitch,
...
TEST_CASE("FPSCameraController: default sensitivity is accessible via setter/getter cycle",
          "[input][fps_camera]") {
    FPSCameraController ctrl;
...
TEST_CASE("FPSCameraController: pitch limits survive a round-trip through setters",
          "[input][fps_camera]") {
    FPSCameraController ctrl;
...
TEST_CASE("FPSCameraController: ApplyToCamera with non-zero position offsets target",
          "[input][fps_camera]") {
    FPSCameraController ctrl;
...
TEST_CASE("FPSCameraController: Update with zero mouse delta leaves yaw unchanged",
          "[input][fps_camera]") {
...
TEST_CASE("FPSCameraController: Update with zero mouse delta leaves pitch unchanged",
          "[input][fps_camera]") {
    FPSCameraController ctrl;
    ctrl.SetYaw(0.0f);
    ctrl.SetPitch(30.0f);
...
TEST_CASE("FPSCameraController: Update clamps pitch within pitchMin/pitchMax",
    ...
    // Set pitch to pitchMax (70) — it should remain unchanged (still within range).
    FPSCameraController ctrl;
    ctrl.SetPitchMin(-70.0f);
    ctrl.SetPitchMax(70.0f);
...
TEST_CASE("Input: mouse transition queries reflect current and last button state",
          "[input]") {
    ResetInputState();
...
TEST_CASE("Input: mouse position/delta accessors return stored values", "[input]") {
    ResetInputState();
