...
TEST_CASE("Sphere-plane impulse reverses vertical velocity", "[physics][collision]")
{
    ...
    sphere.position  = {0, 0.5f, 0}; // sitting at floor level exactly (radius = 0.5)
...
TEST_CASE("SAT::TestBoxBox normal points from B toward A", "[physics][sat]")
{
    ...
    // A is at x=0, B is at x=2. They overlap when A's right face reaches B's left face.
    ...
    // Normal must point from B (wall) toward A (player) — i.e. in the -X direction
