...
TEST_CASE(
    "LoadProjectManifestDocument uses manifest defaults when project key absent",
    "[launcher][project]") {
    const fs::path root =
    ...
    fs::remove_all(root, ec);
    fs::create_directories(root / ".horo", ec);
    // No "project" key: code falls back to embedded defaults (schemaVersion=1, etc.)
...
TEST_CASE("RememberRecentProject prepends and deduplicates entries",
          "[launcher][home]") {
    const fs::path projA =
            Monolith::Tests::SecureTempBase() / "horo_home_projA";
    const fs::path projB =
            Monolith::Tests::SecureTempBase() / "horo_home_projB";
    std::error_code ec;
    ...
    CHECK(doc.state.lastProjectPath.find("horo_home_projA") !=
        std::string::npos);
}

TEST_CASE("RememberRecentProject with null doc does not crash",
          "[launcher][home]") {
    REQUIRE_NOTHROW(RememberRecentProject(nullptr, Monolith::Tests::SecureTempBase()));
}

TEST_CASE("PruneMissingRecentProjects removes non-existent paths",
          "[launcher][home]") {
    EditorHomeDocument doc;
    doc.state.recentProjects.emplace_back("/nonexistent/path/that/does/not/exist");
    ...
    WriteFile(existing / ".horo" / "project.json",
...
TEST_CASE("LoadEditorHomeDocument: reads recentProjects from disk",
    ...
    const std::string projectPath =
            (tempHome / "MyProject").generic_string();
    fs::create_directories(tempHome / "MyProject", ec);

    const std::string json =
            R"({"home":{"recentProjects":[")" + projectPath + R"("],"lastProjectPath":")" +
            projectPath + R"("}})";
