...
TEST_CASE("ProjectPath: Init discovers project root upward and seeds sdk root",
    ...
    const fs::path tempRoot = Monolith::Tests::SecureTempBase() / "horo_projectpath_init";
    std::error_code ec;
    fs::remove_all(tempRoot, ec);
...
TEST_CASE("Shader: FromFiles throws when second shader file is missing",
    ...
    const fs::path tempRoot = Monolith::Tests::SecureTempBase() / "horo_shader_io";
    std::error_code ec;
    fs::remove_all(tempRoot, ec);
    fs::create_directories(tempRoot, ec);
...
TEST_CASE("EngineLaunchArgs: ParseEditorStartupCli delegates to launch option "
    ...
    std::array<char *, 4> argv = {
        arg0.data(), nullptr, editorArg.data(),
        playArg.data()
    };
...
TEST_CASE("EngineLaunchArgs: --project without usable value keeps defaults",
    ...
    std::array<char *, 4> argv = {
        arg0.data(), projectFlag.data(), nullptr,
        unknown.data()
    };
...
TEST_CASE("StringHash: operator() hashes string_view consistently",
    ...
    REQUIRE(hasher(std::string_view("hello")) == hasher(std::string_view("hello")));
    REQUIRE(hasher(std::string_view("hello")) != hasher(std::string_view("world")));
}
