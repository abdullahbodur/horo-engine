...
bool ReturnToLauncherFromEditor(ImGuiTestContext *ctx,
  ...
  ctx->SetRef("##toolbar");
  if (const bool fileVisible = WaitForCondition(
          ctx, 90, [ctx]() { return ctx->ItemExists("File"); });
      fileVisible) {
    ctx->ItemClick("File");
    ctx->Yield(1);
...
void RunLauncherCreateAndVerifyToolbarTest(ImGuiTestContext *ctx) {
  ...
  ctx->SetRef("##toolbar");
  const bool fileMenuReady =
      WaitForCondition(ctx, 180, [ctx]() { return ctx->ItemExists("File"); });
  IM_CHECK(fileMenuReady);
  if (!fileMenuReady)
    return;
...
ImGuiTest *
RegisterLauncherCreateAndVerifyToolbarTest(ImGuiTestEngine *engine,
                                           UiAutomationRunState *state) {
  ImGuiTest *test = IM_REGISTER_TEST(engine, "launcher_ui",
