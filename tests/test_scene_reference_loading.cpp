...
namespace {
    std::string TmpPath(const std::string &name) {
        return (Monolith::Tests::SecureTempBase() /
                "horo_scene_reference_loading" / name)
                .string();
    }
    ...
    SceneDocument
    MakeReferenceSceneDocument(std::string_view filePath,
                               std::string_view sceneId, int panelCount,
                               int propCount, int lightCount, bool includeCamera,
...
TEST_CASE(
    ...
    REQUIRE(scene.GetRegistry().Get<MeshComponent>(propEntity).meshTag == "crate.obj");
