...
TEST_CASE("Vec3 distance", "[math][vec3]")
{
    ...
    REQUIRE(Vec3::Distance(b, a) == Approx(5.0f)); // symmetric
}

TEST_CASE("Vec3 reflect", "[math][vec3]")
{
    // Reflect (1,-1,0) off floor (normal = up)
    Vec3 incident{1, -1, 0};
    Vec3 normal = Vec3::Up(); // (0,1,0)
    Vec3 r = Vec3::Reflect(incident, normal);
    // v - 2*dot(v,n)*n = (1,-1,0) - 2*(-1)*(0,1,0) = (1,1,0)
...
TEST_CASE("Mat3 GetRow", "[math][mat3]")
{
    ...
    //   row 0 = (m[0][0], m[1][0], m[2][0]) = (1, 4, 7)
    //   row 1 = (m[0][1], m[1][1], m[2][1]) = (2, 5, 8)
...
{
    // Diagonal matrix with values 1, 2, 3 → det = 6
...
TEST_CASE("Mat4 Data returns column-major layout", "[math][mat4]")
{
    ...
    // For Translate: m[3][0]=1, m[3][1]=2, m[3][2]=3, m[3][3]=1
    // data[12] = m[3][0] = 1
...
TEST_CASE("Mat4 LookAt: forward vector points toward center", "[math][mat4]")
{
    ...
    // In view space, the center (0,0,0) should be at negative Z
...
{
    float l = -10, r = 10, b = -10, t = 10, n = 0, f = 100;
    Mat4 O = Mat4::Orthographic(l, r, b, t, n, f);
    // Center point of the view volume in X and Y
    float cx = (l + r) / 2;
    float cy = (b + t) / 2;
...
TEST_CASE("Quaternion dot product", "[math][quat]")
{
    ...
    // dot should be cos(45°) ≈ 0.7071 (half-angle)
...
{
    // In this engine: pitch parameter creates a Y-axis rotation
...
{
    // In this engine: roll parameter creates an X-axis rotation
    float angle = ToRadians(90.0f);
...
TEST_CASE("Transform identity: ToMatrix is identity", "[math][transform]")
{
    ...
    Mat4 M = t.ToMatrix();
    // Should transform (0,0,0) to (0,0,0) and (1,0,0) to (1,0,0)
