...
//
// Coverage:
//   - Skeleton: construction, bone lookup, IsValid(), parentIndex/inverseBindMatrix storage
//   - AnimationClip: duration, track management, Sample() interpolation (position/rotation/scale),
//     boundary clamping, absent-channel defaults, per-bone isolation
//   - AnimationSystem: time advancement, looping, pausing, null-guard, bone matrix output,
...
// Build a two-bone skeleton: bone 0 is Root (no parent), bone 1 is Child (parent=0).
...
TEST_CASE("Skeleton: FindBone returns correct index by name", "[skeleton]") {
  ...
  Bone b0; b0.name = "Spine";  b0.parentIndex = -1; b0.inverseBindMatrix = Mat4::Identity();
  Bone b1; b1.name = "Head";   b1.parentIndex =  0; b1.inverseBindMatrix = Mat4::Identity();
  Bone b2; b2.name = "LeftArm"; b2.parentIndex = 0; b2.inverseBindMatrix = Mat4::Identity();
  skel.AddBone(b0);
  skel.AddBone(b1);
  skel.AddBone(b2);
...
TEST_CASE("Skeleton: FindBone returns -1 for unknown name", "[skeleton]") {
  Skeleton skel;
  Bone b; b.name = "Root"; b.parentIndex = -1; b.inverseBindMatrix = Mat4::Identity();
  skel.AddBone(b);
...
TEST_CASE("Skeleton: root bone has parentIndex == -1", "[skeleton]") {
  Skeleton skel;
  Bone root; root.name = "Root"; root.parentIndex = -1; root.inverseBindMatrix = Mat4::Identity();
  skel.AddBone(root);
...
TEST_CASE("AnimationClip: single-key position track returns that key at any time", "[clip]") {
  ...
  // Sample at various times — single-key track must always return the same value.
...
TEST_CASE("AnimationClip: two-key rotation track slerps at t=0.5", "[clip]") {
  ...
  const Quaternion qEnd   = Quaternion::FromAxisAngle(Vec3{0.0f, 1.0f, 0.0f},
                                                       3.14159265f * 0.5f);  // 90 deg Y
...
TEST_CASE("AnimationClip: position-only track defaults rotation to identity and scale to one",
          "[clip]") {
  AnimationClip clip;
  clip.duration = 1.0f;
  ...
  // With identity rotation and unit scale, the TRS matrix equals Translate(0) = identity.
  const Mat4 identity = Mat4::Identity();
  for (int col = 0; col < 4; ++col)
    for (int row = 0; row < 4; ++row)
...
TEST_CASE("AnimationClip: multiple tracks for different bones each get their own data",
          "[clip]") {
  AnimationClip clip;
  clip.duration = 1.0f;
...
TEST_CASE("AnimationSystem: advances time by dt * speed", "[animation-system]") {
  Registry reg;
  Entity e = reg.Create();
  auto& ac = AddAnimComp(reg, e, MakeSingleBoneSkeleton(), MakePositionClip(1.0f));
  ac.speed    = 2.0f;
  ac.time     = 0.0f;
  ac.loop     = false;
...
TEST_CASE("AnimationSystem: boneMatrices.size() equals skeleton BoneCount after update",
          "[animation-system]") {
  Registry reg;
  Entity e = reg.Create();
...
TEST_CASE(
    "AnimationSystem: single root bone with identity inverseBindMatrix and position track — "
    "boneMatrices[0] encodes the sampled position at t=0.5",
    "[animation-system]") {
  // The clip has a position track: pos(0)=(0,0,0), pos(1)=(1,0,0).
  // At time=0 after the update (dt=0.5, so time advances from 0 to 0.5 with loop):
  ...
  // Translation column of the final skinning matrix must match the interpolated position.
...
TEST_CASE(
    "AnimationSystem: single bone at t=0 with position (0,0,0) produces identity boneMatrix",
    "[animation-system]") {
  // At t=0 the only position key is (0,0,0), rotation defaults to identity, scale to (1,1,1).
  // inverseBindMatrix is identity.  Expected result: identity.
  Registry reg;
  Entity e = reg.Create();
...
TEST_CASE(
    "AnimationSystem: two-bone hierarchy — parent transform propagates into child global matrix",
    "[animation-system]") {
  // Bone 0 (Root):  translates +2 along X.
  // Bone 1 (Child): translates +3 along X, parent = 0.
  //
  // Expected chain:
  //   globalTransform[0] = Translate({2,0,0})
  //   globalTransform[1] = globalTransform[0] * Translate({3,0,0}) = Translate({5,0,0})
  //   boneMatrices[1]    = globalTransform[1] * Identity            = Translate({5,0,0})

  auto skel = MakeTwoBoneSkeleton();  // both bones have Identity inverseBindMatrix
  ...
  // Child bone: global translation should be parent (2) + child (3) = 5 along X.
...
TEST_CASE(
  ...
  // After 90-degree Y rotation, the child's world position becomes (0,0,-1) in X-Z plane.
  //
  // globalTransform[0] = Rotate(90Y)
  // globalTransform[1] = Rotate(90Y) * Translate({1,0,0})
  // boneMatrices[1]    = globalTransform[1] * Identity
  //
  // Rotate(90Y) * (1,0,0,1) → approximately (0,0,-1) in world space (right-hand coords).
  // We verify via the boneMatrices[1] translation column.
  ...
  const Quaternion q90Y = Quaternion::FromAxisAngle(Vec3{0.0f, 1.0f, 0.0f},
                                                     3.14159265f * 0.5f);
  ...
  // The translation column of a product (Rotate * Translate) equals Rotate applied to
  // the translation vector of the Translate matrix, which is (1,0,0).
  // q90Y * (1,0,0) ≈ (0, 0, -1).
...
TEST_CASE(
    "AnimationSystem: non-identity inverseBindMatrix is applied to final bone matrix",
    "[animation-system]") {
  // Single bone.  inverseBindMatrix translates -2 along X.
  // Clip positions the bone at +2 along X.
  // Expected boneMatrix = Translate(2,0,0) * Translate(-2,0,0) = Identity translation.
...
TEST_CASE(
    "AnimationSystem: non-looping clip clamps time at duration when dt overshoots",
    "[animation-system]") {
  Registry reg;
  Entity e = reg.Create();
  auto& ac = AddAnimComp(reg, e, MakeSingleBoneSkeleton(), MakePositionClip(1.0f));
...
TEST_CASE(
    "AnimationSystem: boneMatrices is resized correctly when skeleton grows between updates",
    "[animation-system]") {
  // Simulate an entity whose skeleton reference is replaced mid-game (not a realistic
  // engine operation, but verifies that the system resizes on every update rather than
  // assuming a stable size).
  Registry reg;
  Entity e = reg.Create();
  AddAnimComp(reg, e, MakeSingleBoneSkeleton(), MakePositionClip(1.0f));
  ...
  ac.currentClip = MakePositionClip(1.0f);  // track is for bone 0 only — bone 1 gets identity
