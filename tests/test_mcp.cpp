...
namespace {
    ...
    void PopulateSchemaSnapshot(McpEditorSnapshot *snapshot) {
        ...
        script.fields.push_back(
            MakeSchemaField("behaviorTag", "Behavior", "string", "", {},
    ...
    McpEditorSnapshot MakeSnapshot() {
        ...
        snapshot.consoleEntries.emplace_back("12:00:01", "WARN",
    ...
    json ProtocolRequest(const McpProtocol &protocol, std::string_view method,
        ...
        const McpHttpResponse response =
                protocol.HandleHttp(McpHttpRequest{
                    "POST",
                    "/mcp",
                    {},
                    json{
    ...
    json CallTool(const McpProtocol &protocol, std::string_view name,
        ...
        return ProtocolRequest(protocol, "tools/call",
                               json{
...
TEST_CASE("Editor MCP commands preserve reserved ids and reload from disk",
    ...
    const std::filesystem::path scenePath =
            Monolith::Tests::SecureTempBase() / "horo_editor_mcp_consistency" /
            "world.json";
    ...
    const McpCommandResult stringVecFail = editor.ExecuteMcpCommand(
        "editor.create_object", json{
            {"id", "codex_probe_panel"},
            {"type", "Panel"},
...
TEST_CASE("Editor MCP delete_asset reports managed file deletion details",
    ...
    doc.assets["stone"] = AssetDef{
        "assets/models/stone/stone.obj", "1,1,1",
...
TEST_CASE(
    ...
    std::thread selectThread([&settings,
                              promise = std::move(selectPromise)]() mutable {
            promise.set_value(SendHttpPost(settings.port,
                                           json{
                                               {"jsonrpc", "2.0"},
                                               {"id", 3},
                                               {"method", "tools/call"},
...
TEST_CASE("McpProtocol accepts underscore aliases for tool names",
    ...
    const json canonical = CallTool(protocol, "editor.search",
                                    json{{"query", "obj"}}, 910);
    const json alias =
...
TEST_CASE("McpSettings: SanitizePort clamps out-of-range values via SaveMcpSettings",
          "[mcp][settings]") {
...
TEST_CASE("McpSnapshot: BuildSelectionJson with empty selection has no asset key",
          "[mcp][snapshot]") {
    McpEditorSnapshot snapshot = MakeSnapshot();
...
TEST_CASE(
    "McpProtocol: notifications are accepted with 202 and no body",
    "[mcp][protocol]") {
    McpEditorSnapshot snapshot = MakeSnapshot();
    McpProtocol protocol(McpProtocolContext{
...
TEST_CASE(
    ...
    const json enumNotString =
            CallTool(protocol, "editor.create_object",
                     json{{"type", "Light"}, {"props", json{{"lightType", 99}}}},
                     700);
    ...
    const json colorArray = CallTool(
        protocol, "editor.create_object",
        json{
            {"type", "Light"},
            {"id", "new_light_col"},
            {
                "props",
                json{{"lightType", "point"}, {"color", json::array({1.0, 0.5, 0.0})}}
            }
        },
        707);
...
TEST_CASE(
    ...
    const json modeNumber = CallTool(
        protocol, "editor.select",
        json{{"id", "obj_root"}, {"mode", 123}},
        710);
    REQUIRE(modeNumber["error"]["message"] ==
        "mode must be \"preview\" or \"apply\".");

    // mode is an unrecognised string
    const json modeInvalid = CallTool(
        protocol, "editor.select",
        json{{"id", "obj_root"}, {"mode", "execute"}},
        711);
...
TEST_CASE(
    ...
    const json badParentType = CallTool(
        protocol, "editor.reparent_object",
        json{{"id", "obj_root"}, {"parentId", 42}},
        720);
    REQUIRE(badParentType["error"]["message"] ==
        "parentId must be a string or null.");

    // parentId is the object itself
    const json selfParent = CallTool(
        protocol, "editor.reparent_object",
        json{{"id", "obj_root"}, {"parentId", "obj_root"}},
        721);
    ...
    const json cycle = CallTool(
        protocol, "editor.reparent_object",
        json{{"id", "obj_root"}, {"parentId", "obj_light"}},
        722);
...
TEST_CASE(
    ...
    const json unknownType = CallTool(
        ...
        json{
            ...
            {
                "components",
                json::array({json{{"type", "nonexistent"}, {"props", json::object()}}})
            }
        },
        741);
