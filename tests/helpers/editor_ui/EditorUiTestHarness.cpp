...
namespace Horo::Tests
{
    namespace
    {
        ...
        constexpr ImGuiKey kBridgedKeys[]{
            ImGuiKey_A, ImGuiKey_D, ImGuiKey_E, ImGuiKey_F,
                                             ...
                                             StepFunction operation)
    {
        ...
        const ImGuiTestStatus childStatus = context_.
            RunChildTest(childName.c_str(), ImGuiTestRunFlags_ShareTestContext);
    ...
    EditorUiTestHarness::EditorUiTestHarness(std::unique_ptr<IEditorUiTestSurface> surface) : surface_(
        std::move(surface))
    {
        if (surface_ == nullptr)
    ...
    EditorUiScenarioResult EditorUiTestHarness::RunScenario(std::string category, std::string name, GuiFunction gui,
                                                            ...
                                                            const EditorUiScenarioBudget budget,
                                                            Input::InputRouter* router)
    {
        return RunInternal(
            std::move(category), std::move(name), std::move(gui),
                                                            ...
                                                            Input::InputRouter* router)
    {
        ...
        ImGuiTest* registered = ImGuiTestEngine_RegisterTest(engine_, category.c_str(), name.c_str(), __FILE__,
                                                             __LINE__);
        ...
        ImGuiTextBuffer log;
        registered->Output.Log.ExtractLinesForVerboseLevels(ImGuiTestVerboseLevel_Error, ImGuiTestVerboseLevel_Trace,
                                                            &log);
