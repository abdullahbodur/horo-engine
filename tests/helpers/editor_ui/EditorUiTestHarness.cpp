...
namespace Horo::Tests {
    namespace {
        ...
        constexpr std::size_t kCinematicFrameBudgetScale = 30;
    }  // namespace
    ...
    UiScenarioPipe &UiScenarioPipe::RunChild(const UiScenarioStepKind kind, const std::string_view name, StepFunction operation) {
        ...
        const std::string childName = std::string{context_.Test->Name} + "::step_" + std::to_string(stepIndex_++) + "::" + step.name;
        ImGuiTest *child = IM_REGISTER_TEST(context_.Engine, "horo_scenario_step", childName.c_str());
        child->SetOwnedName(childName.c_str());
        child->TestFunc = [operation = std::move(operation), reference = reference_, &result = result_](ImGuiTestContext *childContext) {
            try {
                if (!reference.empty())
    ...
    EditorUiScenarioBudget EditorUiScenarioBudget::Extended(const std::size_t maximumFrames) {
        if (maximumFrames <= 600 || maximumFrames > 6000)
            throw std::invalid_argument("An extended UI scenario budget must be in the range 601..6000 frames.");
        ...
        try {
            ...
            if (surface_->IsInteractive()) {
                if (const char *const envSpeed = std::getenv("HORO_UI_TEST_SPEED"); envSpeed != nullptr && envSpeed[0] != '\0') {
                    ...
                    else if (std::string_view{envSpeed} == "fast")
    ...
    EditorUiScenarioResult EditorUiTestHarness::Run(std::string category, std::string name, GuiFunction gui, TestFunction test,
                                                    const EditorUiScenarioBudget budget, Input::InputRouter *router) {
        return RunInternal(std::move(category), std::move(name), std::move(gui),
                           [test = std::move(test)](ImGuiTestContext *context, EditorUiScenarioResult &) {
            test(context);
        }, budget, router);
    }
    ...
    EditorUiScenarioResult EditorUiTestHarness::RunInternal(std::string category, std::string name, GuiFunction gui,
        ...
        ranScenario_ = true;
        if (budget.maximumFrames == 0 || budget.maximumFrames > 6000)
            throw std::invalid_argument("UI scenario frame budget must be in the range 1..6000.");
        ...
        registered->GuiFunc = [gui = std::move(gui), &bridge, &result, &coroutineAbortRequested](ImGuiTestContext *context) {
            try {
        ...
        std::size_t effectiveMaxFrames = budget.maximumFrames;
        const ImGuiTestRunSpeed runSpeed = ImGuiTestEngine_GetIO(engine_).ConfigRunSpeed;
        if (runSpeed == ImGuiTestRunSpeed_Cinematic)
            effectiveMaxFrames *= kCinematicFrameBudgetScale;
        else if (runSpeed == ImGuiTestRunSpeed_Normal)
            effectiveMaxFrames *= kNormalInteractiveFrameBudgetScale;

        while (!ImGuiTestEngine_IsTestQueueEmpty(engine_) && result.simulatedFrames < effectiveMaxFrames) {
            try {
        ...
        if (!result.cancelled && !coroutineAbortRequested.load(std::memory_order_acquire) && !ImGuiTestEngine_IsTestQueueEmpty(engine_)) {
    ...
    IEditorUiTestSurface &EditorUiTestHarness::Surface() noexcept {
        return *surface_;
    }
}  // namespace Horo::Tests
