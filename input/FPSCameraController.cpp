...
void FPSCameraController::Update(float /*dt*/) {
  ...
  m_pitch = std::max(m_pitchMin, std::min(m_pitchMax, m_pitch));
}

void FPSCameraController::SetPosition(const Vec3 &pos) { m_position = pos; }

void FPSCameraController::SetYaw(float yaw) { m_yaw = yaw; }

void FPSCameraController::SetPitch(float pitch) { m_pitch = pitch; }
