...
class IStructuredLogQuery {
  public:
     ...
     */
    [[nodiscard]] virtual std::optional<StructuredLogSnapshot> SnapshotIfChanged(
        std::uint64_t knownRevision) const = 0;
};
...
class StructuredLogStore final : public IStructuredLogQuery {
  public:
    ...
    [[nodiscard]] std::optional<StructuredLogSnapshot> SnapshotIfChanged(
        std::uint64_t knownRevision) const override;
