...
struct ExtensionInventoryEntry
{
    ...
    {
        return enabled != runtimeActive ||
            (runtimeActive && compositionVersion != runtimeCompositionVersion);
    }
};
...
class ExtensionInventory final
{
  public:
     ...
     */
    [[nodiscard]] Result<std::string> InstallFromDirectory(
        const std::filesystem::path& absoluteSourceDirectory);
