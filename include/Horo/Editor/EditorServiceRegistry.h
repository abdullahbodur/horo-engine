...
class EditorServiceRegistry
{
  public:
    ...
    template <typename T> void Register(T &serviceInstance)
    {
        services_[std::type_index(typeid(T))] = &serviceInstance;
    }

    template <typename T> void RegisterConst(const T &serviceInstance)
    {
    ...
    template <typename T> [[nodiscard]] const T &GetConst() const
    {
    ...
    template <typename T> [[nodiscard]] T *TryGet() const noexcept
    {
    ...
    template <typename T> [[nodiscard]] const T *TryGetConst() const noexcept
    {
        if (const auto it = constServices_.find(std::type_index(typeid(T))); it != constServices_.end())
        {
            return static_cast<const T *>(it->second);
