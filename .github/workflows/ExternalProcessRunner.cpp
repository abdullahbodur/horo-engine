...
#ifdef _WIN32
static void ReadWindowsPipe(HANDLE hRead, const std::string &label,
...
int StopPosixProcess(pid_t *pid) {
  ...
  for (int attempt = 0; attempt < 50; ++attempt) {
    if (const bool ok = TryWaitForPid(*pid, &status, WNOHANG, &reaped); !ok || reaped)
      break;
...
bool ExternalProcessRunner::Start(
    const ResolvedLauncherCommand &command,
