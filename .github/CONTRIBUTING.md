...
- Python 3.10+ (developer commands and pre-commit hooks)
...
python3 scripts/dev.py test           # run unit tests
python3 scripts/dev.py format --check # verify formatting
```
...
- [ ] `python3 scripts/dev.py format --check` passes (run without `--check` to auto-fix)
