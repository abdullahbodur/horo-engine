#include "SceneRuntimeCoordinatorBridge.h"
...
SceneRuntimeOperationResult MakeBuildFailure(SceneRuntimeOperation operation,
                                             SceneLifecycleState state,
...
SceneRuntimeOperationResult LoadSceneDocument(SceneRuntimeCoordinator& coordinator,
  ...
  const RuntimeSceneBuildResult buildResult = BuildRuntimeSceneDefinition(document);
  if (buildResult.HasErrors()) {
    return MakeBuildFailure(SceneRuntimeOperation::Load,
                            coordinator.GetLifecycle().GetState(),
                            buildResult);
  }
