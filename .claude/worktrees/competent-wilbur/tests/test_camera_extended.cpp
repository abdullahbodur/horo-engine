...
TEST_CASE("Camera: Y scale is independent of aspect ratio", "[camera]") {
    ...
    REQUIRE(cam1.GetProjection()(1, 1) == Approx(cam2.GetProjection()(1, 1)).epsilon(1e-5f));
}
...
TEST_CASE("Camera: different zNear/zFar changes depth projection", "[camera]") {
    Camera cam1;
    cam1.fovY = 60.0f; cam1.aspect = 1.0f;
    cam1.zNear = 0.1f; cam1.zFar = 10.0f;

    Camera cam2;
    cam2.fovY = 60.0f; cam2.aspect = 1.0f;
    cam2.zNear = 0.1f; cam2.zFar = 1000.0f;
...
TEST_CASE("Camera: changing fovY changes projection", "[camera]") {
    Camera cam60;
    cam60.fovY = 60.0f; cam60.aspect = 1.0f; cam60.zNear = 0.1f; cam60.zFar = 100.0f;

    Camera cam90;
    cam90.fovY = 90.0f; cam90.aspect = 1.0f; cam90.zNear = 0.1f; cam90.zFar = 100.0f;

    REQUIRE(cam60.GetProjection()(1, 1) != Approx(cam90.GetProjection()(1, 1)).epsilon(0.01f));
}
