...
//
// Usage (identical to function calls, including inside Horo sub-namespaces
...
struct LogDebug {
    LogDebug(std::format_string<Args...> fmt, const Args &... args,
        ...
        Horo::LogImpl(Horo::LogLevel::Debug, loc,
...
struct LogInfo {
    LogInfo(std::format_string<Args...> fmt, const Args &... args,
            std::source_location loc = std::source_location::current()) {
        Horo::LogImpl(Horo::LogLevel::Info, loc,
                          std::vformat(fmt.get(), std::make_format_args(args...)));
    }
};
...
struct LogWarn {
    LogWarn(std::format_string<Args...> fmt, const Args &... args,
            std::source_location loc = std::source_location::current()) {
        Horo::LogImpl(Horo::LogLevel::Warn, loc,
                          std::vformat(fmt.get(), std::make_format_args(args...)));
    }
};
...
struct LogError {
    LogError(std::format_string<Args...> fmt, const Args &... args,
             std::source_location loc = std::source_location::current()) {
        Horo::LogImpl(Horo::LogLevel::Error, loc,
                          std::vformat(fmt.get(), std::make_format_args(args...)));
    }
};
