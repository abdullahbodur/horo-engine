...
namespace Horo {
    ...
    Mat4 Mat4::Zero() { return Mat4(0.0f); }
    ...
    Vec4 operator*(const Mat4 &mat, const Vec4 &v) {
        return {
            mat.m[0][0] * v.x + mat.m[1][0] * v.y + mat.m[2][0] * v.z + mat.m[3][0] * v.w,
    ...
    float Mat4::Determinant() const {
        ...
        auto minor = [&](int c0, int c1, int c2, int r0, int r1, int r2) {
        ...
        return a * minor(1, 2, 3, 1, 2, 3) - b * minor(0, 2, 3, 1, 2, 3) +
               c * minor(0, 1, 3, 1, 2, 3) - d * minor(0, 1, 2, 1, 2, 3);
    }
    ...
    Mat4 Mat4::Inverse() const {
        ...
        float det =
                src[0] * inv[0] + src[1] * inv[4] + src[2] * inv[8] + src[3] * inv[12];
        if (NearlyZero(det))
            return Identity();
