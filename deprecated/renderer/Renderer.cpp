...
namespace Horo {
    namespace {
        std::unique_ptr<IRenderBackend> CreateDefaultOwnedBackend() {
            ...
            HORO_ASSERT(createResult.backend != nullptr,
                            "Failed to create the default render backend");
    ...
    void Renderer::BeginDebugBlend()        { ActiveBackend()->BeginDebugBlend(); }
    void Renderer::EndDebugBlend()          { ActiveBackend()->EndDebugBlend(); }
