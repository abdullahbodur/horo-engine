...
namespace Horo {
    ...
    // When a channel's time/value arrays are empty that channel is considered
    // absent and the sampler will fall back to a sensible default (zero
    ...
    // calling Sample().
    // NOSONAR: cpp:S5414 name and duration are intentionally public data members
    class AnimationClip {
        // NOSONAR: cpp:S1179 name and duration are intentionally public value members
    public:
        std::string name;
    ...
    private:
        ...
        // Boundary behaviour: clamp to first/last key; single-key tracks return the
        // key.
