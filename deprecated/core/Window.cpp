...
static std::function<void()> g_buildReleaseCallback;
...
extern "C" void InvokeBuildRelease() { if (g_buildReleaseCallback) g_buildReleaseCallback(); }
#endif
// clang-format on
...
void Window::SetMenuCallbacks(const MenuCallbacks& callbacks) const {
  ...
  g_buildReleaseCallback = callbacks.build.buildRelease;
#else
  (void)callbacks;
#endif
}
