...
TEST_CASE(
  ...
  REQUIRE(mainCpp.find("m_camera.target = cam.position + forward;") !=
          std::string::npos);
  REQUIRE(mainCpp.find("m_camera.fovY = cam.fovY;") != std::string::npos);
  REQUIRE(mainCpp.find("m_camera.yaw") == std::string::npos);

  REQUIRE(mainCpp.find("spec.name = \"TemplateGame\";") != std::string::npos);
  REQUIRE(mainCpp.find("frameConfig.debugLabel = \"TemplateGame-frame\";") !=
          std::string::npos);
  REQUIRE(mainCpp.find("\"TemplateGame-scene\"})") != std::string::npos);
  REQUIRE(mainCpp.find("= \"\nTemplateGame") == std::string::npos);
  REQUIRE(mainCpp.find("\"\nTemplateGame-") == std::string::npos);
...
TEST_CASE("Launcher project template prefers canonical SDK shaders",
          "[launcher][template]") {
  const std::filesystem::path projectRoot =
      MakeTempRoot("template_sdk_shaders");
  const std::filesystem::path sdkRoot = MakeTempRoot("template_shader_sdk");
  std::filesystem::remove_all(projectRoot);
  std::filesystem::remove_all(sdkRoot);
  std::filesystem::create_directories(sdkRoot / "renderer" / "shaders");

  const std::string canonicalVertex = "#version 410 core\n// sdk vertex\n";
  const std::string canonicalFragment =
      "#version 410 core\n// sdk cascaded shadow fragment\n";
  {
    std::ofstream out(sdkRoot / "renderer" / "shaders" / "basic.vert");
    REQUIRE(out.is_open());
    out << canonicalVertex;
  }
  {
    std::ofstream out(sdkRoot / "renderer" / "shaders" / "basic.frag");
    REQUIRE(out.is_open());
  ...
  std::string error;
  REQUIRE(CreateLauncherProjectTemplate(
      {projectRoot, "CanonicalShaders", sdkRoot, "opengl"}, nullptr, &error));
  REQUIRE(ReadTextFile(projectRoot / "shaders" / "basic.vert") ==
          canonicalVertex);
  REQUIRE(ReadTextFile(projectRoot / "shaders" / "basic.frag") ==
          canonicalFragment);
}

TEST_CASE("Launcher project template validates required request fields",
          "[launcher][template]") {
  std::string error;
  LauncherProjectDocument doc;

  REQUIRE_FALSE(CreateLauncherProjectTemplate(
      {std::filesystem::path(), "TemplateGame", MakeTempRoot("sdk"), "opengl"},
      &doc, &error));
  REQUIRE(error.find("Project root is empty") != std::string::npos);

  error.clear();
  REQUIRE_FALSE(
      CreateLauncherProjectTemplate({MakeTempRoot("template_missing_name"), "",
                                     MakeTempRoot("sdk"), "opengl"},
                                    &doc, &error));
  REQUIRE(error.find("Project name is empty") != std::string::npos);
}

TEST_CASE("Launcher project template falls back to OpenGL for unknown renderer",
          "[launcher][template]") {
  const std::filesystem::path projectRoot =
      MakeTempRoot("template_unknown_renderer");
  std::filesystem::remove_all(projectRoot);

  LauncherProjectDocument doc;
  std::string error;
  REQUIRE(CreateLauncherProjectTemplate(
      {projectRoot, "RendererFallback", MakeTempRoot("sdk"), "direct3d"}, &doc,
      &error));

  REQUIRE(std::find(doc.manifest.configureCommand.args.begin(),
                    doc.manifest.configureCommand.args.end(),
                    "-DHORO_RENDERER=opengl") !=
          doc.manifest.configureCommand.args.end());
}

TEST_CASE("Launcher project template reports file write failures",
          "[launcher][template]") {
  ...
  std::string error;
  REQUIRE_FALSE(CreateLauncherProjectTemplate(
      {projectRoot, "TemplateGame", MakeTempRoot("sdk"), "opengl"}, nullptr,
      &error));
...
TEST_CASE("External process runner reports non-zero exit from child process",
          "[launcher][process]") {
  ExternalProcessRunner runner;
  ResolvedLauncherCommand command;
  command.workingDirectory = Horo::Tests::SecureTempBase();
...
TEST_CASE(
    "External process runner handles large stdout without deadlock",
    "[launcher][process][large-output]") {
  ExternalProcessRunner runner;
  ResolvedLauncherCommand command;
  command.workingDirectory = Horo::Tests::SecureTempBase();
#ifdef _WIN32
  command.executable = "cmd";
  // Generate ~50 KB of output across many lines to stress pipe buffer.
  command.args = {"/c",
                  "for /l %i in (1,1,1000) do @echo "
                  "abcdefghijklmnopqrstuvwxyzABCDEFGHIJKLMNOPQRSTUVWXYZ012345"};
#else
  command.executable = "sh";
  // Generate ~50 KB of output (1000 lines, ~50 bytes each with newline).
  command.args = {
      "-c",
  ...
  std::string error;
  REQUIRE(runner.Start(command, "large-output-process", &error));

  // Poll until completion; must not deadlock even with large output.
  for (int i = 0; i < 300 && runner.IsActive(); ++i) {
    runner.Poll();
...
TEST_CASE(
    "External process runner stop semantics: graceful then force-kill fallback",
    "[launcher][process][termination]") {
  ExternalProcessRunner runner;
  ResolvedLauncherCommand command;
  command.workingDirectory = Horo::Tests::SecureTempBase();
#ifdef _WIN32
  // Windows: spawn a process that ignores CTRL_BREAK and keeps running.
  // Using a tight loop in cmd ensures force-kill path is exercised.
  command.executable = "cmd";
  command.args = {"/c", "for /l %i in (1,1,100) do @ping -n 2 127.0.0.1 >nul"};
#else
  // POSIX: sleep long enough that SIGTERM path is definitely exercised.
  command.executable = "sh";
  ...
  std::string error;
  REQUIRE(runner.Start(command, "termination-test", &error));
  REQUIRE(runner.IsActive());

  // Let the process start fully before stopping.
  std::this_thread::sleep_for(std::chrono::milliseconds(100));

  runner.Stop();

  // Process must be fully stopped after Stop() returns.
  REQUIRE_FALSE(runner.IsActive());
  REQUIRE(runner.GetStatus().finished);
  REQUIRE(runner.GetStatus().terminatedByUser);
}
...
TEST_CASE(
    "External process runner works with Unicode characters in working directory",
    "[launcher][process][unicode-path]") {
  // Create a temp directory with Unicode characters in the path.
  // On Windows, construct the leaf name from a wide literal to avoid
  // char8_t → wchar_t conversion issues in MSVC's <filesystem>.
  const auto tempBase = Horo::Tests::SecureTempBase();
#ifdef _WIN32
  const auto unicodeDir =
      tempBase / L"horo_unicode_test_\u2605\u30C6\u30B9\u30C8_\u03B1\u03B2\u03B3";
#else
  const auto unicodeDir = tempBase / u8"horo_unicode_test_★テスト_αβγ";
#endif
  std::error_code ec;
  std::filesystem::create_directories(unicodeDir, ec);
  if (ec) {
    // Skip if filesystem doesn't support Unicode in paths.
    SUCCEED("Unicode directory creation not supported on this filesystem; "
            "skipping test.");
    return;
  }
  ...
  ExternalProcessRunner runner;
  ResolvedLauncherCommand command;
  command.workingDirectory = unicodeDir;
#ifdef _WIN32
  command.executable = "cmd";
  command.args = {"/c", "cd"};
#else
  ...
  std::string error;
  const bool started =
      runner.Start(command, "unicode-path-process", &error);
