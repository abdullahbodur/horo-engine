...
TEST_CASE("EditorLayer: primitive objects default to solid static physics", "[editor][layer][physics]") {
  EditorLayer editor;
  editor.LoadDocument(SceneDocument{});

  editor.UiAutomationAddPrimitiveObject("box");

  const auto &objects = editor.GetDocument().objects;
  REQUIRE(objects.size() == 1u);
  const SceneObject &object = objects.back();
  REQUIRE(object.type == SceneObjectType::Prop);
  REQUIRE(object.props.at("mesh") == "box");
  REQUIRE(object.components.size() == 1u);
  const ComponentDesc &rigidbody = object.components.front();
  REQUIRE(rigidbody.type == "rigidbody");
  CHECK(rigidbody.props.at("mass") == "0.0000");
  CHECK(rigidbody.props.at("isKinematic") == "true");
  CHECK(rigidbody.props.at("useGravity") == "false");
}

TEST_CASE("EditorLayer: light menu variants create distinct light types", "[editor][layer][light]") {
  EditorLayer editor;
  editor.LoadDocument(SceneDocument{});

  editor.UiAutomationAddLightObject("spot");

  const auto &objects = editor.GetDocument().objects;
  REQUIRE(objects.size() == 1u);
  const SceneObject &object = objects.back();
  REQUIRE(object.type == SceneObjectType::Light);
  CHECK(object.props.at("lightType") == "spot");
  CHECK(object.props.at("radius") == "12.0000");
  CHECK(object.pitch == Approx(-55.0f));
}
...
TEST_CASE("EditorLayerInternal: placement and path helpers cover deterministic branches", "[editor][internal][coverage]") {
  ...
  CHECK(Horo::FormatFileSize(42) == "42 B");
  CHECK(Horo::FormatFileSize(1536) == "1.5 KB");
  CHECK(Horo::FormatFileSize(5 * 1024 * 1024) == "5.0 MB");
  CHECK(Horo::FormatFileSize(3ull * 1024 * 1024 * 1024) == "3.0 GB");
