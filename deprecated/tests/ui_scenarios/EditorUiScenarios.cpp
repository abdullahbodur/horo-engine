...
namespace Horo {
    namespace {
        ...
        bool EnsureProjectCreatedFromLauncher(ImGuiTestContext *ctx,
            ...
            if (!ctx || !state || !state->shellContext)
                return false;
            if (state->shellContext->HasActiveProject())
                return true;

            SetLauncherHomeRef(ctx);
            const bool launcherReady = WaitForCondition(ctx, 180, [ctx]() {
                return ctx->ItemExists("##open-existing-project-action");
            });
            if (!launcherReady) {
                LogWarn("UI scenario: launcher home was not ready.");
                return false;
            }

            ImGuiWindow *launcherPanel = FindLauncherPanelInputWindow();
            if (!launcherPanel) {
                LogWarn("UI scenario: launcher project input panel was not found.");
                return false;
            }
            ...
            const bool projectCreated = WaitForCondition(ctx, 180, [state]() {
                return state->shellContext && state->shellContext->HasActiveProject();
            });
            if (!projectCreated) {
                LogWarn("UI scenario: launcher did not report active project after creation.");
                return false;
            }
            ctx->Yield(10);
            return true;
        ...
        // ── Build Pipeline Modal — Versioning UX ─────────────────────────

        /** @brief Scenario: opens the Build & Release modal, navigates to the
         *         Configuration tab, types an invalid version tag, verifies the
         *         validation error appears, clears it with a valid version, and
         *         exercises the patch-bump helper. */
        void RunBuildPipelineModalVersioning(ImGuiTestContext *ctx) {
            UiAutomationRunState *state =
                    GetTestState(ctx, "editor_ui/build_pipeline_modal_versioning");
            IM_CHECK(state != nullptr);
            if (!state)
                return;
            ...
            // Open Build menu → click "Build & Release..."
            LogDebug("UI scenario action: click Build menu");
            const ImGuiID buildPopupId =
                    OpenToolbarPopup(ctx, "Build", "##toolbar_build_popup",
                                     "Build & Release...");
            IM_CHECK(buildPopupId != ImGuiID(0));
            if (!buildPopupId)
                return;

            LogDebug("UI scenario action: click Build & Release...");
            ctx->ItemClick("Build & Release...");
            ctx->Yield(2);

            // Wait for the modal to open.
            const bool modalReady = WaitForCondition(ctx, 60, [ctx]() {
                ctx->SetRef("Build & Release");
                return CurrentRefHasItemLabelContaining(
                    ctx, "##build_pipeline_tabs");
            });
            IM_CHECK(modalReady);
            if (!modalReady) {
                LogWarn("UI scenario: Build & Release modal did not open.");
                return;
            }

            // Switch to the Configuration tab (index 1, id "configuration").
            LogDebug("UI scenario action: click Configuration tab");
            ctx->SetRef("Build & Release");
            ctx->ItemClick("##build_pipeline_tabs/configuration/##tab");
            ctx->Yield(2);

            // ── Step 1: Type an invalid version ─────────────────────────
            LogDebug("UI scenario action: type invalid version tag");
            ctx->SetRef("Build & Release");
            ctx->ItemInputValue("##version_tag", "not-a-version");
            ctx->Yield(2);

            // Verify the validation error is visible.
            const bool errorVisible = WaitForCondition(ctx, 10, [ctx]() {
                ctx->SetRef("Build & Release");
                return CurrentRefHasItemLabelContaining(ctx, "Invalid SemVer");
            });
            IM_CHECK(errorVisible);
            LogDebug("UI scenario assertion: invalid version shows error message");

            // ── Step 2: Type a valid version ─────────────────────────────
            LogDebug("UI scenario action: type valid version tag");
            ctx->SetRef("Build & Release");
            ctx->ItemInputValue("##version_tag", "1.2.3");
            ctx->Yield(2);

            // Verify the error cleared (the error label is no longer present).
            ctx->SetRef("Build & Release");
            IM_CHECK(!CurrentRefHasItemLabelContaining(ctx, "Invalid SemVer"));
            LogDebug("UI scenario assertion: valid version clears error");

            // ── Step 3: Exercise patch-bump ──────────────────────────────
            LogDebug("UI scenario action: click Bump Patch");
            ctx->SetRef("Build & Release");
            ctx->ItemClick("Bump Patch##bump_patch");
            ctx->Yield(2);

            // ── Step 4: Close via footer Close button ────────────────────
            LogDebug("UI scenario action: click Close button");
            ctx->SetRef("Build & Release");
            if (ctx->ItemExists("Close"))
                ctx->ItemClick("Close");
            else
                ClickCurrentRefItemLabelContaining(ctx, "Close");
            ctx->Yield(2);

            CaptureIfEnabled(ctx, state,
                             "editor_ui__build_pipeline_modal_versioning.png");
            LogInfo("UI scenario done: editor_ui/build_pipeline_modal_versioning");
        }

        ImGuiTest *RegisterBuildPipelineModalVersioning(
                ImGuiTestEngine *engine, UiAutomationRunState *state) {
            ImGuiTest *test =
                    IM_REGISTER_TEST(engine, "editor_ui",
                                     "build_pipeline_modal_versioning");
            test->UserData = state;
            test->TestFunc = &RunBuildPipelineModalVersioning;
            return test;
        }

        // ── Build Pipeline Modal E2E ────────────────────────────────────

        /** @brief E2E scenario: opens the Build & Release modal, verifies tabs
         *         and footer buttons, then closes via the footer Close button. */
        void RunBuildPipelineModalE2E(ImGuiTestContext *ctx) {
            UiAutomationRunState *state =
                    GetTestState(ctx, "editor_ui/build_pipeline_modal_e2e");
            IM_CHECK(state != nullptr);
            if (!state)
                return;
            ...
            // Wait for the modal to open and render its tab bar
            const bool modalReady = WaitForCondition(ctx, 60, [ctx]() {
                ctx->SetRef("Build & Release");
                return CurrentRefHasItemLabelContaining(
                    ctx, "##build_pipeline_tabs");
            ...
            // Verify footer buttons are present in the idle state
            ctx->SetRef("Build & Release");
            IM_CHECK(ctx->ItemExists("Build All") ||
                     CurrentRefHasItemLabelContaining(ctx, "Build All"));
            IM_CHECK(ctx->ItemExists("Close") ||
                     CurrentRefHasItemLabelContaining(ctx, "Close"));

            // Verify platform selection cards are visible on the default
            // Platforms tab (at least one of Windows, macOS, or Linux card
            // must be present).
            ctx->SetRef("Build & Release");
            IM_CHECK(CurrentRefHasItemLabelContaining(ctx, "##card_windows") ||
                     CurrentRefHasItemLabelContaining(ctx, "##card_macos") ||
                     CurrentRefHasItemLabelContaining(ctx, "##card_linux"));

            // Close the modal via the footer Close button
            LogDebug("UI scenario action: click Close button to dismiss modal");
            ctx->SetRef("Build & Release");
            if (ctx->ItemExists("Close"))
                ctx->ItemClick("Close");
            else
            ...
            CaptureIfEnabled(ctx, state,
                             "editor_ui__build_pipeline_modal_e2e.png");
            LogInfo("UI scenario done: editor_ui/build_pipeline_modal_e2e");
        }

        ImGuiTest *RegisterBuildPipelineModalE2E(ImGuiTestEngine *engine,
                                                  UiAutomationRunState *state) {
            ImGuiTest *test =
                    IM_REGISTER_TEST(engine, "editor_ui",
                                     "build_pipeline_modal_e2e");
            test->UserData = state;
            test->TestFunc = &RunBuildPipelineModalE2E;
            return test;
        }
    ...
    void RegisterEditorUiScenarioSet() {
        ...
        RegisterUiScenario("editor/build_pipeline_modal_e2e",
                           &RegisterBuildPipelineModalE2E);

        RegisterUiScenario("editor/build_pipeline_modal_versioning",
                           &RegisterBuildPipelineModalVersioning);
