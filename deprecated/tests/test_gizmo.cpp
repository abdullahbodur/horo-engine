...
struct TransformGizmoTestAccessor {
  ...
  static bool ApplyActiveDrag(TransformGizmo &g, const Ray &ray,
    ...
    const bool consumed = g.ApplyActiveDrag(
        ray, cam, result, translateSnapStep, rotateSnapRadians, scaleSnapStep);
    outDeltaPos = result.deltaPos;
    outDeltaRot = result.deltaRot;
    outDeltaScale = result.deltaScale;
    return consumed;
  }
...
TEST_CASE("TransformGizmo: PickAxis skips Z axis when its tip is behind the camera", "[gizmo]") {
  ...
  Camera cam = MakeCamera({0.0f, 0.0f, 10.0f}, Vec3::Zero(), 60.0f, 1.0f);
  TransformGizmo g;
  g.Activate(GizmoMode::Translate, {0.0f, 0.0f, 9.9999f},
             Quaternion::Identity(), Vec3::One());
  GizmoAxis picked = g.PickAxis(400.0f, 400.0f, cam, 800, 800);
...
TEST_CASE("ApplyActiveDrag: Translate produces axis-aligned delta", "[gizmo][drag]") {
  ...
  Vec3 deltaScale;
  bool consumed =
      TA::ApplyActiveDrag(g, ray, cam, deltaPos, deltaRot, deltaScale);
  CHECK(!consumed);
...
// ============================================================================
// Tests 33-35 — Update: public frame-input path
// ============================================================================

TEST_CASE("TransformGizmo: Update returns default result when inactive", "[gizmo][update]") {
  TransformGizmo g;
  TransformGizmoUpdateParams params;

  const TransformGizmoResult result = g.Update(params);

  CHECK(!result.consumedMouse);
  CHECK(result.deltaPos.x == Approx(0.0f));
  CHECK(result.deltaPos.y == Approx(0.0f));
  CHECK(result.deltaPos.z == Approx(0.0f));
  CHECK(result.deltaScale.x == Approx(1.0f));
  CHECK(result.deltaScale.y == Approx(1.0f));
  CHECK(result.deltaScale.z == Approx(1.0f));
}

TEST_CASE("TransformGizmo: Update consumes active drag when drag plane is invalid", "[gizmo][update]") {
  TransformGizmo g;
  g.Activate(GizmoMode::Translate, Vec3::Zero(), Quaternion::Identity(),
             Vec3::One());
  TA::SetDragging(g, GizmoAxis::X);
  TA::SetDragPlaneNormal(g, Vec3::Zero());
  TA::SetDragAnchorPos(g, Vec3::Zero());
...
TEST_CASE("TransformGizmo: Update accepts viewport dimensions while dragging", "[gizmo][update]") {
  TransformGizmo g;
  ...
  params.screenW = 800;
  params.screenH = 800;
  params.viewportX = 100.0f;
  params.viewportY = 50.0f;
  params.viewportW = 800.0f;
  params.viewportH = 800.0f;
