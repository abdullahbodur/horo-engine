...
[[maybe_unused]] SceneObject *FindSceneObject(SceneDocument *doc, std::string_view id) {
  if (!doc)
    return nullptr;
...
[[maybe_unused]] const SceneObject *FindSceneObject(const SceneDocument *doc,
                                   std::string_view id) {
  if (!doc)
    return nullptr;
  for (const SceneObject &object : doc->objects) {
...
TEST_CASE("McpProtocol serves initialize, lists, all resources, and all read tools", "[mcp][protocol][coverage]") {
  ...
  REQUIRE(toolList["result"]["tools"].size() == 35);
