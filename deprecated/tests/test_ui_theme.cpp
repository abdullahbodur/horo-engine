...
TEST_CASE("RenderEditorDialogFooter renders progress and actions without crash") {
  ImGuiContext *context = ImGui::CreateContext();
  REQUIRE(context != nullptr);
  ImGuiIO &io = ImGui::GetIO();
  io.DisplaySize = ImVec2(720.0f, 240.0f);
  io.Fonts->Build();
  ImGui::NewFrame();
  ImGui::Begin("dialog-footer-test");

  const auto &theme = Horo::Ui::GetEditorTheme();
  const std::array<Horo::Ui::EditorDialogFooterButton, 3> buttons = {{
      {"rebuild", "Rebuild",
       Horo::Ui::EditorDialogFooterButtonStyle::Secondary,
       Horo::Ui::DrawRefreshIcon, 118.0f, true},
      {"export", "Export Log",
       Horo::Ui::EditorDialogFooterButtonStyle::Secondary,
       Horo::Ui::DrawDocumentIcon, 138.0f, true},
      {"close", "Close",
       Horo::Ui::EditorDialogFooterButtonStyle::Primary,
       nullptr, 104.0f, true},
  }};
  const Horo::Ui::EditorDialogFooterConfig cfg{
      "##test_dialog_footer",
      64.0f,
      {"Build Progress:", "Succeeded", 1.0f, 300.0f},
      buttons,
  };
  REQUIRE_NOTHROW(Horo::Ui::RenderEditorDialogFooter(theme, cfg));
...
TEST_CASE("RenderRecentRunsCard renders structured run rows without crash") {
  ImGuiContext *context = ImGui::CreateContext();
  REQUIRE(context != nullptr);
  ImGuiIO &io = ImGui::GetIO();
  io.DisplaySize = ImVec2(360.0f, 280.0f);
  io.Fonts->Build();
  ImGui::NewFrame();
  ImGui::Begin("recent-runs-test");

  const auto &theme = Horo::Ui::GetEditorTheme();
  const std::vector<Horo::Ui::RecentRunEntry> entries = {
      {true, "Today, 10:22", "Succeeded", "00:01:18", "macOS", nullptr},
      {false, "Yesterday, 18:47", "Failed", "00:00:45", "Windows", nullptr},
  };
  REQUIRE_NOTHROW(Horo::Ui::RenderRecentRunsCard(theme, entries, 5));
