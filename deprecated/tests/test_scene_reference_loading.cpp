...
TEST_CASE("SceneReferenceRuntime builds rigid bodies for props with rigidbody components", "[scene][reference-runtime][physics]") {
  SceneDocument doc =
      MakeReferenceSceneDocument(TmpPath("reference_scene_physics.json"),
                                 "scene_physics", 1, 1, 0, false);
  REQUIRE(doc.objects.size() >= 2);
  doc.objects[1].components.push_back(
      {"rigidbody", {{"mass", "3.0"}, {"isKinematic", "false"}, {"useGravity", "true"}}});

  Scene scene;
  SceneReferenceRuntime runtime(&scene);
  const SceneRuntimeOperationResult load = runtime.LoadDocument(doc);

  REQUIRE(load.ok);
  REQUIRE(scene.GetPhysics().GetBodies().size() == 2);
  const auto bodies = scene.GetRegistry().GetEntities<RigidBodyComponent>();
  REQUIRE(bodies.size() == 1);
  const auto *body = scene.GetRegistry().Get<RigidBodyComponent>(bodies[0]).body;
