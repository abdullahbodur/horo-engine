...
TEST_CASE("CreateLauncherProjectTemplate writes required project files", "[launcher][template]") {
  ...
  if (!ok) {
    ...
    std::ifstream mainCpp(root / "src" / "main.cpp");
    REQUIRE(mainCpp.is_open());
    const std::string mainCppText((std::istreambuf_iterator<char>(mainCpp)),
                                  std::istreambuf_iterator<char>());
    CHECK(mainCppText.find("#include \"ui/editor/SceneSerializer.h\"") !=
          std::string::npos);
    CHECK(mainCppText.find("#include \"editor/SceneSerializer.h\"") ==
          std::string::npos);
    CHECK(mainCppText.find("m_scene.GetPhysics()") != std::string::npos);
    CHECK(mainCppText.find("m_scene.physics") == std::string::npos);
    std::ifstream cmakeLists(root / "CMakeLists.txt");
    REQUIRE(cmakeLists.is_open());
    const std::string cmakeText((std::istreambuf_iterator<char>(cmakeLists)),
                                std::istreambuf_iterator<char>());
    CHECK(cmakeText.find("HORO_RENDERER_TARGET HoroEngine::RendererOpenGL") !=
          std::string::npos);
    CHECK(cmakeText.find("${HORO_ENGINE_TARGET} ${HORO_RENDERER_TARGET}") !=
          std::string::npos);
  }
}
