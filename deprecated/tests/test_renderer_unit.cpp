...
namespace {
void CheckTriangleWindingMatchesVertexNormals(const char *meshName,
                                              const Mesh &mesh) {
  ...
  for (size_t i = 0; i < indices.size(); i += 3) {
    const Vertex &a = vertices.at(indices[i]);
    const Vertex &b = vertices.at(indices[i + 1]);
    const Vertex &c = vertices.at(indices[i + 2]);
    const Vec3 geometricNormal =
        Vec3::Cross(b.position - a.position, c.position - a.position);
    if (geometricNormal.LengthSq() <= 1e-8f)
      continue;

    const Vec3 averageNormal = (a.normal + b.normal + c.normal).Normalized();
    INFO("mesh=" << meshName << " triangle=" << (i / 3));
    CHECK(Vec3::Dot(geometricNormal.Normalized(), averageNormal) > 0.0f);
  }
}
} // namespace
...
TEST_CASE("Mesh: procedural triangle winding matches outward vertex normals",
          "[renderer][mesh][shadow]") {
  CheckTriangleWindingMatchesVertexNormals(
      "sphere", Mesh::CreateSphere(1.0f, 8, 12));
  CheckTriangleWindingMatchesVertexNormals(
      "cylinder", Mesh::CreateCylinder(1.0f, 1.0f, 12));
  CheckTriangleWindingMatchesVertexNormals(
      "pyramid", Mesh::CreatePyramid(1.0f, 1.0f));
}
