...
namespace Horo::Launcher {
    namespace {
        /** @brief Emits and captures one prefixed process output line. */
        void ForwardProcessLine(const std::string &label, const std::string &line,
                                std::mutex &outputMutex, std::string &capturedOutput) {
            if (line.empty())
                return;
            {
                std::lock_guard lock(outputMutex);
                capturedOutput += std::format("[{}] {}\n", label, line);
            }
            LogInfo("[{}] {}", label, line);
        }
            ...
            while (ReadFile(hRead, buffer.data(), static_cast<DWORD>(buffer.size()),
                ...
                while ((newlinePos = pending.find('\n')) != std::string::npos) {
                    ...
                    if (!line.empty() && line.back() == '\r')
                        line.pop_back();
                    ForwardProcessLine(label, line, outputMutex, capturedOutput);
                    pending.erase(0, newlinePos + 1);
                }
            }
            if (!pending.empty())
                ForwardProcessLine(label, pending, outputMutex, capturedOutput);
            readerDone = true;
        }
#else
        ...
        std::wstring Utf8ToWide(const std::string &value) {
            ...
            if (MultiByteToWideChar(CP_UTF8, 0, value.c_str(), -1, out.data(),
                                    length) <= 0)
                return {};
            out.pop_back();
            return out;
        ...
        /** @brief Gracefully terminates a Windows process, falling back to force kill.
         *
         *  Phase 1: Sends CTRL_BREAK_EVENT to the process group (enabled by
         *  CREATE_NEW_PROCESS_GROUP at spawn) and waits up to 5 seconds for
         *  graceful exit. Phase 2: Force-terminates via TerminateProcess.
         *
         *  @param processHandle Handle to the target process.
         *  @param processId     Process ID used to identify the process group.
         *  @return The process exit code after termination. */
        int StopWindowsProcess(HANDLE processHandle, DWORD processId) {
            if (!processHandle)
                return 1;

            // Phase 1: Graceful — send CTRL_BREAK to the process group
            if (processId != 0) {
            ...
            // Phase 2: Force kill — terminate the process tree then the root
    ...
    struct ExternalProcessRunner::ProcessHandle {
        ...
        std::string capturedOutput; /**< Full stdout/stderr text captured for UI consumers. */
    ...
    bool ExternalProcessRunner::Start(const ResolvedLauncherCommand &command,
        ...
        m_status.commandLine = RedactCommandLine(command.debugString);
        ...
        // 64 KB pipe buffer to reduce backpressure on large build output.
        static constexpr DWORD kPipeBufferSize = 64 * 1024;
        if (!CreatePipe(&process->stdoutRead, &process->stdoutWrite, &sa,
                        kPipeBufferSize)) {
            if (outError)
        ...
        const std::wstring workdir = command.workingDirectory.native();
        if (const BOOL created = CreateProcessW(
                ...
                workdir.empty() ? nullptr : workdir.c_str(), &startup, &info);
            !created) {
            ...
            if (outError)
                *outError =
                    std::format("CreateProcessW failed with error {}.",
                                createError);
            return false;
        }
        ...
        process->readerThread = ReaderThread([handle = process.get(), label]() {
            ReadWindowsPipe(handle->stdoutRead, label, handle->readerMutex,
                            handle->capturedOutput, handle->readerDone);
        });
#else
        ...
        process->readerThread = ReaderThread([handle = process.get(), label]() {
            ReadPosixPipe(handle->stdoutRead, label, handle->readerMutex,
                          handle->capturedOutput, handle->readerDone);
        });
#endif
        ...
        LogInfo("[{}] started: {}", label, RedactCommandLine(command.debugString));
        return true;
    }
    ...
    void ExternalProcessRunner::Poll() {
        ...
        {
            std::lock_guard lock(m_process->readerMutex);
            m_status.output = m_process->capturedOutput;
        }
    ...
    void ExternalProcessRunner::Stop() {
        ...
        if (m_status.active) {
#ifdef _WIN32
            const int exitCode = StopWindowsProcess(m_process->process, m_process->processId);
#else
    ...
    void ExternalProcessRunner::Finish(int exitCode, bool terminatedByUser,
                                       std::string error) {
        if (m_process && m_process->readerThread.joinable())
            m_process->readerThread.join();
        if (m_process) {
            std::lock_guard lock(m_process->readerMutex);
            m_status.output = m_process->capturedOutput;
