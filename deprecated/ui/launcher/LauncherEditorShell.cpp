...
void LauncherEditorShell::OnPathsDropped(int pathCount, const char **utf8Paths,
  ...
  for (int i = 0; i < pathCount; ++i) {
    ...
    std::string_view sv(utf8Paths[i]);
    std::u8string u8Path(sv.begin(), sv.end());
    if (OpenProject(fs::path(u8Path),
                    &openError)) {
      m_launcherError.clear();
      return;
    }
...
void LauncherEditorShell::RenderLauncherSidebar(float sidebarWidth,
  ...
  ImGui::TextDisabled("Horo Engine v%s", Horo::Build::EngineVersion());
  ImGui::EndChild();
  ImGui::PopStyleColor();
  ImGui::PopStyleVar(3);
}
...
void LauncherEditorShell::RenderNewProjectActions(float innerWidth,
  ...
  if (m_newProjectAdvancedSettingsOpen) {
    ...
    Ui::Combo(Ui::GetLauncherTheme(), "##new-project-renderer",
              &m_newProjectRendererBackendIndex,
...
LauncherEditorShell::LoadMeshForTag(const std::string &meshTag) {
  ...
  if (meshTag == "box" || meshTag == "cube") {
    ...
    *mesh = Mesh::CreatePlane();
    ...
    LogInfo("[Launcher] LoadMeshForTag: creating primitive 'quad'");
