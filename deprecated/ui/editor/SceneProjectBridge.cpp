...
namespace Horo::Editor {
    namespace {
        /** @brief Returns the serialized light kind string used by editor props. */
        const char *LightKindToPropValue(SceneLightKind kind) {
            switch (kind) {
        ...
        SceneLightProperties BuildLightFromNodeProps(SceneNodeDefinition &node) {
            ...
            if (const std::string *lightType = FindProp(node.extraProps, "lightType")) {
                if (*lightType == "directional")
                    ...
                    light.kind = SceneLightKind::Rect;
                else if (*lightType == "sky")
                    light.kind = SceneLightKind::Sky;
                else
        ...
        void ApplyLightObjectToObjectProps(SceneObject &object,
            ...
            object.props["lightType"] = LightKindToPropValue(light.kind);
        ...
        ComponentDesc BuildLightComponent(const SceneLightProperties &light) {
            ComponentDesc desc;
            desc.type = "light";
            desc.props["lightType"] = LightKindToPropValue(light.kind);
