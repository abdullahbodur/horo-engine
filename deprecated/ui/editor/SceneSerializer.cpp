...
namespace Horo::Editor {
    ...
    /** @brief Converts parsed scene JSON into a SceneDocument. */
    static SceneDocument BuildSceneDocumentFromJson(const json &j,
                                                    const std::string &sourceName) {
        SceneDocument doc;
        doc.filePath = sourceName;
        ...
        if (!j.contains("objects") || !j["objects"].is_array())
            throw SceneSerializerException(
                "SceneSerializer: missing or invalid 'objects' array in '" +
                sourceName + "'");
    ...
    SceneDocument SceneSerializer::LoadFromFile(const std::string &path) {
        ...
        return BuildSceneDocumentFromJson(j, path);
    }

    /** @copydoc SceneSerializer::LoadFromString */
    SceneDocument SceneSerializer::LoadFromString(const std::string &jsonText,
                                                  const std::string &sourceName) {
        json j;
        try {
        ...
        return BuildSceneDocumentFromJson(j, sourceName);
    }
