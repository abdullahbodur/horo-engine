...
namespace Horo::Editor {
    ...
    public:
        ...
        /** @brief Asynchronously performs the import, returning a future.
         *  Default wraps Import() — heavy importers should override with a worker thread.
         */
        virtual std::future<AssetImportResult>
        ImportAsync(const AssetImportRequest &request) const {
            return std::async(std::launch::async, [this, request]() { return Import(request); });
        }
    };
