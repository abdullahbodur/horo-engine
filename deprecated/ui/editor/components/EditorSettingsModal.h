...
public:
    ...
    enum class Tab {
        ...
        Build,      /**< Per-platform build enablement and toolchain selection. */
    };
     ...
     * @param toolchains Current toolchain settings store snapshot.
     */
    void Open(const Mcp::McpSettings &mcp, const EditorUserSettings &user, const Horo::Build::ToolchainSettingsStore &toolchains);
    ...
    /**
     * @brief Injects the toolchain settings store.
     * @param store Non-owning pointer to the toolchain settings store.
     */
    void SetToolchainSettingsStore(Horo::Build::ToolchainSettingsStore *store) {
...
private:
    ...
    Horo::Build::ToolchainSettingsStore m_toolchainDraft;      /**< In-progress toolchain edits. */
    Horo::Build::ToolchainSettingsStore m_toolchainOriginal;   /**< Toolchain state at modal open. */
    bool m_toolchainsDirty = false;                            /**< Flag set when toolchain draft is modified. */
    bool m_toolchainAutoDetectAttempted = false;                /**< True after the Build tab seeds auto-detected toolchains. */
    ...
    Horo::Build::ToolchainSettingsStore *m_toolchainStore = nullptr; /**< Non-owning. */
    ...
    /** @brief Renders the Build Settings / Toolchains tab content. */
    void DrawBuildTab(const Horo::Ui::EditorTheme& theme);
    /**
