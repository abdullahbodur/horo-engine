...
void EditorLayer::Init(GLFWwindow *window) {
  ...
  m_toolchainStore.LoadFromFile(Horo::Build::ToolchainSettingsStore::GetDefaultSettingsPath());
  m_settingsModal.SetToolchainSettingsStore(&m_toolchainStore);
...
void EditorLayer::InitUiWidgetCallbacks() {
  ...
  callbacks.getStatusBarText = [this]() {
    ...
    std::string text = std::format("Sel: {} | Dirty: {} | Nav: {} | Reload: {}",
                       status.selectionCount, status.dirtyText, status.navText,
                       status.reloadText);
    if (m_editorOperationActive)
        text = std::format("{}... | {}", m_editorOperation.label, text);
    return text;
  };
  ...
  callbacks.getStatusBarItems = [this]() {
    const EditorStatusText status = BuildEditorStatusText(
        EditorStatusSnapshot{static_cast<int>(m_selectedIndices.size()),
                             m_document.dirty, m_viewportNavActive, m_wantsReload});
    auto items = BuildStatusBarItemsHelper(status, m_document.dirty, m_viewportNavActive, m_wantsReload, RendererBackendStatusLabel());
    if (m_editorOperationActive) {
        std::string operationValue = m_editorOperation.label;
        if (!m_editorOperation.detail.empty())
            operationValue += " - " + m_editorOperation.detail;
        items.push_back(Ui::EditorStatusBarItem{.id = m_editorOperation.id,
                                                .icon = ICON_FA_SPINNER,
                                                .label = "Process",
                                                .value = std::move(operationValue),
                                                .level = m_editorOperation.level,
                                                .showProgress = true,
                                                .progress = m_editorOperation.progress});
    }
    return items;
  };
