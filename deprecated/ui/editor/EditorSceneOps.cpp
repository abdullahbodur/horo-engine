...
/** @brief Returns true for built-in 3D primitives that should be solid by default. */
bool IsSolidPrimitiveMeshTag(std::string_view meshTag) {
  return meshTag == "box" || meshTag == "cube" || meshTag == "sphere" ||
         meshTag == "capsule" || meshTag == "cylinder" || meshTag == "plane" ||
         meshTag == "quad";
}

/** @brief Applies static solid physics defaults to a newly created primitive. */
void ApplySolidPrimitivePhysicsDefaults(ComponentDesc &rigidbody) {
  rigidbody.props["mass"] = "0.0000";
  rigidbody.props["isKinematic"] = "true";
  rigidbody.props["useGravity"] = "false";
}
} // namespace
...
/** @copydoc EditorLayer::AddLightObject */
void EditorLayer::AddLightObject(std::string_view lightType,
  ...
  obj.type = Light;
  ApplySchemaDefaults(obj);
  obj.props["lightType"] = std::string(lightType);
  if (lightType == "directional") {
    obj.pitch = -35.0f;
    obj.yaw = -35.0f;
    obj.props["intensity"] = "2.0000";
    obj.props["radius"] = "25.0000";
  } else if (lightType == "spot") {
    obj.pitch = -55.0f;
    obj.props["intensity"] = "3.0000";
    obj.props["radius"] = "12.0000";
  } else if (lightType == "rect") {
    obj.pitch = -45.0f;
    obj.props["intensity"] = "2.5000";
    obj.props["radius"] = "8.0000";
  } else if (lightType == "sky") {
    obj.pitch = -90.0f;
    obj.props["intensity"] = "0.8";
    obj.props["radius"] = "50.0000";
  }
  if (!parentId.empty())
    obj.props["parentId"] = parentId;
...
/** @copydoc EditorLayer::AddPrimitiveObject */
void EditorLayer::AddPrimitiveObject(std::string_view meshTag,
                                     std::string_view parentId) {
  using enum SceneObjectType;
  const EditorHistorySnapshot before = CaptureHistorySnapshot();
  SceneObject obj;
  ...
  obj.type = Prop;
  ApplySchemaDefaults(obj);
  obj.props["mesh"] = std::string(meshTag);
  if (!parentId.empty())
    obj.props["parentId"] = parentId;
  if (IsSolidPrimitiveMeshTag(meshTag)) {
    ComponentDesc rigidbody;
    rigidbody.type = "rigidbody";
    ApplyComponentSchemaDefaults(rigidbody);
    ApplySolidPrimitivePhysicsDefaults(rigidbody);
    obj.components.push_back(std::move(rigidbody));
  }
