...
void EditorLayer::DrawObjectList() {
  ...
  // Top bar — categorised "+" dropdown
  {
    // Primitives sub-menu items (no icons — uniform style)
    const std::array<Ui::EditorPanelDropdownItem, 6> kPrimitiveItems = {{
        {nullptr, "Cube",     [this] { AddPrimitiveObject("box");      }, nullptr, 0},
        {nullptr, "Sphere",   [this] { AddPrimitiveObject("sphere");   }, nullptr, 0},
        {nullptr, "Capsule",  [this] { AddPrimitiveObject("capsule");  }, nullptr, 0},
        {nullptr, "Cylinder", [this] { AddPrimitiveObject("cylinder"); }, nullptr, 0},
        {nullptr, "Plane",    [this] { AddPrimitiveObject("plane");    }, nullptr, 0},
        {nullptr, "Quad",     [this] { AddPrimitiveObject("quad");     }, nullptr, 0},
    }};
    // Environments & Landscapes sub-menu items
    const std::array<Ui::EditorPanelDropdownItem, 3> kEnvironmentItems = {{
        {nullptr, "Terrain",   [this] { AddPrimitiveObject("terrain");   }, nullptr, 0},
        {nullptr, "Tree",      [this] { AddPrimitiveObject("tree");      }, nullptr, 0},
        {nullptr, "Wind Zone", [this] { AddPrimitiveObject("wind_zone"); }, nullptr, 0},
    }};
    // Text & Physics Utilities sub-menu items
    const std::array<Ui::EditorPanelDropdownItem, 2> kUtilityItems = {{
        {nullptr, "3D Text (TextMeshPro)", [this] { AddPrimitiveObject("text_3d"); }, nullptr, 0},
        {nullptr, "Ragdoll",               [this] { AddPrimitiveObject("ragdoll"); }, nullptr, 0},
    }};
    // Lights sub-menu items
    const std::array<Ui::EditorPanelDropdownItem, 5> kLightItems = {{
        {nullptr, "Directional Light", [this] { AddLightObject("directional"); }, nullptr, 0},
        {nullptr, "Point Light",       [this] { AddLightObject("point");       }, nullptr, 0},
        {nullptr, "Spot Light",        [this] { AddLightObject("spot");        }, nullptr, 0},
        {nullptr, "Rect Light",        [this] { AddLightObject("rect");        }, nullptr, 0},
        {nullptr, "Sky Light",         [this] { AddLightObject("sky");         }, nullptr, 0},
    }};
    // Top-level "+" dropdown (no icons on any item)
    const std::array<Ui::EditorPanelDropdownItem, 6> kAddItems = {{
        Ui::EditorPanelDropdownItem{nullptr, "3D Object", {}, kPrimitiveItems.data(),
                                    kPrimitiveItems.size()},
        Ui::EditorPanelDropdownItem{nullptr, "Environments & Landscapes", {},
                                    kEnvironmentItems.data(), kEnvironmentItems.size()},
        Ui::EditorPanelDropdownItem{nullptr, "Text & Physics Utilities", {},
                                    kUtilityItems.data(), kUtilityItems.size()},
        Ui::EditorPanelDropdownItem{nullptr, "Lights", {}, kLightItems.data(),
                                    kLightItems.size()},
        {nullptr, "Panel",  [this] { AddObject(Panel);  }, nullptr, 0},
        {nullptr, "Camera", [this] { AddObject(Camera); }, nullptr, 0},
    }};
    ...
    const std::array actions = {
        Ui::EditorPanelActionItem{ICON_FA_PLUS, nullptr, kAddItems},
...
/** @copydoc EditorLayer::DrawHierarchyPrimitiveMenu */
void EditorLayer::DrawHierarchyPrimitiveMenu(std::string_view parentId) {
  if (ImGui::MenuItem("Cube"))
    AddPrimitiveObject("box", parentId);
  if (ImGui::MenuItem("Sphere"))
    AddPrimitiveObject("sphere", parentId);
  if (ImGui::MenuItem("Capsule"))
    AddPrimitiveObject("capsule", parentId);
  if (ImGui::MenuItem("Cylinder"))
    AddPrimitiveObject("cylinder", parentId);
  if (ImGui::MenuItem("Plane"))
    AddPrimitiveObject("plane", parentId);
  if (ImGui::MenuItem("Quad"))
    AddPrimitiveObject("quad", parentId);
}

/** @copydoc EditorLayer::DrawHierarchyEnvironmentMenu */
void EditorLayer::DrawHierarchyEnvironmentMenu(std::string_view parentId) {
  if (ImGui::MenuItem("Terrain"))
    AddPrimitiveObject("terrain", parentId);
  if (ImGui::MenuItem("Tree"))
    AddPrimitiveObject("tree", parentId);
  if (ImGui::MenuItem("Wind Zone"))
    AddPrimitiveObject("wind_zone", parentId);
}

/** @copydoc EditorLayer::DrawHierarchyUtilityMenu */
void EditorLayer::DrawHierarchyUtilityMenu(std::string_view parentId) {
  if (ImGui::MenuItem("3D Text (TextMeshPro)"))
    AddPrimitiveObject("text_3d", parentId);
  if (ImGui::MenuItem("Ragdoll"))
    AddPrimitiveObject("ragdoll", parentId);
}

/** @copydoc EditorLayer::DrawHierarchyLightMenu */
void EditorLayer::DrawHierarchyLightMenu(std::string_view parentId) {
  if (ImGui::MenuItem("Directional Light"))
    AddLightObject("directional", parentId);
  if (ImGui::MenuItem("Point Light"))
    AddLightObject("point", parentId);
  if (ImGui::MenuItem("Spot Light"))
    AddLightObject("spot", parentId);
  if (ImGui::MenuItem("Rect Light"))
    AddLightObject("rect", parentId);
  if (ImGui::MenuItem("Sky Light"))
    AddLightObject("sky", parentId);
}

/** @copydoc EditorLayer::DrawHierarchyAddMenu */
void EditorLayer::DrawHierarchyAddMenu(std::string_view parentId) {
  using enum SceneObjectType;
  if (ImGui::BeginMenu("3D Object")) {
    DrawHierarchyPrimitiveMenu(parentId);
    ImGui::EndMenu();
  }
  if (ImGui::BeginMenu("Environments & Landscapes")) {
    DrawHierarchyEnvironmentMenu(parentId);
    ImGui::EndMenu();
  }
  if (ImGui::BeginMenu("Text & Physics Utilities")) {
    DrawHierarchyUtilityMenu(parentId);
    ImGui::EndMenu();
  }
  if (ImGui::BeginMenu("Lights")) {
    DrawHierarchyLightMenu(parentId);
    ImGui::EndMenu();
  }
  ImGui::Separator();
  if (ImGui::MenuItem("Panel"))
    AddObject(Panel, parentId);
  if (ImGui::MenuItem("Camera"))
    AddObject(Camera, parentId);
}
