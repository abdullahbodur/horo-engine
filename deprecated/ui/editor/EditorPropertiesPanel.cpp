...
std::string ComponentHeaderLabel(const Horo::Editor::ComponentDesc &comp) {
  if (comp.type == "light")
    return "Light";
...
void EditorLayer::DrawScriptComponentField(ComponentDesc &comp) {
  ...
  if (options.empty()) {
    // If there are no predefined behaviors to pick from, provide a text input
    // so the user can manually type their C++ Behavior class name.
    std::array<char, 256> buf{};
    std::copy_n(current.data(), std::min(current.size(), buf.size() - 1), buf.data());
    
    if (ImGui::InputText("Behavior##Text", buf.data(), buf.size())) {
      comp.props["behaviorTag"] = buf.data();
      m_document.dirty = true;
    }
    return;
  }
...
void EditorLayer::DrawPropertiesComponentsList(SceneObject &obj) {
  ...
  for (int ci = 0; ci < static_cast<int>(obj.components.size()); ++ci) {
    ...
    const std::string headerLabel = ComponentHeaderLabel(comp);
    ImGui::PushID(ci);
    ...
    if (open)
      DrawPropertiesComponentBody(obj, comp);
    ImGui::PopID();
  }
...
/** @copydoc EditorLayer::DrawPropertiesComponentBody */
void EditorLayer::DrawPropertiesComponentBody(SceneObject &obj,
                                              ComponentDesc &comp) {
  if (comp.type == "script") {
  ...
  const ComponentSchema *schema = m_schema.GetComponentSchema(comp.type);
  if (!schema)
    return;

  for (const auto &fd : schema->fields) {
    std::string &val = comp.props[fd.key];
    if (val.empty() && fd.hasDefault)
      val = fd.defaultValue;
    DrawSchemaFieldWidget(obj, fd, val);
  }
}
