...
namespace Horo::Editor {
    namespace {
        ...
        Mcp::McpBuildSnapshot BuildMcpBuildSnapshot(const SceneDocument &document) {
            ...
            for (const SceneProjectValidationIssue &issue: validation.issues) {
                ...
                snapshot.issues.emplace_back("validation", severity, issue.path,
                                             issue.message);
            }
            ...
            for (const RuntimeSceneBuildIssue &issue: runtimeBuild.issues) {
                ...
                snapshot.issues.emplace_back("runtime", severity, issue.path,
                                             issue.message);
            }
        ...
        Horo::Build::BuildTargetOS HostMcpBuildTargetOS() {
#if defined(_WIN32)
        ...
        Horo::Build::BuildArch HostMcpBuildArch() {
        ...
        Horo::Build::BuildConfig ParseMcpBuildConfig(const json &arguments) {
            const std::string config =
                arguments.value("config", std::string("Release"));
            if (config == "Debug" || config == "debug")
                return Horo::Build::BuildConfig::Debug;
            if (config == "MinSizeRel" || config == "minsizerel")
                return Horo::Build::BuildConfig::MinSizeRel;
            return Horo::Build::BuildConfig::Release;
        }

        std::string NormalizeMcpVersionTag(std::string version) {
            if (version.empty())
                return Horo::Build::DefaultVersionTag();
            if (version.front() != 'v')
                version.insert(version.begin(), 'v');
            return version;
        }
    ...
    void EditorLayer::PublishMcpSnapshot() {
        ...
        for (int idx: m_selectedIndices) {
            if (idx >= 0 && idx < static_cast<int>(m_document.objects.size()))
                snapshot.selectedObjectIds.push_back(ObjectAt(m_document, idx).id);
        }
    ...
    EditorLayer::ExecuteMcpCommand(std::string_view toolName,
        ...
        if (toolName == "project.release" || toolName == "editor.release_project")
            return McpHandleProjectRelease(arguments);
    ...
    EditorLayer::McpHandleSelect(const nlohmann::json &arguments) {
        ...
        return Mcp::McpCommandResult{
            true, json{{"selectedObjectIds", GetSelectedObjectIds()}}, std::string()
        };
    }
    ...
    EditorLayer::McpHandleCreateObject(const nlohmann::json &arguments) {
        ...
        if (const std::string parentId = arguments.value("parentId", "");
            !parentId.empty()) {
    ...
    EditorLayer::McpHandleCreateObjectFromAsset(const nlohmann::json &arguments) {
        ...
        if (std::string createError;
            !CreateObjectFromAsset(assetId, parentId, positionPtr,
    ...
    EditorLayer::McpHandleDuplicate(const nlohmann::json &arguments) {
        ...
        const int count = (sourceIndices.size() == 1)
        ...
        for (int idx: sourceIndices) {
            for (int i = 0; i < count; ++i) {
                SceneObject clone =
                        DuplicateObject(m_document, ObjectAt(m_document, idx));
    ...
    EditorLayer::McpHandleUpdateAsset(const nlohmann::json &arguments) {
        ...
        if (arguments.contains("renderScale"))
            it->second.renderScale = arguments["renderScale"].is_null()
                                         ? std::string()
    ...
    EditorLayer::McpHandleReloadScene(const nlohmann::json &) {
        ...
        if (std::string reloadError; !ReloadDocumentFromDisk(
            &reloadError, &previousSelectionIds, &previousAssetId))
    ...
    /** @copydoc EditorLayer::McpHandleProjectRelease */
    Mcp::McpCommandResult
    EditorLayer::McpHandleProjectRelease(const nlohmann::json &arguments) {
        std::filesystem::path projectRoot;
        if (arguments.contains("projectRoot") && arguments["projectRoot"].is_string()) {
            projectRoot = arguments["projectRoot"].get<std::string>();
        } else if (m_projectBrowserRootValid) {
            projectRoot = m_projectBrowserRoot;
        } else {
            projectRoot = std::filesystem::current_path();
        }

        std::error_code ec;
        projectRoot = std::filesystem::absolute(projectRoot, ec);
        if (ec)
            return Mcp::McpCommandResult{
                false, json::object(),
                "Could not resolve project root: " + ec.message()
            };
        if (!std::filesystem::is_directory(projectRoot, ec) || ec)
            return Mcp::McpCommandResult{
                false, json::object(),
                "Project root is not a readable directory: " + projectRoot.string()
            };

        Horo::Build::BuildPipelineDraft draft;
        draft.versionTag = NormalizeMcpVersionTag(
            arguments.value("version", std::string(Horo::Build::EngineVersion())));
        if (arguments.contains("outputRoot") && arguments["outputRoot"].is_string())
            draft.outputRoot = arguments["outputRoot"].get<std::string>();
        if (arguments.contains("archivePassword") &&
            arguments["archivePassword"].is_string())
            draft.archivePassword = Horo::Core::SecureString(arguments["archivePassword"].get<std::string>());

        draft.jobs.push_back(Horo::Build::MakePendingJob(
            HostMcpBuildTargetOS(), HostMcpBuildArch(), ParseMcpBuildConfig(arguments)));

        const std::string plannedOutput =
            Horo::Build::ResolveJobOutputPath(draft, draft.jobs.front(), projectRoot);
        const std::string command =
            Horo::Build::BuildCommandForJob(draft, draft.jobs.front(), projectRoot);

        std::string startError;
        if (!m_buildPipelineModal.StartRelease(
                std::move(draft), projectRoot.generic_string(), &startError)) {
            return Mcp::McpCommandResult{
                false, json::object(),
                startError.empty() ? "Failed to start release build." : startError
            };
        }
        ...
        return Mcp::McpCommandResult{
            ...
            json{
                ...
                {"version", NormalizeMcpVersionTag(
                    arguments.value("version", std::string(Horo::Build::EngineVersion())))},
                {"outputPath", plannedOutput},
                {"command", command},
                {"dirtyScene", m_document.dirty}
            },
            std::string()
        };
    }
