...
/** @copydoc EditorLayer::OnMenuBuildPipeline */
void EditorLayer::OnMenuBuildPipeline() {
  m_buildPipelineModal.SetToolchainStore(&m_toolchainStore);
  m_buildPipelineModal.SetProjectRoot(
      (m_projectBrowserRootValid ? m_projectBrowserRoot : std::filesystem::current_path()).generic_string());
  m_buildPipelineModal.Open();
}
