...
namespace Horo::Editor {
    namespace {
        ...
        /** @brief Returns true when @p indices are the direct corner stream 0,1,2,... */
        bool IsSequentialIndexBuffer(const std::vector<uint32_t> &indices) {
            for (size_t i = 0; i < indices.size(); ++i) {
                if (indices[i] != static_cast<uint32_t>(i))
                    return false;
            }
            return true;
        }

        /** @brief Reads a boolean importer setting, accepting common true/false spellings. */
        ...
        public:
            ...
            AssetImportResult Import(const AssetImportRequest &request) const override {
                ...
                const double fbxpMs = std::chrono::duration<double, std::milli>(t1 - t0).count();
                LogInfo("[FBX Import] LoadStaticMesh '{}' — {} verts, {:.1f} ms",
                        sourcePath.string(), loaded.vertices.size(), fbxpMs);
                if (!loaded.ok) {
                    AssetImportResult result;
            ...
            /** @brief Async import — runs LoadStaticMesh on worker thread. */
            std::future<AssetImportResult>
            ImportAsync(const AssetImportRequest &request) const override {
                return std::async(std::launch::async, [this, request]() {
                    return FbxAssetImporter::Import(request);
                });
            }
        ...
        private:
            ...
            AssetImportResult ImportStatic(const AssetImportRequest &request,
                ...
                const std::vector<uint32_t> *indicesToWrite = &loaded.indices;
                std::vector<uint32_t> optimizedIndices;
                if (IsSequentialIndexBuffer(loaded.indices)) {
                    LogInfo("[FBX Import] Skipping vertex-cache optimization for '{}' "
                            "because the loader emitted unshared corner vertices ({} indices)",
                            sourcePath.string(), loaded.indices.size());
                } else {
                    optimizedIndices = loaded.indices;
                    const auto optimizeStart = std::chrono::high_resolution_clock::now();
                    MeshOptimizer::OptimizeVertexCache(
                        optimizedIndices,
                        static_cast<uint32_t>(loaded.vertices.size()));
                    const auto optimizeEnd = std::chrono::high_resolution_clock::now();
                    const double optimizeMs =
                        std::chrono::duration<double, std::milli>(
                            optimizeEnd - optimizeStart).count();
                    LogInfo("[FBX Import] OptimizeVertexCache '{}' — {} indices, {:.1f} ms",
                            sourcePath.string(), optimizedIndices.size(), optimizeMs);
                    indicesToWrite = &optimizedIndices;
                }
                ...
                LogInfo("[FBX Import] Writing MeshBin '{}' — {} verts, {} indices",
                        destMeshBin.string(), loaded.vertices.size(), indicesToWrite->size());
                if (auto writeResult =
                ...
                LogInfo("[FBX Import] Wrote MeshBin '{}' — {:.1f} ms",
                        destMeshBin.string(), writeMs);
            ...
            AssetImportResult ImportSkinned(const AssetImportRequest &request,
                ...
                namespace fs = std::filesystem;
                AssetImportResult result;
                LogInfo("[FBX Import] LoadSkeletalMesh '{}' starting", sourcePath.string());
                const auto skeletalStart = std::chrono::high_resolution_clock::now();
                FbxLoader::FbxSkeletalLoadResult skeletal =
                    FbxLoader::LoadSkeletalMesh(sourcePath.string());
                const auto skeletalEnd = std::chrono::high_resolution_clock::now();
                const double skeletalMs =
                    std::chrono::duration<double, std::milli>(
                        skeletalEnd - skeletalStart).count();
                LogInfo("[FBX Import] LoadSkeletalMesh '{}' — {} verts, {} bones, {:.1f} ms",
                        sourcePath.string(), skeletal.vertices.size(),
                        skeletal.bones.size(), skeletalMs);
                if (!skeletal.ok) {
                ...
                if (ReadBoolSetting(request.settings, "import.animations", false)) {
                    LogInfo("[FBX Import] LoadAnimations '{}' starting", sourcePath.string());
                    ...
                    const double animMs =
                        std::chrono::duration<double, std::milli>(
                            animEnd - animStart).count();
                    LogInfo("[FBX Import] LoadAnimations '{}' finished in {:.1f} ms",
                            sourcePath.string(), animMs);
                } else {
                    LogInfo("[FBX Import] Skipping animations for '{}' "
