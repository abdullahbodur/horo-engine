...
/** @copydoc EditorLayer::HasBlockingEditorDialogOpen */
bool EditorLayer::HasBlockingEditorDialogOpen() const {
  return m_settingsModal.IsOpen() || m_importAssetModal.IsOpen() ||
         m_buildPipelineModal.IsOpen() || m_quickOpenOpen ||
         m_commandPaletteOpen || m_projectPanelCreateModalRequested ||
         m_projectPanelCreateModalOpen || m_assetSearchOpen ||
         m_uiWidgets.IsConfirmDeleteObjectsOpen() ||
         m_uiWidgets.IsConfirmDeleteAssetOpen() ||
         m_uiWidgets.IsConfirmExitOpen() || m_uiWidgets.IsRenameObjectOpen();
}

/** @copydoc EditorLayer::SyncInputStateWhileDialogOpen */
void EditorLayer::SyncInputStateWhileDialogOpen() {
  if (!m_window)
    return;
  ...
  m_prevViewportNavRmbDown =
      glfwGetMouseButton(m_window, GLFW_MOUSE_BUTTON_RIGHT) == GLFW_PRESS;
  m_prevDel = glfwGetKey(m_window, GLFW_KEY_DELETE) == GLFW_PRESS;
  m_prevCopyRef =
      accelHeld && shiftHeld && glfwGetKey(m_window, GLFW_KEY_C) == GLFW_PRESS;
  m_prevEsc = glfwGetKey(m_window, GLFW_KEY_ESCAPE) == GLFW_PRESS;
  m_prevGizmoW = glfwGetKey(m_window, GLFW_KEY_W) == GLFW_PRESS;
  m_prevGizmoE = glfwGetKey(m_window, GLFW_KEY_E) == GLFW_PRESS;
  m_prevGizmoR = glfwGetKey(m_window, GLFW_KEY_R) == GLFW_PRESS;
  m_prevHelpToggle = f1Held || (slashHeld && shiftHeld);
  m_prevQuickOpenToggle =
...
void EditorLayer::HandleEditorKeyboardShortcuts( // NOSONAR
  ...
  const bool hasBlockingPopup = m_helpPopup.IsOpen() || HasBlockingEditorDialogOpen();
  const bool currUndo =
      accelHeld && !shiftHeld && glfwGetKey(m_window, GLFW_KEY_Z) == GLFW_PRESS;
  const bool currRedo =
      accelHeld &&
...
bool EditorLayer::OnUpdate(float dt, Camera &cam, int screenW, int screenH) {
  ...
  if (HasBlockingEditorDialogOpen()) {
    SyncInputStateWhileDialogOpen();
    PublishMcpSnapshot();
    return true;
  }
