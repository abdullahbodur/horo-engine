...
namespace Horo {
    ...
    namespace Editor {
         ...
         */
        class EditorLayer { // NOSONAR
        public:
            /** @brief Visual severity for editor snackbar notifications. */
            enum class SnackbarLevel {
                Info,    /**< Neutral progress or informational message. */
                Success, /**< Completed operation or positive outcome. */
                Warning, /**< Recoverable issue that needs attention. */
                Error    /**< Failed operation or destructive outcome. */
            };

            /** @brief Optional snackbar action rendered as a compact text button. */
            struct SnackbarAction {
                std::string label;             /**< Visible action label. */
                std::function<void()> handler; /**< Invoked when the action is clicked. */
            };

            /** @brief Complete snackbar payload owned by the editor overlay. */
            struct SnackbarNotification {
                SnackbarLevel level = SnackbarLevel::Info; /**< Notification severity. */
                std::string title;                         /**< Primary message line. */
                std::string detail;                        /**< Secondary detail line. */
                SnackbarAction action;                     /**< Optional right-side action. */
                bool dismissible = true;                   /**< True when a close button is shown. */
                bool pinWhileAsyncImport = false;          /**< True when import progress keeps the snackbar visible. */
                float duration = 4.0f;                     /**< Lifetime in seconds after the snackbar is shown. */
            };

            /** @brief Generic background operation displayed in the editor status bar. */
            struct EditorOperation {
                std::string id;                     /**< Stable operation identifier. */
                std::string label = "Operation";    /**< Short operation name. */
                std::string detail;                 /**< Current operation subject or phase. */
                Ui::EditorStatusLevel level =
                    Ui::EditorStatusLevel::Info;    /**< Text and progress tint. */
                float progress = -1.0f;             /**< [0,1] determinate progress; negative for indeterminate. */
            };
            ...
            /** @brief Menu handler: open the Build & Release pipeline modal. */
            void OnMenuBuildPipeline();
            ...
            /** @brief UI-automation hook that adds a primitive prop by mesh tag. */
            void UiAutomationAddPrimitiveObject(std::string_view meshTag) {
                AddPrimitiveObject(meshTag);
            }
            /** @brief UI-automation hook that adds a light object by serialized light type. */
            void UiAutomationAddLightObject(std::string_view lightType) {
                AddLightObject(lightType);
            }
            ...
            /** @brief UI-automation hook: returns the build pipeline modal for CI scenario injection. */
            EditorBuildPipelineModal *UiAutomationGetBuildPipelineModal() {
                return &m_buildPipelineModal;
            }
#endif
        ...
        private:
            ...
            void DrawProjectAddMenuItems(const std::string& prefillPath);
            ...
            /** @brief Template type enum for project file creation */
            ...
            /** @brief Draws the modal to name a new script template. */
            void DrawProjectCreateScriptModal();

            /** @brief Validates and creates the requested script file from a template. */
            void HandleProjectCreateScriptSubmit();
...
/** @brief Draws the Favorites and root nodes of the project tree.
            ...
            /** @brief Draws editable fields for one attached component. */
            void DrawPropertiesComponentBody(SceneObject &obj, ComponentDesc &comp);
            ...
            /** @brief Queues the project-panel add menu for a directory tree row. */
            void RequestProjectTreeAddMenuForDirectory(const std::filesystem::path &directoryPath);
            ...
            /** @brief Returns true while a modal dialog should own editor input. */
            bool HasBlockingEditorDialogOpen() const;

            /** @brief Mirrors current physical input into edge-tracking state while dialogs are focused. */
            void SyncInputStateWhileDialogOpen();
            ...
            /** @brief Creates and inserts a light object with a concrete light kind. */
            void AddLightObject(std::string_view lightType, std::string_view parentId = {});

            /** @brief Creates and inserts a Prop with a built-in primitive mesh tag.
             *  @param meshTag  Identifier for the primitive shape (e.g. "cube", "sphere").
             *  @param parentId Optional parent object ID.
             */
            void AddPrimitiveObject(std::string_view meshTag, std::string_view parentId = {});
            ...
            /**
             * @brief Draws hierarchy actions for adding an object under a parent.
             * @param parentId Parent object identifier, or empty for a scene root.
             */
            void DrawHierarchyAddMenu(std::string_view parentId);

            /**
             * @brief Draws hierarchy primitive-object menu entries.
             * @param parentId Parent object identifier, or empty for a scene root.
             */
            void DrawHierarchyPrimitiveMenu(std::string_view parentId);

            /**
             * @brief Draws hierarchy environment-object menu entries.
             * @param parentId Parent object identifier, or empty for a scene root.
             */
            void DrawHierarchyEnvironmentMenu(std::string_view parentId);

            /**
             * @brief Draws hierarchy utility-object menu entries.
             * @param parentId Parent object identifier, or empty for a scene root.
             */
            void DrawHierarchyUtilityMenu(std::string_view parentId);

            /**
             * @brief Draws hierarchy light-object menu entries.
             * @param parentId Parent object identifier, or empty for a scene root.
             */
            void DrawHierarchyLightMenu(std::string_view parentId);
            ...
            // ── Async import state ──
            bool m_asyncImportActive = false;
            std::future<AssetImportResult> m_asyncImportFuture;
            ImportAssetRequest m_asyncImportRequest;
            std::string m_asyncImportAssetGuid;

            /** @brief Polls the async import future; processes result when ready. */
            void PollAsyncImport();

            SnackbarNotification m_snackbar; /**< Current snackbar payload. */
            float m_snackbarTimer = 0.0f;    /**< Remaining snackbar lifetime in seconds. */
            float m_snackbarDismissTimer = 0.0f;
            /**< Elapsed time in the snackbar slide-out animation. */
            bool m_snackbarDismissing = false;
            /**< True while the snackbar is sliding out of the viewport. */
            static constexpr float kSnackbarDuration = 4.0f;
            static constexpr float kSnackbarDismissDuration = 0.28f;

            /** @brief Shows a snackbar notification at the bottom-right.
             *  @param notification Complete snackbar payload to render. */
            void ShowSnackbar(SnackbarNotification notification);

            /** @brief Shows a simple informational snackbar notification.
             *  @param msg Message used as the snackbar title. */
            void ShowSnackbar(const std::string& msg);

            /** @brief Shows a simple informational snackbar notification for a custom duration.
             *  @param msg Message used as the snackbar title.
             *  @param duration Seconds before the snackbar fades out. */
            void ShowSnackbar(const std::string& msg, float duration);

            /** @brief Draws the snackbar overlay with fade-out timer. */
            void DrawSnackbar();

            /** @brief Starts the snackbar slide-out animation if one is visible. */
            void DismissSnackbar();

            EditorOperation m_editorOperation; /**< Current status-bar background operation. */
            bool m_editorOperationActive = false;
            /**< True while an operation is shown in the status bar. */

            /** @brief Shows or replaces the status-bar background operation.
             *  @param operation Operation payload to render. */
            void ShowEditorOperation(EditorOperation operation);

            /** @brief Clears the status-bar background operation with @p id when active.
             *  @param id Identifier of the operation to clear. Empty clears any active operation. */
            void ClearEditorOperation(std::string_view id = {});
            EditorBuildPipelineModal m_buildPipelineModal; /**< Build & Release pipeline modal. */
            EditorUserSettingsDocument m_userSettingsDocument{};
                                                       /**< Persisted user-wide editor preferences (theme preset). */
            Horo::Build::ToolchainSettingsStore m_toolchainStore;
            ...
            bool m_projectPanelAddMenuRequested = false;    /**< One-frame request to open the add menu popup. */
            bool m_projectPanelCreateModalRequested = false;/**< One-frame request to open create-entry modal. */
            bool m_projectPanelCreateModalOpen = false;     /**< True while project create modal owns focus. */
            bool m_projectPanelCreateFolder = true;         /**< Create modal target type: folder when true, file when false. */
            bool m_projectPanelCreateScriptModalRequested = false; /**< One-frame request to open create-script modal. */
            bool m_projectPanelCreateScriptModalOpen = false;      /**< True while create-script modal owns focus. */
            ScriptTemplateType m_projectPanelScriptTemplateType = ScriptTemplateType::BehaviorScript; /**< Selected template type. */
            std::string m_projectPanelTemplateSearchText;   /**< Draft search text in template dropdown. */
            std::string m_projectPanelCreateBasePath;       /**< Target base path relative to project root. */
            ...
            /** @brief MCP handler: start a release build for the current project. */
            Mcp::McpCommandResult McpHandleProjectRelease(const nlohmann::json &arguments);
        };
    } // namespace Editor
} // namespace Horo
