...
struct EditorCardConfig {
  ...
  ImVec2      padding  = ImVec2(-1.0f, -1.0f); /**< Optional inner padding; negative uses theme card padding. */
};
...
/**
 * @brief One item in a horizontal editor tab strip.
 */
struct EditorTopTabItem {
    const char *id = nullptr;    /**< Stable ImGui id suffix. */
    const char *label = nullptr; /**< Visible tab label. */
    void (*drawIcon)(ImDrawList &, ImVec2, float, ImU32) = nullptr; /**< Optional icon. */
    bool selected = false;       /**< Whether this tab is currently active. */
};

/**
 * @brief Result from RenderEditorTopTabBar().
 */
struct EditorTopTabBarResult {
    int clickedIndex = -1; /**< Index clicked this frame, or -1. */
};

/**
 * @brief Renders a full-width bordered tab strip with icon+label tabs.
 *
 * The selected tab receives accent-coloured text/icon and an accent underline,
 * while inactive tabs share the surrounding card background. The caller owns
 * selection state and updates it from the returned clicked index.
 *
 * @param theme Active editor theme.
 * @param id    Stable ImGui id for the strip.
 * @param tabs  Ordered tab definitions.
 * @param width Pixel width; 0 = fill available width.
 * @param height Pixel height for the strip.
 * @return Result containing the clicked tab index.
 */
...
/**
 * @brief One item in a compact segmented selection control.
 */
struct EditorSegmentedButtonItem {
    const char *id = nullptr;    /**< Stable ImGui id suffix. */
    const char *label = nullptr; /**< Visible button label. */
    bool selected = false;       /**< Whether this segment is active. */
};

/**
 * @brief Renders compact adjacent selection buttons.
 *
 * The component does not own selection state. It only reports which segment
 * was clicked so callers can implement single- or multi-selection semantics.
 *
 * @param theme Active editor theme.
 * @param id Stable ImGui id prefix for all segments.
 * @param items Ordered segment definitions.
 * @return Index clicked this frame, or -1.
 */
...
struct EditorStatusBarItem {
  ...
  bool showProgress = false; /**< True when the item renders a compact progress rail. */
  float progress = -1.0f;    /**< [0,1] for determinate progress; negative for indeterminate. */
};
...
/**
 * @brief Draws a circular-arrow refresh icon directly into an ImDrawList.
 *
 * @param drawList Target draw list.
 * @param origin   Top-left corner of the icon bounding box.
 * @param size     Width and height of the icon (square).
 * @param color    RGBA32 fill colour.
 */
void DrawRefreshIcon(ImDrawList &drawList, ImVec2 origin, float size, ImU32 color);

/**
 * @brief Draws a document-with-lines export icon directly into an ImDrawList.
 *
 * @param drawList Target draw list.
 * @param origin   Top-left corner of the icon bounding box.
 * @param size     Width and height of the icon (square).
...
/**
 * @brief Renders a themed button with an icon on the left and a label.
 *
 * The icon is drawn via @p drawIcon, which receives the draw-list, origin,
 * size, and current icon color (derived from theme / hover / active state).
 *
 * @param theme    Active editor theme.
 * @param label    Button label displayed to the right of the icon.
 * @param drawIcon Function that draws the icon into the draw-list.
 * @param size     Button size; defaults to (0,0) = auto-sized.
 * @return True when the button is clicked this frame.
 */
...
/**
 * @brief Visual style for an action button in an editor dialog footer.
 */
enum class EditorDialogFooterButtonStyle {
    Secondary,  /**< Neutral outlined/card button. */
    Primary,    /**< Accent-filled primary action. */
    Destructive /**< Danger-filled action. */
};

/**
 * @brief One action button rendered in a dialog footer.
 */
struct EditorDialogFooterButton {
    const char *id = nullptr;    /**< Stable ImGui id suffix. */
    const char *label = nullptr; /**< Visible button text. */
    EditorDialogFooterButtonStyle style = EditorDialogFooterButtonStyle::Secondary;
    void (*drawIcon)(ImDrawList &, ImVec2, float, ImU32) = nullptr; /**< Optional leading icon. */
    float width = 0.0f; /**< Button width; 0 = measured from content. */
    bool enabled = true; /**< Disabled when false. */
};

/**
 * @brief Optional left-side status/progress content for a dialog footer.
 */
struct EditorDialogFooterProgress {
    const char *label = nullptr; /**< Muted prefix, e.g. "Build Progress:". */
    const char *value = nullptr; /**< Strong status value, e.g. "Succeeded". */
    float progress = -1.0f;      /**< 0..1 shows a progress bar; negative hides it. */
    float barWidth = 300.0f;     /**< Preferred progress bar width. */
};

/**
 * @brief Configuration for RenderEditorDialogFooter().
 */
struct EditorDialogFooterConfig {
    const char *id = nullptr; /**< Stable child id for the footer. */
    float height = 64.0f;     /**< Footer height in pixels. */
    EditorDialogFooterProgress progress{}; /**< Optional left-side progress block. */
    std::span<const EditorDialogFooterButton> buttons{}; /**< Right-aligned actions. */
};

/**
 * @brief Renders a common full-width dialog footer with optional progress.
 *
 * The footer owns the top divider, inner padding, right-aligned action layout,
 * and primary/secondary/destructive button styling. It does not close popups;
 * callers handle the returned clicked index.
 *
 * @param theme Active editor theme.
 * @param cfg   Footer configuration.
 * @return Index of the clicked button in @p cfg.buttons, or -1.
 */
int RenderEditorDialogFooter(const EditorTheme &theme,
                             const EditorDialogFooterConfig &cfg);

/** @brief Configuration and callbacks for RenderBuildFooterBar(). */
struct EditorBuildFooterConfig {
    const char *statusLabel = nullptr; /**< Left-side build status text. */
    float progress = -1.0f;            /**< 0..1 progress; negative hides the bar. */
    bool showRebuild = false;          /**< Show the Rebuild action. */
    bool showExportLog = false;        /**< Show the Export Log action. */
    std::function<void()> onRebuild;   /**< Invoked when Rebuild is clicked. */
    std::function<void()> onExportLog; /**< Invoked when Export Log is clicked. */
    std::function<void()> onClose;     /**< Invoked when Close is clicked. */
};

/**
 * @brief Renders build progress and actions in a shared footer.
 * @param theme Active editor theme.
 * @param cfg Footer content, visibility flags, and action callbacks.
 * @return True when Close is clicked.
 */
bool RenderBuildFooterBar(const EditorTheme &theme,
                          const EditorBuildFooterConfig &cfg);

/**
 * @brief Minimal entry for a recent-run list item.
 *
 * Populated by the build pipeline modal from BuildHistoryEntry data.
 * The renderer uses @c platformLabel and @c detail together to produce
 * a self-describing row that distinguishes e.g. a Windows failure from
 * a macOS success at a glance.
 */
struct RecentRunEntry {
    bool succeeded = true;           /**< Whether the run passed. */
    std::string label;               /**< Primary label (e.g. "Today, 10:34"). */
    std::string detail;              /**< Secondary text (e.g. "Succeeded"). */
    std::string duration;            /**< Right-aligned duration (e.g. "00:01:24"). */
    std::string platformLabel;       /**< Platform name (e.g. "macOS", "Windows", "Linux"). */
    std::function<void()> onClick;   /**< Optional callback when the entry row is clicked. */
};

/**
 * @brief Renders a bordered card with a "Build Progress" header, a progress
 *        bar, and a percentage label.
 *
 * @param theme    Active editor theme.
 * @param progress 0.0 – 1.0 progress value.
 * @param height   Card height; 0 = auto (compact).
 */
void RenderBuildProgressCard(const EditorTheme &theme, float progress,
                             float height = 0.0f);

/**
 * @brief Renders a bordered card listing recent build runs with status dots.
 *
 * @param theme    Active editor theme.
 * @param entries  Recent run entries, most recent last.
 * @param maxShow  Maximum number of entries to display (0 = unlimited).
 */
...
/**
 * @brief Renders a filterable, scrollable log viewer with All / Warnings /
 *        Errors filter buttons.
 *
 * @param theme  Active editor theme.
 * @param logText Raw multi-line log text.
 * @param height Fixed height; 0 = fill remaining space.
 */
void RenderLogViewer(const EditorTheme &theme, const std::string &logText,
                     float height = 0.0f);

/**
 * @brief Renders a rounded summary card showing build metadata in a
 *        four-column labelled row.
 *
 * @param theme   Active editor theme.
 * @param cells   Array of 4 {label, value} pairs (Target, Preset, Status, Elapsed).
 * @param accent  Accent colour hint for the Status value (e.g. red for failed);
 *                pass an alpha of 0 to use the default text colour.
 */
void RenderBuildSummaryCard(const EditorTheme &theme,
                            const std::pair<const char *, const char *> (&cells)[4],
                            const ImVec4 &statusAccent = ImVec4(0, 0, 0, 0));

/**
 * @brief Entry for a selectable card in a multi- or single-select grid.
 */
struct SelectableCardEntry {
     std::string id;                                 /**< Unique identifier. */
     std::string title;                              /**< Primary label. */
     std::string subtitle;                           /**< Secondary muted label. */
     void (*drawIcon)(ImDrawList &, ImVec2, float, ImU32) = nullptr; /**< Optional icon. */
     bool selected = false;                          /**< Current toggle state. */
     bool disabled = false;                          /**< When true, card is greyed out and cannot be toggled. */
     std::string tooltip;                           /**< Optional hover tooltip shown when the card is hovered. */
 };

/**
 * @brief Renders a grid of rounded selectable cards.
 *
 * Clicking toggles @c selected and fires @p onToggle by default.  When
 * @p disabled is true on an entry, the card is drawn muted and clicking
 * has no effect.  Callers that want single-select (radio) behaviour can
 * implement it inside @p onToggle by clearing other entries' selection.
 *
 * @param theme    Active editor theme.
 * @param entries  Card definitions; updated in-place when toggled.
 * @param onToggle Called with the index of the toggled card.
 * @param columns  Number of cards per row (0 = auto-fill).
 */
