...
class TransformGizmo {
 public:
  void Activate(GizmoMode mode, Vec3 targetPos, Quaternion targetRot, Vec3 targetScale);
  void Deactivate();
  ...
  // Screen-space axis hit test. Returns nearest axis whose 2D segment is within 12 px,
  // or GizmoAxis::None.
  GizmoAxis PickAxis(float mx, float my, const Camera& cam, int screenW, int screenH) const;
  ...
  // Ray-plane intersection. Returns false if ray is parallel or hit is behind origin.
  static bool RayHitPlane(const Ray& ray, const Vec3& normal, const Vec3& point, Vec3& outHit);

  // Closest point on an infinite line to a ray's nearest approach.
  static Vec3 RayClosestOnLine(const Ray& ray, const Vec3& lineOrigin, const Vec3& lineDir);
