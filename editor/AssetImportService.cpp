...
namespace Monolith::Editor {
    namespace {
        ...
        void AppendUniqueDependency(AssetMetadata *metadata, AssetDependencyKind kind,
            ...
            if (!metadata || value.empty())
                return;
            const auto it =
                    std::ranges::find_if(metadata->dependencies,
        ...
        void LogDiagnostics(const std::vector<AssetImportDiagnostic> &diagnostics) {
            for (const AssetImportDiagnostic &diagnostic: diagnostics) {
                ...
                if (diagnostic.severity == AssetDiagnosticSeverity::Info) {
                    LogInfo("[AssetImport][{}][{}] {} ({})", importer,
                            diagnostic.code, diagnostic.message, source);
                } else if (diagnostic.severity == AssetDiagnosticSeverity::Warning) {
                    LogWarn("[AssetImport][{}][{}] {} ({})", importer,
                            diagnostic.code, diagnostic.message, source);
                } else {
                    LogError("[AssetImport][{}][{}] {} ({})", importer,
                             diagnostic.code, diagnostic.message, source);
                }
            }
        }
    ...
    namespace {
        ...
        void BuildReimportLookupMaps(
            ...
            std::unordered_map<std::string, std::string, StringHash, std::equal_to<> > &assetIdByGuid,
            std::unordered_map<std::string, AssetMetadata, StringHash, std::equal_to<> > &metadataByGuid) {
    ...
    AssetReimportResult AssetImportService::ReimportAssetWithDependents(
        ...
        if (!doc) {
            ...
            result.records.emplace_back(std::string{}, std::string{}, reason, false, result.error);
            return result;
        }
        ...
        if (!assetIdByGuid.contains(rootAssetGuid)) {
            ...
            result.records.emplace_back(std::string{}, rootAssetGuid, reason, false, result.error);
            return result;
        }

        ReimportDepGraph depGraph = BuildReimportDepGraph(rootAssetGuid, metadataByGuid);
    ...
    bool AssetImportService::ReimportSingleAsset(const std::string &guid,
                                                 const std::string &recordReason,
                                                 SceneDocument *doc,
                                                 AssetReimportResult &result) const {
        auto assetIt = std::ranges::find_if(
            doc->assets,
        ...
        if (metadata.importerId.empty() || metadata.sourcePath.empty()) {
            ...
            result.records.emplace_back(assetIt->first, guid, recordReason, false, error);
            result.error = error;
            return false;
        }
        ...
        if (!importResult.ok) {
            ...
            result.records.emplace_back(assetIt->first, guid, recordReason, false, importResult.error);
        ...
        if (const auto albedoIt = metadata.settings.find("albedoSourcePath");
            ...
            if (textureImporter) {
                AssetImportRequest textureRequest{
                    assetIt->first, assetIt->second.guid, assetIt->second.displayName,
                    albedoPath, metadata.settings
                };
        ...
        if (std::string saveError; !SaveAssetMetadata(updatedMetadata, &saveError)) {
            result.records.emplace_back(assetIt->first, guid, recordReason, false, saveError);
            result.error = saveError;
            return false;
        }

        result.records.emplace_back(assetIt->first, guid, recordReason, true, std::string{});
        return true;
    }
} // namespace Monolith::Editor
