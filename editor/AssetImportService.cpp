...
AssetImportResult AssetImportService::ImportAssetFromSource(
  ...
  const AssetImportRequest request{assetId, assetGuid, displayName, sourcePath,
                                   settings};
...
bool AssetImportService::ImportTextureForAsset(const std::string &sourcePath,
  ...
  const AssetImportRequest request{assetId, asset->guid, asset->displayName,
...
bool AssetImportService::ReimportSingleAsset(
  ...
  AssetImportRequest request{assetIt->first, assetIt->second.guid,
  ...
  if (const auto albedoIt = metadata.settings.find("albedoSourcePath");
    ...
    if (textureImporter) {
      AssetImportRequest textureRequest{assetIt->first, assetIt->second.guid,
