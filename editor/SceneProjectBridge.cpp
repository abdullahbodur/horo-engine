...
namespace Monolith::Editor {
    namespace {
        ...
        const std::string *
        FindProp(const std::unordered_map<std::string, std::string, StringHash, std::equal_to<> > &props,
                 const char *key) {
            const auto it = props.find(key);
            return (it != props.end()) ? &it->second : nullptr;
        }

        void EraseKeys(std::unordered_map<std::string, std::string, StringHash, std::equal_to<> > *props,
        ...
        SceneLightProperties BuildLightFromComponent(const ComponentDesc &component) {
            ...
            if (const auto it = component.props.find("color"); it != component.props.end())
    ...
    SceneProjectModel BuildSceneProjectModel(const SceneDocument &doc) {
        ...
        if (const auto spawnIt = model.scene.settings.extraSettings.find("spawnPoint");
