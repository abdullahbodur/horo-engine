    ...
    namespace {
        ...
        ScanMtlForTextureNames(const std::filesystem::path &mtlSource) {
            ...
            while (std::getline(mtlFile, line)) {
                if (line.size() < 8)
                    continue;
                if (const std::string prefix = ToLowerAscii(line.substr(0, 4)); prefix != "map_")
                    continue;
        ...
        AssetMetadata BuildImportedMetadata(const AssetImportRequest &request,
            ...
            metadata.dependencies.emplace_back(AssetDependencyKind::Source, request.sourcePath);
            for (const std::string &path: metadata.producedFiles)
                metadata.dependencies.emplace_back(AssetDependencyKind::ProducedOutput, path);
            return metadata;
        }
            ...
            AssetImportResult Import(const AssetImportRequest &request) const override {
                ...
                if (const std::string diffuseTexture =
                    ...
                    asset.albedoMap =
                            ...
                            .generic_string();
                    if (std::ranges::find(producedFiles, asset.albedoMap) == producedFiles.end())
