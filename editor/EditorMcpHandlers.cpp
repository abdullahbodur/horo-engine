...
BuildIssueSeverityText(SceneProjectValidationIssue::Severity severity) {
  return severity == SceneProjectValidationIssue::Severity::Error ? "error"
                                                                  : "warning";
}

const char *BuildIssueSeverityText(RuntimeSceneBuildIssue::Severity severity) {
  return severity == RuntimeSceneBuildIssue::Severity::Error ? "error"
                                                             : "warning";
}
...
McpParseProps(const json &value) {
  ...
  for (auto it = value.begin(); it != value.end(); ++it) {
    props[it.key()] = it.value().is_string() ? it.value().get<std::string>()
                                             : it.value().dump();
  }
  return props;
}
...
EditorLayer::ExecuteMcpCommand(std::string_view toolName,
  ...
  return Mcp::McpCommandResult{false, json::object(),
...
EditorLayer::McpHandleReparentObject(const nlohmann::json &arguments) {
  ...
  MarkDirtyAndReload();
  CommitHistoryChange(before);
  return Mcp::McpCommandResult{true, json{{"id", id}, {"parentId", parentId}},
                               std::string()};
}
...
EditorLayer::McpHandleDuplicate(const nlohmann::json &arguments) {
  ...
  MarkDirtyAndReload();
  CommitHistoryChange(before);
  return Mcp::McpCommandResult{true, json{{"duplicates", std::move(created)}},
                               std::string()};
}
