...
namespace Monolith::Editor {
    namespace {
        // Must match DrawToolbar / DrawStatusBar so left-column windows do not overlap
        // each other or the status strip.
        ...
        const char *
        BuildIssueSeverityText(SceneProjectValidationIssue::Severity severity) {
            return severity == SceneProjectValidationIssue::Severity::Error
                       ? "error"
                       : "warning";
        }

        const char *BuildIssueSeverityText(RuntimeSceneBuildIssue::Severity severity) {
            return severity == RuntimeSceneBuildIssue::Severity::Error
                       ? "error"
                       : "warning";
        }
        ...
        Mcp::McpBuildSnapshot BuildMcpBuildSnapshot(const SceneDocument &document) {
            ...
            for (const SceneProjectValidationIssue &issue: validation.issues) {
                ...
                validationIssueKeys.insert(
                    BuildIssueKey(severity, issue.path, issue.message));
            ...
            for (const RuntimeSceneBuildIssue &issue: runtimeBuild.issues) {
                const std::string severity = BuildIssueSeverityText(issue.severity);
                if (validationIssueKeys.contains(
                    BuildIssueKey(severity, issue.path, issue.message)))
                    continue;
            ...
            const size_t totalErrors =
                    snapshot.sceneValidationErrors + snapshot.runtimeBuildErrors;
            const size_t totalWarnings =
                    snapshot.sceneValidationWarnings + snapshot.runtimeBuildWarnings;
            if (totalErrors > 0)
        ...
        Mcp::McpSchemaEntrySnapshot
        BuildMcpSchemaEntrySnapshot(const TypeSchema &schema) {
        ...
        Mcp::McpSchemaEntrySnapshot
        BuildMcpSchemaEntrySnapshot(const ComponentSchema &schema) {
            Mcp::McpSchemaEntrySnapshot snapshot;
        ...
        Mcp::McpSchemaCatalogSnapshot
        BuildMcpSchemaCatalogSnapshot(const EditorSchema &schema) {
            ...
            for (const std::string &typeName: typeNames)
                snapshot.objectTypes.push_back(
                    BuildMcpSchemaEntrySnapshot(schema.TypeSchemas().at(typeName)));
            ...
            for (const std::string &componentType: componentTypes)
                snapshot.components.push_back(BuildMcpSchemaEntrySnapshot(
                    schema.ComponentSchemas().at(componentType)));

            return snapshot;
        }

        bool SchemaAppliesToObjectType(const std::vector<std::string> &appliesTo,
        ...
        bool AssetDefsEqual(const AssetDef &lhs, const AssetDef &rhs) {
            return lhs.mesh == rhs.mesh && lhs.renderScale == rhs.renderScale &&
                   lhs.albedoMap == rhs.albedoMap && lhs.guid == rhs.guid &&
        ...
        bool SceneObjectsEqual(const SceneObject &lhs, const SceneObject &rhs) {
            ...
            if (lhs.position.x != rhs.position.x || lhs.position.y != rhs.position.y ||
                ...
                return false;
            if (lhs.scale.x != rhs.scale.x || lhs.scale.y != rhs.scale.y ||
                lhs.scale.z != rhs.scale.z)
                return false;
            if (lhs.yaw != rhs.yaw || lhs.pitch != rhs.pitch || lhs.roll != rhs.roll)
                return false;
            if (lhs.assetId != rhs.assetId || lhs.props != rhs.props)
                return false;
            ...
            if (!prefabEqual || lhs.components.size() != rhs.components.size())
                return false;
        ...
        bool SceneDocumentsEqual(const SceneDocument &lhs, const SceneDocument &rhs) {
            if (lhs.version != rhs.version || lhs.sceneId != rhs.sceneId ||
                lhs.sceneName != rhs.sceneName || lhs.filePath != rhs.filePath ||
                lhs.settings != rhs.settings || lhs.assets.size() != rhs.assets.size() ||
        ...
        constexpr std::string_view kEmptyParentId;

        std::string_view GetParentId(const SceneObject &obj) {
            const auto it = obj.props.find("parentId");
            return (it != obj.props.end()) ? std::string_view{it->second} : kEmptyParentId;
        }

        int FindObjectIndexById(const SceneDocument &doc,
                                std::string_view id) {
            if (id.empty())
                return -1;
        ...
        bool IsDescendantOf(const SceneDocument &doc, int nodeIdx,
                            int ancestorIdx) {
        ...
        void PropagateHierarchyTransformDelta(
            ...
            const Quaternion deltaRot =
                    newParent.rotation * oldParent.rotation.Inverse();
        ...
        bool ParseSceneObjectType(std::string_view raw, SceneObjectType *outType) {
            using enum SceneObjectType;
            if (!outType)
                return false;
                ...
                for (const auto &[key, val]: object.props) {
                    ...
                    if (!objectIds.contains(val))
                        LogWarn("[Editor] Dangling object reference in {}: {}.{} -> {}",
                                std::string(sourceLabel), object.id,
                                key, val);
                }
            }
        }
        ...
        void SyncAssetScaleMetadata(SceneDocument *doc) {
            if (!doc)
                return;
        ...
        // Parses "r,g,b" from a comma-separated string into a 3-element float array.
        void ParseRGBString(std::string_view s, float col[3]) {
        ...
        int FindEnumOptionIndex(const std::vector<std::string> &options,
                                std::string_view val) {
        ...
        std::string BuildImGuiComboItems(const std::vector<std::string> &options) {
            std::string items;
        ...
        SceneObject &ObjectAt(SceneDocument &doc, int idx) {
            return ObjectAt(doc, idx);
        }

        const SceneObject &ObjectAt(const SceneDocument &doc, int idx) {
            return ObjectAt(doc, idx);
        }
    } // namespace
    ...
    void EditorLayer::DrawPropertiesPanel() { // NOSONAR: cpp:S3776 properties panel integrates multiple section editors with mode-specific branching
        using enum SceneObjectType;
        const ImGuiIO &io = ImGui::GetIO();
        const float bottomDockH = ComputeEditorBottomDockHeight(io.DisplaySize.y);
        const float rightDockW = ComputeEditorRightPanelWidth(io.DisplaySize.x);
    ...
    void EditorLayer::ApplyBatchAssetChange(
        ...
        if (m_batchAssetChoice == 1) {
            ...
            for (int idx: m_selectedIndices) {
                if (idx >= 0 && idx < static_cast<int>(m_document.objects.size()))
                    ObjectAt(m_document, idx).assetId =
                            selectedAssetId;
            }
        }
    ...
    void EditorLayer::DrawPropertiesMultiSelect() {
        ...
        ImGui::Separator();
        if (std::array<float, 3> moveBy = {
                m_batchTranslateDraft.x,
        ...
        if (std::array<float, 3> rotateBy = {
                m_batchRotateDraft.x,
        ...
        if (std::array<float, 3> scaleBy = {
                m_batchScaleDraft.x, m_batchScaleDraft.y,
                m_batchScaleDraft.z
            };
    ...
    void EditorLayer::DrawAssetDiagnosticsSection(const AssetMetadata &metadata) const {
    ...
    void EditorLayer::DrawPropertiesSelectedAsset(
        std::unordered_map<std::string, AssetDef, StringHash, std::equal_to<> >::iterator assetIt) {
        ImGui::TextDisabled("Asset");
        ...
        const bool hasAssetMetadata = LoadAssetMetadata(
            assetIt->second.guid, &assetMetadata, &metadataError);
        if (hasAssetMetadata)
            ...
            if (ImGui::Button("Reimport Asset", ImVec2(-1.0f, 0.0f))) {
                const AssetReimportResult reimportResult =
                        m_assetImportService.ReimportAssetWithDependents(
                            &m_document, assetIt->second.guid,
        ...
        assetIt->second.displayName.copy(displayEditBuf.data(), displayEditBuf.size() - 1);
        ...
        assetIt->second.renderScale.copy(scaleEditBuf.data(), scaleEditBuf.size() - 1);
...
#if defined(_WIN32) || defined(__APPLE__)
        if (ImGui::Button("Browse texture...##alb_pick_asset",
                          ImVec2(-1.0f, 0.0f)))
    ...
    void EditorLayer::DrawPropertiesIdentitySection(
        SceneObject &obj, int primaryIdx) {
        using enum SceneObjectType;
        // ---- Identity ----
    ...
    void EditorLayer::DrawPropertiesCameraSection(
        SceneObject &obj, int primaryIdx) {
        ...
        if (std::array<float, 3> pos = {obj.position.x, obj.position.y, obj.position.z};
        ...
        if (float pitch = obj.pitch; ImGui::DragFloat("Pitch", &pitch, 1.0f, -89.0f, 89.0f)) {
        ...
        auto drawCamFloatProp = [&](const char *key, auto drawWidget, const char *defaultVal) {
        ...
        drawCamFloatProp("fov", [](float &v) { return ImGui::SliderFloat("FOV", &v, 1.0f, 179.0f); }, "60");
        drawCamFloatProp("nearClip", [](float &v) { return ImGui::DragFloat("Near Clip", &v, 0.01f, 0.001f, 100.0f); }, "0.1");
        drawCamFloatProp("farClip", [](float &v) { return ImGui::DragFloat("Far Clip", &v, 1.0f, 1.0f, 100000.0f); }, "500");
    ...
    void EditorLayer::DrawPropertiesTransformSection(
        SceneObject &obj, int primaryIdx) {
        const Vec3 oldPos = obj.position;
        const Quaternion oldRot = Quaternion::FromEuler(
            ToRadians(obj.pitch), ToRadians(obj.yaw), ToRadians(obj.roll));
        ...
        if (std::array<float, 3> rot = {obj.pitch, obj.yaw, obj.roll};
            ImGui::DragFloat3("Rotation (P/Y/R)", rot.data(), 1.0f, -360.0f, 360.0f)) {
            obj.pitch = rot[0];
            obj.yaw = rot[1];
            obj.roll = rot[2];
            changedTransform = true;
    ...
    void EditorLayer::DrawSchemaFieldWidget(const SceneObject &obj, const FieldDef &fd,
                                            std::string &val) {
        using enum SceneObjectType;
    ...
    void EditorLayer::DrawLightComponentFields(ComponentDesc &comp) {
        // Intensity
        if (float intensity = comp.props.contains("intensity")
    ...
    void EditorLayer::DrawScriptComponentField(ComponentDesc &comp) {
        ...
        if (!current.empty() &&
            std::ranges::find(options, current) == options.end())
    ...
    void EditorLayer::DrawPropertiesComponentsList(SceneObject &obj) {
        // NOSONAR: cpp:S3776 component list with per-type edit panels; complexity from component type dispatch
        // ---- Components ----
        ImGui::Separator();
    ...
    void EditorLayer::DrawAddComponentMenuItems(SceneObject &obj) { // NOSONAR: cpp:S3776 sorted component add menu with schema-driven items
    ...
    void EditorLayer::CommitHistoryChange(const EditorHistorySnapshot &before) {
        if (EditorHistorySnapshot after = CaptureHistorySnapshot(); HistorySnapshotsEqual(before, after))
            return;
        ...
        auto refreshSnapshot = [this](EditorHistorySnapshot *snapshot) {
            ...
            if (snapshot->document.filePath == m_document.filePath) {
                ...
                snapshot->document.dirty =
                        !SceneDocumentsEqual(snapshot->document, snapshot->savedDocument);
            }
        };
