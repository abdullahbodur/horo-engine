...
void EditorLayer::HandleEditorKeyboardShortcuts(
    const Camera &cam) { // NOSONAR: cpp:S3776 multi-shortcut dispatch; each
                         // branch is a distinct binding
  if (!m_window)
    return;
...
void EditorLayer::ProcessDeferredFilePicks() { // NOSONAR: cpp:S3776 switch over
                                               // all deferred pick types; each
                                               // case is distinct
...
void EditorLayer::DrawViewGimbal(
    const Camera
        &cam) { // NOSONAR: cpp:S3776 complex axis gimbal with sort, hover, and
                // draw; extraction would obscure the per-axis draw loop
  using enum ViewSnap;
  const ImGuiIO &io = ImGui::GetIO();
  ...
  ImGui::PushStyleColor(ImGuiCol_Button,
                        m_wireframeMode ? ImVec4(0.15f, 0.55f, 0.15f, 0.90f)
  ...
  for (int i = 0; i < static_cast<int>(doc.objects.size()); ++i) {
    ...
    if (isPrimary && ImGui::BeginDragDropTarget()) {
      if (const ImGuiPayload *payload =
        ...
        const std::string assetId(static_cast<const char *>(payload->Data));
        std::string createError;
        if (!CreateObjectFromAsset(
                assetId, obj.id, nullptr, nullptr, nullptr,
                &createError)) // NOSONAR: cpp:S134 depth from drag-drop handler
                               // nesting in search mode
