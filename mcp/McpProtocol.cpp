...
namespace Monolith::Mcp {
    ...
    namespace {
        ...
        bool IsWriteToolName(std::string_view name) {
            static const std::unordered_set<std::string, StringHash, std::equal_to<> > kWriteTools = {
        ...
        bool IsDestructiveToolName(std::string_view name) {
            static const std::unordered_set<std::string, StringHash, std::equal_to<> > kDestructiveTools = {
                "editor.delete", "editor.delete_asset",
                "editor.new_scene", "editor.reload_scene",
            };
        ...
        bool NormalizePropsObject(const json &input,
            ...
            if (schema && !ValidateNormalizedSchemaProps(*schema, prefix, normalized,
                                                         error))
                return false;
        ...
        bool NormalizeComponentList(const json &input,
            ...
            for (size_t index = 0; index < input.size(); ++index) {
                ...
                if (!item.is_object() || !item.contains("type") ||
                    !item["type"].is_string()) {
                    if (error)
                        *error = std::format("components[{}] must include a string type.",
                                             index);
                    return false;
                }
                ...
                if (!NormalizePropsObject(
                    item.value("props", json::object()), schema, false, applyDefaults,
                    &props, error,
        ...
        bool NormalizeObjectTypeName(const std::string &raw, std::string *outType) {
            if (!outType)
                return false;
            static const std::unordered_map<std::string, std::string, StringHash, std::equal_to<> > kTypes = {
                {"panel", "Panel"}, {"prop", "Prop"}, {"light", "Light"}, {"camera", "Camera"},
            };
        ...
        bool ConfigureLookupToolSchema(std::string_view toolName, json *inputSchema) {
            if (static const std::unordered_set<std::string, StringHash, std::equal_to<> > kIdTools = {
                "editor.get_object", "editor.get_object_edges",
                "editor.get_asset", "editor.get_object_children",
                "editor.get_object_parent", "editor.rename_object",
                "editor.reparent_object", "editor.select_asset",
                "editor.update_asset", "editor.delete_asset",
        ...
        json BuildResourcePayload(const McpEditorSnapshot &snapshot,
            ...
            if (uri == "scene://objects")
                return BuildObjectListJson(snapshot, params.value("limit", 12),
                                           params.value("type", ""),
                                           params.value("query", ""),
                                           params.value("selectedOnly", false),
                                           params.value("offset", 0));
        ...
        bool NormalizeUpdateAssetArgs(const McpEditorSnapshot &snapshot,
            ...
            for (const char *key: {"mesh", "renderScale", "albedoMap", "displayName"}) {
                ...
                if (!arguments[key].is_string() && !arguments[key].is_null())
                    return FailNormalization(error, std::string(key) + " must be a string or null.");
        ...
        bool NormalizeMutationCreateFromAssetFields(const ObjectMutationContext &ctx,
            ...
            if (!ctx.flags.isCreateFromAsset)
                return true;
            if (!ctx.arguments.contains("assetId") || !ctx.arguments["assetId"].is_string())
        ...
        bool NormalizeMutationIdentityFields(const ObjectMutationContext &ctx,
            ...
            if (ctx.arguments.contains("assetId")) {
                ...
                if (!ctx.arguments["assetId"].is_string() && !ctx.arguments["assetId"].is_null())
        ...
        bool NormalizeMutationTransformFields(const ObjectMutationContext &ctx,
            ...
            for (const char *key: {"yaw", "pitch", "roll"}) {
                ...
                if (!ctx.arguments[key].is_number())
                    return FailNormalization(ctx.error, std::string(key) + " must be a number.");
        ...
        bool NormalizeMutationDetailFields(const ObjectMutationContext &ctx,
                                           ...
                                           json *normalized) {
            const auto *objectSchema = objectTypeName.empty()
                                           ? nullptr
                                           : FindObjectSchema(ctx.snapshot,
                                                              std::string(objectTypeName));
        ...
        bool NormalizeObjectMutationArgs(const McpEditorSnapshot &snapshot,
            ...
            if (!NormalizeMutationRequiredFields(ctx, &normalized, &objectTypeName))
                return false;
            if (!NormalizeMutationCreateFromAssetFields(ctx, &normalized, &objectTypeName))
                return false;
        ...
        bool NormalizeWriteArguments(const McpEditorSnapshot &snapshot,
            ...
            if (toolName == "editor.create_object" ||
                ...
                return NormalizeObjectMutationArgs(snapshot, toolName, arguments, out, error);
            }
        ...
        json BuildDeleteAssetPreview(const McpEditorSnapshot &snapshot,
            ...
            if (const McpAssetSnapshot *asset = FindAssetById(snapshot, assetId)) {
                preview["asset"] = BuildAssetJson(*asset);
                preview["clearedObjectReferences"] = static_cast<size_t>(
                    std::ranges::count_if(snapshot.objects, [&](const McpObjectSnapshot &object) {
        ...
        size_t CountMatchingObjects(const McpEditorSnapshot &snapshot,
            ...
            return static_cast<size_t>(std::ranges::count_if(
                snapshot.objects, [&](const McpObjectSnapshot &object) {
                    return (typeFilter.empty() || ToLowerAscii(object.type) == typeFilter) &&
        ...
        bool TryHandleObjectReadOnlyToolCall(const McpEditorSnapshot &snapshot,
            ...
            if (name == "editor.search") {
                const json payloadOut =
                        SearchSnapshot(snapshot, arguments.value("query", ""),
            ...
            if (name == "editor.list_objects") {
                const json payloadOut =
                        BuildObjectListJson(snapshot, arguments.value("limit", 12),
            ...
            if (name == "editor.get_object_children") {
                ...
                const size_t limit = std::max<size_t>(
                    1, std::min<size_t>(arguments.value("limit", 12), 64));
                const json payloadOut = BuildObjectChildrenPayload(snapshot, objectId, limit);
                const json result = MakeSuccess(id, BuildTextToolResult(payloadOut));
                finish({"tool", name, method}, id, {true, 200, "", &result, {}});
                *response = MakeJsonResponse(200, "OK", result);
                return true;
            ...
            if (name == "editor.count_objects") {
                ...
                auto payloadOut =
                        json{{"count", count}, {"type", arguments.value("type", "")}, {"query", query}};
                const json result = MakeSuccess(id, BuildTextToolResult(payloadOut));
                finish({"tool", name, method}, id, {true, 200, "", &result, {}});
                *response = MakeJsonResponse(200, "OK", result);
                return true;
        ...
        bool TryHandleAssetReadOnlyToolCall(const McpEditorSnapshot &snapshot,
            ...
            if (name == "editor.list_assets") {
                const json payloadOut =
                        BuildAssetsCatalogJson(snapshot, arguments.value("limit", 12),
                                               arguments.value("query", ""));
                const json result = MakeSuccess(id, BuildTextToolResult(payloadOut));
                finish({"tool", name, method}, id, {true, 200, "", &result, {}});
                *response = MakeJsonResponse(200, "OK", result);
            ...
            if (name == "editor.get_asset") {
                ...
                payloadOut["objectReferenceCount"] = CountAssetObjectReferences(snapshot, assetId);
                const json result = MakeSuccess(id, BuildTextToolResult(payloadOut));
                finish({"tool", name, method}, id, {true, 200, "", &result, {}});
                *response = MakeJsonResponse(200, "OK", result);
                return true;
            ...
            if (name == "editor.search_assets") {
                const json payloadOut =
                        SearchAssetsSnapshot(snapshot, arguments.value("query", ""),
                                             arguments.value("limit", 8));
                const json result = MakeSuccess(id, BuildTextToolResult(payloadOut));
                finish({"tool", name, method}, id, {true, 200, "", &result, {}});
                *response = MakeJsonResponse(200, "OK", result);
        ...
        bool TryHandleSceneReadOnlyToolCall(const McpEditorSnapshot &snapshot,
            ...
            if (name == "editor.search_console") {
                const json payloadOut =
                        SearchConsoleSnapshot(snapshot, arguments.value("query", ""),
                                              arguments.value("limit", 8));
                const json result = MakeSuccess(id, BuildTextToolResult(payloadOut));
                finish({"tool", name, method}, id, {true, 200, "", &result, {}});
                *response = MakeJsonResponse(200, "OK", result);
            ...
            if (name == "editor.list_schema_types") {
                const auto payloadOut = BuildSchemaCatalogJson(snapshot, arguments.value("kind", ""));
                const json result = MakeSuccess(id, BuildTextToolResult(payloadOut));
                finish({"tool", name, method}, id, {true, 200, "", &result, {}});
                *response = MakeJsonResponse(200, "OK", result);
                return true;
        ...
        McpHttpResponse HandleWriteToolCall(const WriteToolCallInput &input,
                                            const CommandInvokerFn &commandInvoker,
                                            const StorePreviewTokenFn &storePreviewToken,
                                            const ConsumePreviewTokenFn &consumePreviewToken,
                                            const FinishFn &finish) {
            std::string mode;
            ...
            if (mode == "preview") {
                ...
                const json result = MakeSuccess(input.id, BuildTextToolResult(previewPayload));
                finish({"tool", input.name, input.method}, input.id,
                       {true, 200, "", &result, {}});
                return MakeJsonResponse(200, "OK", result);
            }
            ...
            if (IsDestructiveToolName(input.name)) {
                if (!input.arguments.contains("previewToken") ||
                    ...
                    const json result =
                            MakeError(input.id, -32602, "previewToken is required for apply mode.");
                    finish({"tool", input.name, input.method}, input.id,
                           {false, 200, "previewToken is required for apply mode.", &result, {}});
                    return MakeJsonResponse(200, "OK", result);
                }
                const std::string previewToken =
                        input.arguments["previewToken"].get<std::string>();
                std::string previewError;
                if (const json canonicalArguments = StripMutationControls(normalizedArguments);
                    !consumePreviewToken(previewToken, canonicalArguments, &previewError)) {
                    appendApplyAudit(
                        false,
                        BuildMutationPreviewPayload(input.snapshot, input.name, canonicalArguments),
                        previewError);
            ...
            if (!commandInvoker) {
                ...
                const json result = MakeError(input.id, -32002, "Command queue unavailable.");
                finish({"tool", input.name, input.method}, input.id,
        ...
        bool TryHandleTransportRequest(const McpHttpRequest &request,
            ...
            if (request.method == "GET") {
                ...
                finish({"http", "transport.info", ""}, nullptr,
                       {true, 200, "", &body, {}});
    ...
    McpHttpResponse McpProtocol::HandleHttp(const McpHttpRequest &request) const {
        ...
        if (McpHttpResponse preSnapshotResponse;
            TryHandleRpcMethodBeforeSnapshot(method, id, finish,
                                             &preSnapshotResponse)) {
        ...
        if (McpHttpResponse resourceResponse;
            TryHandleResourceRead(*snapshot, method, id, params, finish,
                                  &resourceResponse)) {
        ...
        if (method == "tools/call") {
            ...
            const auto storePreviewToken = [this, &name, &snapshot](
                const json &canonicalArguments) {
                ...
                m_previewRecords[token.str()] =
                        PreviewRecord{
                            name, canonicalArguments, snapshot->sceneId,
                            snapshot->sceneFilePath
                        };
                return token.str();
            };
