...
vec3 CalcLight(Light L, vec3 pos, vec3 N, vec3 V, vec3 albedo, float rough, float metal)
{
    ...
    if (L.type == 0) {
        ...
        L_dir = toLight / d;
        // Smooth inverse-square falloff — no hard edge at radius boundary
        float dOverR = d / L.radius;
        float window = max(0.0, 1.0 - dOverR * dOverR * dOverR * dOverR);
        attenuation  = (window * window) / (d * d + 1.0);
    }
    ...
    return attenuation * L.color * L.color * (diff * albedo + spec * specColor);
}
void main() {
    ...
    if (u_hasNormalMap != 0) {
        // HORO-67 plumbing: TBN deferred until per-vertex tangents exist.
