...
namespace Monolith {
    namespace {
        ...
        std::unique_ptr<IRenderBackend> g_ownedBackend = CreateDefaultOwnedBackend(); // NOSONAR: cpp:S5421 backend ownership must remain mutable for backend switching/reset
        // NOSONAR: cpp:S1966 backend must be mutable
        IRenderBackend *g_backend = g_ownedBackend.get(); // NOSONAR: cpp:S1966 backend pointer must be reassignable
    ...
    void Renderer::SubmitWireframe(const Mesh &mesh, const Mat4 &model,
                                   const Shader &shader, float r, float g, float b) {
