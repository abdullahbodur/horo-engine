...
std::array<DebugHUD::GlyphVertex, DebugHUD::MAX_GLYPHS * 6>
    DebugHUD::s_glyphBuf = {};
...
void DebugHUD::BuildFontAtlas() {
  ...
  for (int ch = 0; ch < 128; ++ch) {
    for (int row = 0; row < 8; ++row) {
      const auto bits =
          std::byte{FONT_DATA[static_cast<std::size_t>(ch * 8 + row)]};
      for (int col = 0; col < 8; ++col) {
        const bool set = (std::to_integer<int>(bits >> (7 - col)) & 1) != 0;
        pixels[static_cast<std::size_t>(row * ATLAS_W + ch * 8 + col)] =
            set ? 0xFF : 0x00;
      }
    }
  }
...
void DebugHUD::Render() {
  ...
  if (s_visible) {
    ...
    y += LINE;
    {
      ...
      y += LINE;
      const std::string frameBuf =
          std::format("FRAME: {:.2f} ms", s_smoothFrameMs);
    ...
    y += LINE;
    {
      DrawText(std::format("POS  : {:.2f}  {:.2f}  {:.2f}", s_stats.posX,
                           s_stats.posY, s_stats.posZ)
                   .c_str(),
               X0, y, 1.0f, 1.0f, 1.0f, SCALE);
      y += LINE;
      DrawText(std::format("VELY : {:.2f}", s_stats.velY).c_str(), X0, y, 1.0f,
               1.0f, 1.0f, SCALE);
      y += LINE;
      DrawText(
          std::format("GND  : {}", s_stats.onGround ? "YES" : "NO").c_str(), X0,
          y, 1.0f, 1.0f, 1.0f, SCALE);
      y += LINE;
      DrawText(std::format("SPD  : {:.2f}", s_stats.speed).c_str(), X0, y, 1.0f,
               1.0f, 1.0f, SCALE);
      y += LINE;
    }
    y += 4.0f;
    ...
    y += LINE;
    {
      if (!s_stats.sceneCameraOn) {
        ...
        y += LINE;
        const std::string pitchBuf =
            std::format("PITCH: {:.1f}", s_stats.camPitch);
    ...
    {
      DrawText(
          std::format("BLOCKS   : {}", s_stats.collisionBlockCount).c_str(), X0,
          y, 1.0f, 1.0f, 1.0f, SCALE);
      y += LINE;
      DrawText(std::format("ENTITIES : {}", s_stats.entityCount).c_str(), X0, y,
               1.0f, 1.0f, 1.0f, SCALE);
      y += LINE;
      DrawText(std::format("DRAWCALLS: {}", s_stats.drawCallCount).c_str(), X0,
               y, 1.0f, 1.0f, 1.0f, SCALE);
      y += LINE;
    }
    y += 4.0f;
    ...
    // --- Toggle hints ---
    {
      ...
      DrawText(std::format("[F4] BOXES: {}  [F5] SETTINGS",
                           s_showCollisionBoxes ? "ON" : "OFF")
                   .c_str(),
               X0, y, 0.6f, 0.6f, 0.6f, SCALE);
#else
      DrawText(
          std::format("[F4] BOXES: {}", s_showCollisionBoxes ? "ON" : "OFF")
              .c_str(),
          X0, y, 0.6f, 0.6f, 0.6f, SCALE);
#endif
    }
  ...
  if (s_settingsOpen) {
    ...
    py += LINE;
    DrawText(
        std::format("[L] LABELS  : {}", s_labelsVisible ? "ON" : "OFF").c_str(),
        px, py, 1.0f, 1.0f, 1.0f, SCALE);
    py += LINE;
    DrawText(std::format("[O] OCCLUDE : {}", s_occlusionCulling ? "ON" : "OFF")
                 .c_str(),
             px, py, 1.0f, 1.0f, 1.0f, SCALE);
    py += LINE;
...
void DebugHUD::RenderWorldLabels() {
  ...
  if (s_occlusionCulling &&
    ...
    if (std::string readbackError; !Renderer::ReadbackDepth32F(
            s_screenW, s_screenH, s_depthBuf, &readbackError)) {
...
void DebugHUD::DrawLabelsWithDepthCull() {
  ...
  for (int i = 0; i < s_labelCount; ++i) {
    ...
    const int py = std::clamp(s_screenH - 1 - static_cast<int>(s_labels[i].y),
                              0, s_screenH - 1);
    const float bufDepth =
        s_depthBuf[static_cast<std::vector<float>::size_type>(py * s_screenW +
                                                              px)];
    if (const float labelDepth = (s_labels[i].ndcZ + 1.0f) * 0.5f;
        labelDepth > bufDepth + 0.005f)
      continue;
    DrawText(s_labels[i].text.c_str(), s_labels[i].x, s_labels[i].y, 0.2f, 1.0f,
             0.4f, 1.5f);
  }
}
#endif
