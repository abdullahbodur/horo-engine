...
// to avoid collision
#ifdef DrawText
#undef DrawText // NOSONAR: cpp:S959 DrawText macro originates in platform headers; local undef avoids symbol collision in this TU
#endif
...
namespace Monolith {
    ...
    std::array<DebugHUD::WorldLabel, DebugHUD::MAX_LABELS> DebugHUD::s_labels = {};
    ...
    std::array<DebugHUD::GlyphVertex, DebugHUD::MAX_GLYPHS * 6> DebugHUD::s_glyphBuf = {};
...
void main() {
    ...
    static const char *const HUD_FRAG = R"glsl(
#version 410 core
in vec2 v_uv;
in vec4 v_color;
...
void main() {
    ...
    void DebugHUD::Init(int screenW, int screenH) {
        ...
        glVertexAttribPointer(1, 2, GL_FLOAT, GL_FALSE, sizeof(GlyphVertex),
                              static_cast<const void *>(static_cast<const std::byte *>(nullptr) + 2 * sizeof(float)));
        // a_color (location 2): 4 floats
        glEnableVertexAttribArray(2);
        glVertexAttribPointer(2, 4, GL_FLOAT, GL_FALSE, sizeof(GlyphVertex),
                              static_cast<const void *>(static_cast<const std::byte *>(nullptr) + 4 * sizeof(float)));
    ...
    void DebugHUD::BuildFontAtlas() {
        ...
        for (int ch = 0; ch < 128; ++ch) {
            for (int row = 0; row < 8; ++row) {
                const auto bits = std::byte{FONT_DATA[static_cast<std::size_t>(ch * 8 + row)]};
                for (int col = 0; col < 8; ++col) {
                    const bool set = (std::to_integer<int>(bits >> (7 - col)) & 1) != 0;
                    pixels[static_cast<std::size_t>(row * ATLAS_W + ch * 8 + col)] = set ? 0xFF : 0x00;
                }
            }
        }
    ...
    void DebugHUD::Render() {
        ...
        if (s_visible) {
            ...
            y += LINE;
            {
                const std::string fpsBuf = std::format("FPS  : {:.1f}", s_smoothFps);
                DrawText(fpsBuf.c_str(), X0, y, 1.0f, 1.0f, 1.0f, SCALE);
                y += LINE;
                const std::string frameBuf = std::format("FRAME: {:.2f} ms", s_smoothFrameMs);
                DrawText(frameBuf.c_str(), X0, y, 1.0f, 1.0f, 1.0f, SCALE);
                y += LINE;
            }
            y += 4.0f;
            ...
            y += LINE;
            {
                DrawText(std::format("POS  : {:.2f}  {:.2f}  {:.2f}", s_stats.posX,
                                     s_stats.posY, s_stats.posZ).c_str(),
                         X0, y, 1.0f, 1.0f, 1.0f, SCALE);
                y += LINE;
                DrawText(std::format("VELY : {:.2f}", s_stats.velY).c_str(),
                         X0, y, 1.0f, 1.0f, 1.0f, SCALE);
                y += LINE;
                DrawText(std::format("GND  : {}", s_stats.onGround ? "YES" : "NO").c_str(),
                         X0, y, 1.0f, 1.0f, 1.0f, SCALE);
                y += LINE;
                DrawText(std::format("SPD  : {:.2f}", s_stats.speed).c_str(),
                         X0, y, 1.0f, 1.0f, 1.0f, SCALE);
                y += LINE;
            }
            y += 4.0f;
            ...
            y += LINE;
            {
                if (!s_stats.sceneCameraOn) {
                    ...
                    const std::string yawBuf = std::format("YAW  : {:.1f}", s_stats.camYaw);
                    DrawText(yawBuf.c_str(), X0, y, 1.0f, 1.0f, 1.0f, SCALE);
                    y += LINE;
                    const std::string pitchBuf = std::format("PITCH: {:.1f}", s_stats.camPitch);
                    DrawText(pitchBuf.c_str(), X0, y, 1.0f, 1.0f, 1.0f, SCALE);
                    y += LINE;
                }
            }
            y += 4.0f;
            ...
            y += LINE;
            {
                DrawText(std::format("BLOCKS   : {}", s_stats.collisionBlockCount).c_str(),
                         X0, y, 1.0f, 1.0f, 1.0f, SCALE);
                y += LINE;
                DrawText(std::format("ENTITIES : {}", s_stats.entityCount).c_str(),
                         X0, y, 1.0f, 1.0f, 1.0f, SCALE);
                y += LINE;
                DrawText(std::format("DRAWCALLS: {}", s_stats.drawCallCount).c_str(),
                         X0, y, 1.0f, 1.0f, 1.0f, SCALE);
                y += LINE;
            }
            y += 4.0f;
            ...
            {
#ifndef NDEBUG
                DrawText(std::format("[F4] BOXES: {}  [F5] SETTINGS",
                                     s_showCollisionBoxes ? "ON" : "OFF").c_str(),
                         X0, y, 0.6f, 0.6f, 0.6f, SCALE);
#else
                DrawText(std::format("[F4] BOXES: {}",
                                     s_showCollisionBoxes ? "ON" : "OFF").c_str(),
                         X0, y, 0.6f, 0.6f, 0.6f, SCALE);
#endif
            }
        ...
        if (s_settingsOpen) {
            ...
            py += LINE;
            DrawText(std::format("[L] LABELS  : {}",
                                 s_labelsVisible ? "ON" : "OFF").c_str(),
                     px, py, 1.0f, 1.0f, 1.0f, SCALE);
            py += LINE;
            DrawText(std::format("[O] OCCLUDE : {}",
                                 s_occlusionCulling ? "ON" : "OFF").c_str(),
                     px, py, 1.0f, 1.0f, 1.0f, SCALE);
            py += LINE;
...
#ifndef NDEBUG
    void DebugHUD::RenderWorldLabels() {
        if (s_labelCount <= 0)
            return;

        if (s_occlusionCulling &&
            Renderer::GetBackendCapabilities().supportsDepthReadback) {
            if (std::string readbackError;
                !Renderer::ReadbackDepth32F(s_screenW, s_screenH, s_depthBuf,
                                            &readbackError)) {
                LogWarn("DebugHUD: depth readback failed ({}). Rendering labels "
                        "without occlusion.",
                        readbackError);
                s_depthBuf.clear();
            }
            DrawLabelsWithDepthCull();
        } else {
            for (int i = 0; i < s_labelCount; ++i)
                DrawText(s_labels[i].text.c_str(), s_labels[i].x, s_labels[i].y, 0.2f,
                         1.0f, 0.4f, 1.5f);
        }
    }

    void DebugHUD::DrawLabelsWithDepthCull() {
        if (s_depthBuf.empty()) {
            for (int i = 0; i < s_labelCount; ++i)
                DrawText(s_labels[i].text.c_str(), s_labels[i].x, s_labels[i].y, 0.2f,
                         1.0f, 0.4f, 1.5f);
        ...
        for (int i = 0; i < s_labelCount; ++i) {
            const int px =
                    std::clamp(static_cast<int>(s_labels[i].x), 0, s_screenW - 1);
            const int py =
                    std::clamp(s_screenH - 1 - static_cast<int>(s_labels[i].y),
                               0, s_screenH - 1);
            const float bufDepth =
                    s_depthBuf[static_cast<std::vector<float>::size_type>(
                        py * s_screenW + px)];
            if (const float labelDepth = (s_labels[i].ndcZ + 1.0f) * 0.5f;
                labelDepth > bufDepth + 0.005f)
                continue;
            DrawText(s_labels[i].text.c_str(), s_labels[i].x, s_labels[i].y, 0.2f,
                     1.0f, 0.4f, 1.5f);
        }
