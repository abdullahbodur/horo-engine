...
namespace Monolith {
    ...
    namespace {
        ...
        bool TryCreateShaderModule(VkDevice device, const std::vector<char> &spirvBytes,
                                   VkShaderModule *outShaderModule,
                                   std::string &outError, const char *context) {
            if (!outShaderModule)
                return false;
            *outShaderModule = VK_NULL_HANDLE;
            ...
            if ((spirvBytes.size() % sizeof(uint32_t)) != 0) {
                outError = std::string(context) +
                           " failed: SPIR-V bytecode size is not a multiple of 4.";
                return false;
            }
            ...
            return CheckVk(
                vkCreateShaderModule(device, &shaderModuleInfo, nullptr, outShaderModule),
                outError, context);
        }
        ...
        bool CreateHostVisibleBuffer(VkPhysicalDevice physicalDevice, VkDevice device,
            ...
            *outMemory = VK_NULL_HANDLE;
            ...
            if (!CheckVk(vkCreateBuffer(device, &bufferInfo, nullptr, outBuffer),
                         outError, context))
                return false;
            ...
            const uint32_t memoryTypeIndex =
                    FindMemoryTypeIndex(physicalDevice, memoryRequirements.memoryTypeBits,
                                        VK_MEMORY_PROPERTY_HOST_VISIBLE_BIT |
                                        VK_MEMORY_PROPERTY_HOST_COHERENT_BIT);
            if (memoryTypeIndex == kInvalidQueueFamily) {
                outError =
                        std::string(context) + " failed: no host-visible coherent memory type.";
            ...
            if (!CheckVk(vkAllocateMemory(device, &allocInfo, nullptr, outMemory),
                         outError, context)) {
                vkDestroyBuffer(device, *outBuffer, nullptr);
                *outBuffer = VK_NULL_HANDLE;
                return false;
            ...
            if (!CheckVk(vkBindBufferMemory(device, *outBuffer, *outMemory, 0), outError,
                         context)) {
        ...
        bool HasFormatFeature(VkPhysicalDevice physicalDevice, VkFormat format,
        ...
        bool FindSupportedDepthFormat(VkPhysicalDevice physicalDevice,
                                      bool requireTransferSrc, VkFormat *outFormat) {
            if (!outFormat)
                return false;
            *outFormat = VK_FORMAT_UNDEFINED;
            ...
            for (VkFormat candidate: candidates) {
                if (!HasFormatFeature(physicalDevice, candidate,
                                      VK_FORMAT_FEATURE_DEPTH_STENCIL_ATTACHMENT_BIT)) {
                    continue;
                }

                if (requireTransferSrc &&
                    !HasFormatFeature(physicalDevice, candidate,
        ...
        constexpr std::array<uint32_t, kFallbackOpaqueIndexCount>
        kFallbackOpaqueIndices = {0u, 1u, 2u};
    ...
    namespace {
        ...
        uint8_t
        PackOpaquePipelineKey(const VulkanRenderBackend::OpaquePipelineKey &key) {
            uint8_t packed = 0u;
    ...
    VulkanRenderBackend::TranslatedMaterialState
    VulkanRenderBackend::TranslateMaterialState(const Material &material) {
    ...
    VulkanRenderBackend::OpaquePipelineKey
    VulkanRenderBackend::BuildOpaquePipelineKey(
    ...
    bool VulkanRenderBackend::TryGetImGuiVulkanInitData(
        void **outInstance, void **outPhysicalDevice, void **outDevice,
        uint32_t *outQueueFamily, void **outQueue, void **outRenderPass,
    ...
    void *VulkanRenderBackend::GetActiveCommandBufferHandle() const {
        ...
        return reinterpret_cast<void *>(
            m_context->commandBuffers[m_context->activeImageIndex]);
    }

    void VulkanRenderBackend::QueueOverlayRenderCallback(
        OverlayRenderCallback callback, void *userData) {
        if (!m_frameActive) {
            m_pendingOverlayRenderCallback = nullptr;
            m_pendingOverlayRenderUserData = nullptr;
            m_lastError = "Vulkan overlay render callbacks must be queued during an "
                    "active frame.";
            return;
        }
    ...
    bool VulkanRenderBackend::ReadbackColorBgr8(int width, int height,
                                                std::vector<uint8_t> &outPixels,
                                                std::string *outError) {
        if (!m_context || !m_context->supportsColorReadback ||
            m_context->colorReadbackBuffer == VK_NULL_HANDLE) {
            if (outError)
                *outError = "Vulkan color readback is unavailable (swapchain "
                        "transfer-source usage not supported).";
            return false;
        }
        if (m_frameActive) {
            if (outError)
                *outError = "Vulkan color readback must be requested outside active "
        ...
        if (static_cast<uint32_t>(width) != m_context->swapchainExtent.width ||
            ...
            if (outError) {
                *outError =
                        "Vulkan color readback dimensions must match swapchain extent (" +
        ...
        if (!m_context->hasColorReadbackData) {
            if (outError)
                *outError = "Vulkan color readback buffer has no captured frame yet. "
                        "Render at least one frame first.";
            return false;
        }

        if (!CheckVk(vkWaitForFences(m_context->device, 1, &m_context->inFlightFence,
                                     VK_TRUE, UINT64_MAX),
                     m_lastError, "vkWaitForFences(colorReadback)")) {
            if (outError)
        ...
        const size_t pixelCount =
                static_cast<size_t>(m_context->swapchainExtent.width) *
                m_context->swapchainExtent.height;
        void *mapped = nullptr;
        if (!CheckVk(vkMapMemory(m_context->device, m_context->colorReadbackMemory, 0,
                                 m_context->colorReadbackSize, 0, &mapped),
                     m_lastError, "vkMapMemory(colorReadback)")) {
            if (outError)
                *outError = m_lastError;
            return false;
        }
        ...
        const bool formatIsBgra =
                m_context->swapchainFormat == VK_FORMAT_B8G8R8A8_UNORM ||
        ...
        for (uint32_t y = 0; y < m_context->swapchainExtent.height; ++y) {
            ...
            for (uint32_t x = 0; x < m_context->swapchainExtent.width; ++x) {
                const size_t srcIndex =
                        (static_cast<size_t>(srcY) * m_context->swapchainExtent.width + x) *
                        4u;
                const size_t dstIndex =
                        (static_cast<size_t>(y) * m_context->swapchainExtent.width + x) * 3u;
                if (formatIsBgra) {
    ...
    bool VulkanRenderBackend::ReadbackDepth32F(int width, int height,
                                               std::vector<float> &outDepth,
                                               std::string *outError) {
        if (!m_context || !m_context->supportsDepthReadback ||
            m_context->depthReadbackBuffer == VK_NULL_HANDLE) {
            if (outError) {
                *outError = "Vulkan depth readback is unavailable (depth transfer-source "
                        "format support not available).";
            }
            return false;
        }
        if (m_frameActive) {
            if (outError)
                *outError = "Vulkan depth readback must be requested outside active "
                        "BeginFrame/EndFrame scope.";
            return false;
        }
        if (width <= 0 || height <= 0) {
        ...
        if (static_cast<uint32_t>(width) != m_context->swapchainExtent.width ||
            ...
            if (outError) {
                *outError =
                        "Vulkan depth readback dimensions must match swapchain extent (" +
                        std::to_string(m_context->swapchainExtent.width) + "x" +
                        std::to_string(m_context->swapchainExtent.height) + ").";
            }
            return false;
        ...
        if (!m_context->hasDepthReadbackData) {
            if (outError)
                *outError = "Vulkan depth readback buffer has no captured frame yet. "
                        "Render at least one frame first.";
            return false;
        }
        ...
        if (!CheckVk(vkWaitForFences(m_context->device, 1, &m_context->inFlightFence,
                                     VK_TRUE, UINT64_MAX),
                     m_lastError, "vkWaitForFences(depthReadback)")) {
            if (outError)
                *outError = m_lastError;
            return false;
        }
        ...
        const size_t pixelCount =
                ...
                m_context->swapchainExtent.height;
        void *mapped = nullptr;
        if (!CheckVk(vkMapMemory(m_context->device, m_context->depthReadbackMemory, 0,
                                 m_context->depthReadbackSize, 0, &mapped),
                     m_lastError, "vkMapMemory(depthReadback)")) {
            if (outError)
                *outError = m_lastError;
            return false;
        }
        ...
        for (uint32_t y = 0; y < m_context->swapchainExtent.height; ++y) {
            ...
            for (uint32_t x = 0; x < m_context->swapchainExtent.width; ++x) {
                const size_t srcIndex =
                        static_cast<size_t>(srcY) * m_context->swapchainExtent.width + x;
                const size_t dstIndex =
                        static_cast<size_t>(y) * m_context->swapchainExtent.width + x;
    ...
    bool VulkanRenderBackend::EnsureEditorViewportRenderTarget(
        uint32_t width, uint32_t height, std::string *outError) {
        const bool ok =
                EnsureOffscreenRenderTarget(kEditorViewportTargetKey, width, height);
        if (!ok && outError)
            *outError = m_lastError;
        return ok;
    }

    bool VulkanRenderBackend::TryGetEditorViewportRenderTargetHandle(
        RenderTargetHandle *outHandle, bool needsYFlip, std::string *outError) {
        const bool ok = TryGetOffscreenRenderTargetHandle(kEditorViewportTargetKey,
                                                          outHandle, needsYFlip);
        if (!ok && outError)
            *outError = m_lastError;
        return ok;
    }

    bool VulkanRenderBackend::EnsureOffscreenRenderTarget(
        const std::string &targetKey, uint32_t width, uint32_t height) {
        if (!m_context || m_context->device == VK_NULL_HANDLE ||
            m_context->graphicsQueue == VK_NULL_HANDLE) {
            m_lastError =
                    "Cannot create Vulkan offscreen target before backend initialization.";
            return false;
        }
        if (targetKey.empty()) {
        ...
        if (it != m_context->offscreenTargets.end() && it->second.width == width &&
            it->second.height == height && it->second.readyForSampling) {
            return true;
        }
        ...
        return CreateOffscreenRenderTargetResources(targetKey, width, height,
                                                    previousGeneration);
    }

    bool VulkanRenderBackend::TryGetOffscreenRenderTargetHandle(
        const std::string &targetKey, RenderTargetHandle *outHandle,
        bool needsYFlip) {
        if (!outHandle)
            return false;
        *outHandle = {};
        if (!m_context) {
            m_lastError = "Vulkan offscreen target handle request requires an "
                    "initialized backend.";
            return false;
        }
        ...
        if (it == m_context->offscreenTargets.end()) {
            m_lastError = "Vulkan offscreen target handle request references an "
                          "unknown target key: " +
                          targetKey;
            return false;
        }

        return TryRegisterOffscreenTargetImGuiDescriptor(targetKey, outHandle,
                                                         needsYFlip);
    }

    bool VulkanRenderBackend::TryGetOffscreenRenderTargetMetadata(
    ...
    void VulkanRenderBackend::DestroyOffscreenRenderTarget(
    ...
    bool VulkanRenderBackend::EnsureOffscreenRenderPass() {
        ...
        return CheckVk(vkCreateRenderPass(m_context->device, &renderPassInfo, nullptr,
                                          &m_context->offscreenRenderPass),
                       m_lastError, "vkCreateRenderPass(offscreen)");
    }
    ...
    void VulkanRenderBackend::DestroyOffscreenRenderPass() {
        if (!m_context || m_context->device == VK_NULL_HANDLE)
            return;
        if (m_context->offscreenRenderPass != VK_NULL_HANDLE) {
            vkDestroyRenderPass(m_context->device, m_context->offscreenRenderPass,
                                nullptr);
            m_context->offscreenRenderPass = VK_NULL_HANDLE;
        }
    }

    bool VulkanRenderBackend::CreateOffscreenRenderTargetResources(
        const std::string &targetKey, uint32_t width, uint32_t height,
        ...
        if (!CheckVk(
            vkCreateImage(m_context->device, &imageInfo, nullptr, &target.image),
            m_lastError, "vkCreateImage(offscreen)")) {
            return false;
        }

        VkMemoryRequirements imageRequirements{};
        vkGetImageMemoryRequirements(m_context->device, target.image,
                                     &imageRequirements);
        const uint32_t memoryTypeIndex = FindMemoryTypeIndex(
            m_context->physicalDevice, imageRequirements.memoryTypeBits,
            VK_MEMORY_PROPERTY_DEVICE_LOCAL_BIT);
        if (memoryTypeIndex == kInvalidQueueFamily) {
            m_lastError = "No Vulkan device-local memory type available for offscreen "
                    "render target.";
        ...
        if (!CheckVk(vkAllocateMemory(m_context->device, &memoryInfo, nullptr,
                                      &target.memory),
                     m_lastError, "vkAllocateMemory(offscreen)")) {
            vkDestroyImage(m_context->device, target.image, nullptr);
            return false;
        }
        if (!CheckVk(
            vkBindImageMemory(m_context->device, target.image, target.memory, 0),
            m_lastError, "vkBindImageMemory(offscreen)")) {
        ...
        if (!CheckVk(vkCreateImageView(m_context->device, &imageViewInfo, nullptr,
                                       &target.imageView),
                     m_lastError, "vkCreateImageView(offscreen)")) {
            vkFreeMemory(m_context->device, target.memory, nullptr);
            vkDestroyImage(m_context->device, target.image, nullptr);
            return false;
        }
        ...
        if (!CheckVk(vkCreateSampler(m_context->device, &samplerInfo, nullptr,
                                     &target.sampler),
                     m_lastError, "vkCreateSampler(offscreen)")) {
        ...
        if (!CheckVk(vkCreateFramebuffer(m_context->device, &framebufferInfo, nullptr,
                                         &target.framebuffer),
                     m_lastError, "vkCreateFramebuffer(offscreen)")) {
        ...
        if (!CheckVk(vkAllocateCommandBuffers(m_context->device, &commandBufferInfo,
                                              &commandBuffer),
                     m_lastError, "vkAllocateCommandBuffers(offscreen)")) {
        ...
        if (!CheckVk(vkEndCommandBuffer(commandBuffer), m_lastError,
                     "vkEndCommandBuffer(offscreen)")) {
            vkFreeCommandBuffers(m_context->device, m_context->commandPool, 1,
                                 &commandBuffer);
            vkDestroyFramebuffer(m_context->device, target.framebuffer, nullptr);
            vkDestroySampler(m_context->device, target.sampler, nullptr);
        ...
        if (!CheckVk(vkCreateFence(m_context->device, &fenceInfo, nullptr, &fence),
                     m_lastError, "vkCreateFence(offscreen)")) {
            vkFreeCommandBuffers(m_context->device, m_context->commandPool, 1,
                                 &commandBuffer);
            vkDestroyFramebuffer(m_context->device, target.framebuffer, nullptr);
            vkDestroySampler(m_context->device, target.sampler, nullptr);
        ...
        if (!CheckVk(vkQueueSubmit(m_context->graphicsQueue, 1, &submitInfo, fence),
                     m_lastError, "vkQueueSubmit(offscreen)")) {
        ...
        if (!CheckVk(
            ...
            m_lastError, "vkWaitForFences(offscreen)")) {
            vkDestroyFence(m_context->device, fence, nullptr);
            vkFreeCommandBuffers(m_context->device, m_context->commandPool, 1,
                                 &commandBuffer);
            vkDestroyFramebuffer(m_context->device, target.framebuffer, nullptr);
    ...
    void VulkanRenderBackend::DestroyOffscreenRenderTargetResources(
        const std::string &targetKey) {
        if (!m_context || m_context->device == VK_NULL_HANDLE)
            return;
    ...
    bool VulkanRenderBackend::TryRegisterOffscreenTargetImGuiDescriptor(
        const std::string &targetKey, RenderTargetHandle *outHandle,
        bool needsYFlip) {
        ...
        Context::OffscreenRenderTarget &target = it->second;
        if (!target.readyForSampling || target.imageView == VK_NULL_HANDLE ||
            target.sampler == VK_NULL_HANDLE) {
            m_lastError =
                    "Vulkan offscreen target is not ready for sampling: " + targetKey;
            return false;
        }

        if (target.imguiDescriptorSet == VK_NULL_HANDLE) {
            if (!IsImGuiVulkanTextureApiReady()) {
                m_lastError = "ImGui Vulkan texture registration is not ready for "
                              "offscreen target: " +
                              targetKey;
                return false;
            }
            target.imguiDescriptorSet =
                    ImGui_ImplVulkan_AddTexture(target.sampler, target.imageView,
                                                VK_IMAGE_LAYOUT_SHADER_READ_ONLY_OPTIMAL);
            if (target.imguiDescriptorSet == VK_NULL_HANDLE) {
                m_lastError = "Failed to register Vulkan offscreen target with ImGui "
                              "texture API: " +
                              targetKey;
                return false;
            }
    ...
    bool VulkanRenderBackend::Initialize(void *nativeWindowHandle) {
        ...
        if (!CheckVk(vkCreateInstance(&instanceInfo, nullptr, &m_context->instance),
                     m_lastError, "vkCreateInstance")) {
            Shutdown();
            return false;
        }
        ...
        if (!CheckVk(glfwCreateWindowSurface(m_context->instance, m_context->window,
                                             nullptr, &m_context->surface),
                     m_lastError, "glfwCreateWindowSurface")) {
            Shutdown();
            return false;
        }

        uint32_t physicalDeviceCount = 0;
        if (!CheckVk(vkEnumeratePhysicalDevices(m_context->instance,
                                                &physicalDeviceCount, nullptr),
                     m_lastError, "vkEnumeratePhysicalDevices(count)")) {
            Shutdown();
            return false;
        }
        ...
        if (!CheckVk(vkEnumeratePhysicalDevices(m_context->instance,
                     ...
                     m_lastError, "vkEnumeratePhysicalDevices")) {
            Shutdown();
            return false;
        }
        ...
        for (VkPhysicalDevice device: physicalDevices) {
            ...
            for (uint32_t i = 0; i < queueFamilyCount; ++i) {
                ...
                vkGetPhysicalDeviceSurfaceSupportKHR(device, i, m_context->surface,
                                                     &presentSupported);
            ...
            vkEnumerateDeviceExtensionProperties(device, nullptr,
                                                 &extensionCountAvailable, nullptr);
            std::vector<VkExtensionProperties> extensions(extensionCountAvailable);
            vkEnumerateDeviceExtensionProperties(
                device, nullptr, &extensionCountAvailable, extensions.data());
            ...
            vkGetPhysicalDeviceSurfaceFormatsKHR(device, m_context->surface,
        ...
        if (m_context->physicalDevice == VK_NULL_HANDLE) {
            m_lastError = "No Vulkan physical device supports graphics, present, and "
        ...
        if (!CheckVk(vkCreateDevice(m_context->physicalDevice, &deviceInfo, nullptr,
                                    &m_context->device),
                     m_lastError, "vkCreateDevice")) {
            Shutdown();
            return false;
        }

        volkLoadDevice(m_context->device);
        vkGetDeviceQueue(m_context->device, m_context->graphicsQueueFamily, 0,
                         &m_context->graphicsQueue);
        vkGetDeviceQueue(m_context->device, m_context->presentQueueFamily, 0,
        ...
        if (!CheckVk(vkCreateCommandPool(m_context->device, &commandPoolInfo, nullptr,
                     ...
                     m_lastError, "vkCreateCommandPool")) {
            Shutdown();
            return false;
        }
        ...
        if (!CheckVk(vkCreateSemaphore(m_context->device, &semaphoreInfo, nullptr,
                     ...
                     m_lastError, "vkCreateSemaphore(imageAvailable)")) {
            Shutdown();
            return false;
        }
        if (!CheckVk(vkCreateSemaphore(m_context->device, &semaphoreInfo, nullptr,
                                       &m_context->renderFinishedSemaphore),
                     m_lastError, "vkCreateSemaphore(renderFinished)")) {
            Shutdown();
            return false;
        }
        ...
        if (!CheckVk(vkCreateFence(m_context->device, &fenceInfo, nullptr,
                                   &m_context->inFlightFence),
                     m_lastError, "vkCreateFence")) {
            Shutdown();
            return false;
        }
        ...
        if (!CheckVk(vkCreatePipelineLayout(m_context->device, &pipelineLayoutInfo,
                                            nullptr,
                                            &m_context->opaquePipelineLayout),
                     m_lastError, "vkCreatePipelineLayout")) {
            Shutdown();
            return false;
        }
    ...
    void VulkanRenderBackend::Shutdown() {
        ...
        if (m_context->opaquePipelineLayout != VK_NULL_HANDLE)
            vkDestroyPipelineLayout(m_context->device, m_context->opaquePipelineLayout,
                                    nullptr);
        ...
        if (m_context->renderFinishedSemaphore != VK_NULL_HANDLE)
            vkDestroySemaphore(m_context->device, m_context->renderFinishedSemaphore,
                               nullptr);
        if (m_context->imageAvailableSemaphore != VK_NULL_HANDLE)
            vkDestroySemaphore(m_context->device, m_context->imageAvailableSemaphore,
                               nullptr);
    ...
    void VulkanRenderBackend::DestroySwapchain() {
        ...
        if (!m_context->commandBuffers.empty()) {
            vkFreeCommandBuffers(
                m_context->device, m_context->commandPool,
    ...
    bool VulkanRenderBackend::CreateDepthResources() {
        ...
        VkFormat depthFormat = VK_FORMAT_UNDEFINED;
        if (!FindSupportedDepthFormat(m_context->physicalDevice, false,
                                      &depthFormat)) {
            m_lastError = "Failed to find a Vulkan depth format with depth-stencil "
                    "attachment support.";
            return false;
        }
        const bool depthTransferSrcSupported =
                HasFormatFeature(m_context->physicalDevice, depthFormat,
                                 VK_FORMAT_FEATURE_TRANSFER_SRC_BIT);
        ...
        const VkImageCreateInfo depthImageInfo{
            ...
            .extent = {
                ...
                m_context->swapchainExtent.height, 1
            },
            .mipLevels = 1,
            .arrayLayers = 1,
            .samples = VK_SAMPLE_COUNT_1_BIT,
        ...
        if (!CheckVk(vkCreateImage(m_context->device, &depthImageInfo, nullptr,
                                   &m_context->depthImage),
                     m_lastError, "vkCreateImage(depth)")) {
            return false;
        }

        VkMemoryRequirements memoryRequirements{};
        vkGetImageMemoryRequirements(m_context->device, m_context->depthImage,
                                     &memoryRequirements);
        const uint32_t memoryTypeIndex = FindMemoryTypeIndex(
            m_context->physicalDevice, memoryRequirements.memoryTypeBits,
            VK_MEMORY_PROPERTY_DEVICE_LOCAL_BIT);
        if (memoryTypeIndex == kInvalidQueueFamily) {
            m_lastError =
                    "Failed to find Vulkan device-local memory for depth image allocation.";
            return false;
        }
        ...
        if (!CheckVk(vkAllocateMemory(m_context->device, &allocInfo, nullptr,
                                      &m_context->depthImageMemory),
                     m_lastError, "vkAllocateMemory(depth)")) {
            return false;
        }

        if (!CheckVk(vkBindImageMemory(m_context->device, m_context->depthImage,
                                       m_context->depthImageMemory, 0),
                     m_lastError, "vkBindImageMemory(depth)")) {
            return false;
        }
        ...
        if (!CheckVk(vkCreateImageView(m_context->device, &depthViewInfo, nullptr,
                                       &m_context->depthImageView),
                     m_lastError, "vkCreateImageView(depth)")) {
            return false;
        }
    ...
    bool VulkanRenderBackend::EnsureReadbackBuffers() {
        ...
        const VkDeviceSize colorBytes =
                static_cast<VkDeviceSize>(m_context->swapchainExtent.width) *
                m_context->swapchainExtent.height * 4u;
        if (m_context->supportsColorReadback) {
            if (!CreateHostVisibleBuffer(m_context->physicalDevice, m_context->device,
                                         colorBytes, VK_BUFFER_USAGE_TRANSFER_DST_BIT,
                                         &m_context->colorReadbackBuffer,
                                         &m_context->colorReadbackMemory, m_lastError,
        ...
        if (m_context->supportsDepthReadback) {
            const VkDeviceSize depthBytes =
                    static_cast<VkDeviceSize>(m_context->swapchainExtent.width) *
                    m_context->swapchainExtent.height * sizeof(float);
            if (!CreateHostVisibleBuffer(m_context->physicalDevice, m_context->device,
                                         depthBytes, VK_BUFFER_USAGE_TRANSFER_DST_BIT,
                                         &m_context->depthReadbackBuffer,
                                         &m_context->depthReadbackMemory, m_lastError,
    ...
    bool VulkanRenderBackend::CreateOpaqueRasterScaffold() {
        ...
        if (!CheckVk(vkCreateRenderPass(m_context->device, &renderPassInfo, nullptr,
                                        &m_context->opaqueRenderPass),
                     m_lastError, "vkCreateRenderPass")) {
            return false;
        }
        ...
        for (size_t i = 0; i < m_context->swapchainImageViews.size(); ++i) {
            ...
            if (!CheckVk(vkCreateFramebuffer(m_context->device, &framebufferInfo,
                                             nullptr,
                                             &m_context->opaqueFramebuffers[i]),
                         m_lastError, "vkCreateFramebuffer")) {
                return false;
            }
        }
    ...
    void VulkanRenderBackend::DestroyOpaqueRasterScaffold() {
        ...
        if (m_context->opaqueRenderPass != VK_NULL_HANDLE) {
            vkDestroyRenderPass(m_context->device, m_context->opaqueRenderPass,
                                nullptr);
            m_context->opaqueRenderPass = VK_NULL_HANDLE;
        }
    }
    ...
    bool VulkanRenderBackend::CreateOpaquePipelineCreationScaffold() {
        ...
        return CheckVk(vkCreatePipelineCache(m_context->device, &pipelineCacheInfo,
                                             nullptr,
                                             &m_context->opaquePipelineCache),
                       m_lastError, "vkCreatePipelineCache");
    }
    ...
    void VulkanRenderBackend::DestroyOpaquePipelineCreationScaffold() {
        ...
        if (m_context->opaquePipelineCache != VK_NULL_HANDLE) {
            vkDestroyPipelineCache(m_context->device, m_context->opaquePipelineCache,
                                   nullptr);
            m_context->opaquePipelineCache = VK_NULL_HANDLE;
        }
    }
    ...
    bool VulkanRenderBackend::CreateOpaqueMaterialBindingScaffold() {
        ...
        if (!CheckVk(vkCreateDescriptorSetLayout(m_context->device, &setLayoutInfo,
                                                 nullptr,
                                                 &m_context->opaqueMaterialSetLayout),
                     m_lastError, "vkCreateDescriptorSetLayout(opaqueMaterial)")) {
            return false;
        }
        ...
        if (!CheckVk(vkCreateDescriptorPool(m_context->device, &poolInfo, nullptr,
                                            &m_context->opaqueMaterialDescriptorPool),
                     m_lastError, "vkCreateDescriptorPool(opaqueMaterial)")) {
            return false;
        }

        const VkDeviceSize materialBufferSize = static_cast<VkDeviceSize>(
            sizeof(OpaqueMaterialGpuData) * kOpaqueMaterialDescriptorCapacity);
        ...
        if (!CheckVk(vkCreateBuffer(m_context->device, &materialBufferInfo, nullptr,
                                    &m_context->opaqueMaterialBuffer),
                     m_lastError, "vkCreateBuffer(opaqueMaterial)")) {
            return false;
        }
        ...
        const uint32_t memoryTypeIndex = FindMemoryTypeIndex(
            m_context->physicalDevice, materialMemoryRequirements.memoryTypeBits,
            VK_MEMORY_PROPERTY_HOST_VISIBLE_BIT |
            VK_MEMORY_PROPERTY_HOST_COHERENT_BIT);
        if (memoryTypeIndex == kInvalidQueueFamily) {
            m_lastError = "No host-visible Vulkan memory type supports opaque material "
                    "uniform buffer.";
            return false;
        }
        ...
        if (!CheckVk(vkAllocateMemory(m_context->device, &materialMemoryAlloc,
                                      nullptr,
                                      &m_context->opaqueMaterialBufferMemory),
                     m_lastError, "vkAllocateMemory(opaqueMaterial)")) {
            return false;
        }

        if (!CheckVk(vkBindBufferMemory(m_context->device,
                                        m_context->opaqueMaterialBuffer,
                                        m_context->opaqueMaterialBufferMemory, 0),
                     m_lastError, "vkBindBufferMemory(opaqueMaterial)")) {
            return false;
        }
        ...
        if (!CheckVk(vkMapMemory(m_context->device,
                     ...
                     m_lastError, "vkMapMemory(opaqueMaterial)")) {
            return false;
        }

        std::vector<VkDescriptorSetLayout> setLayouts(
            kOpaqueMaterialDescriptorCapacity, m_context->opaqueMaterialSetLayout);
        m_context->opaqueMaterialDescriptorSets.resize(
            kOpaqueMaterialDescriptorCapacity);
        ...
        if (!CheckVk(vkAllocateDescriptorSets(
                         m_context->device, &allocateInfo,
                         m_context->opaqueMaterialDescriptorSets.data()),
                     m_lastError, "vkAllocateDescriptorSets(opaqueMaterial)")) {
            return false;
        }
    ...
    void VulkanRenderBackend::DestroyOpaqueMaterialBindingScaffold() {
        ...
        if (m_context->opaqueMaterialBuffer != VK_NULL_HANDLE) {
            vkDestroyBuffer(m_context->device, m_context->opaqueMaterialBuffer,
                            nullptr);
            m_context->opaqueMaterialBuffer = VK_NULL_HANDLE;
        }
        if (m_context->opaqueMaterialBufferMemory != VK_NULL_HANDLE) {
            vkFreeMemory(m_context->device, m_context->opaqueMaterialBufferMemory,
                         nullptr);
        ...
        if (m_context->opaqueMaterialDescriptorPool != VK_NULL_HANDLE) {
            vkDestroyDescriptorPool(m_context->device,
                                    m_context->opaqueMaterialDescriptorPool, nullptr);
        ...
        if (m_context->opaqueMaterialSetLayout != VK_NULL_HANDLE) {
            vkDestroyDescriptorSetLayout(m_context->device,
                                         m_context->opaqueMaterialSetLayout, nullptr);
            m_context->opaqueMaterialSetLayout = VK_NULL_HANDLE;
        }
    }
    ...
    bool VulkanRenderBackend::CreateOpaqueDrawIndexBuffer() {
        ...
        if (!CheckVk(vkCreateBuffer(m_context->device, &bufferInfo, nullptr,
                                    &m_context->opaqueFallbackIndexBuffer),
                     m_lastError, "vkCreateBuffer(opaqueIndex)")) {
            return false;
        }
        ...
        const uint32_t memoryTypeIndex = FindMemoryTypeIndex(
            m_context->physicalDevice, indexMemoryRequirements.memoryTypeBits,
            VK_MEMORY_PROPERTY_HOST_VISIBLE_BIT |
            VK_MEMORY_PROPERTY_HOST_COHERENT_BIT);
        if (memoryTypeIndex == kInvalidQueueFamily) {
            m_lastError = "No host-visible Vulkan memory type supports opaque fallback "
                    "index buffer.";
            return false;
        }
        ...
        if (!CheckVk(vkAllocateMemory(m_context->device, &memoryAllocInfo, nullptr,
                                      &m_context->opaqueFallbackIndexBufferMemory),
                     m_lastError, "vkAllocateMemory(opaqueIndex)")) {
            return false;
        }
        if (!CheckVk(vkBindBufferMemory(
                         m_context->device, m_context->opaqueFallbackIndexBuffer,
                         m_context->opaqueFallbackIndexBufferMemory, 0),
                     m_lastError, "vkBindBufferMemory(opaqueIndex)")) {
            return false;
        }
        ...
        if (!CheckVk(vkMapMemory(m_context->device,
                                 m_context->opaqueFallbackIndexBufferMemory, 0,
                                 sizeof(kFallbackOpaqueIndices), 0, &mapped),
                     m_lastError, "vkMapMemory(opaqueIndex)")) {
            return false;
        }
        std::memcpy(mapped, kFallbackOpaqueIndices.data(),
    ...
    void VulkanRenderBackend::DestroyOpaqueDrawIndexBuffer() {
        ...
        if (m_context->opaqueFallbackIndexBuffer != VK_NULL_HANDLE) {
            vkDestroyBuffer(m_context->device, m_context->opaqueFallbackIndexBuffer,
                            nullptr);
            m_context->opaqueFallbackIndexBuffer = VK_NULL_HANDLE;
        }
        if (m_context->opaqueFallbackIndexBufferMemory != VK_NULL_HANDLE) {
            vkFreeMemory(m_context->device, m_context->opaqueFallbackIndexBufferMemory,
                         nullptr);
    ...
    bool VulkanRenderBackend::EnsureOpaqueMeshGpuBuffers(const Mesh &mesh) {
        ...
        if (m_context->opaqueMeshGpuBuffers.find(&mesh) !=
            m_context->opaqueMeshGpuBuffers.end())
            return true;
        ...
        if (!CreateHostVisibleBuffer(
            m_context->physicalDevice, m_context->device, vertexBufferSize,
            VK_BUFFER_USAGE_VERTEX_BUFFER_BIT, &gpuBuffers.vertexBuffer,
            &gpuBuffers.vertexMemory, m_lastError,
        ...
        if (!CreateHostVisibleBuffer(m_context->physicalDevice, m_context->device,
                                     ...
                                     &gpuBuffers.indexBuffer, &gpuBuffers.indexMemory,
                                     m_lastError, "CreateHostVisibleBuffer(index)")) {
        ...
        if (!CheckVk(vkMapMemory(m_context->device, gpuBuffers.vertexMemory, 0,
                                 vertexBufferSize, 0, &vertexMapped),
                     m_lastError, "vkMapMemory(meshVertex)")) {
        ...
        std::memcpy(vertexMapped, vertices.data(),
                    static_cast<size_t>(vertexBufferSize));
        vkUnmapMemory(m_context->device, gpuBuffers.vertexMemory);

        void *indexMapped = nullptr;
        if (!CheckVk(vkMapMemory(m_context->device, gpuBuffers.indexMemory, 0,
                                 indexBufferSize, 0, &indexMapped),
                     m_lastError, "vkMapMemory(meshIndex)")) {
            vkDestroyBuffer(m_context->device, gpuBuffers.indexBuffer, nullptr);
            vkFreeMemory(m_context->device, gpuBuffers.indexMemory, nullptr);
            vkDestroyBuffer(m_context->device, gpuBuffers.vertexBuffer, nullptr);
            vkFreeMemory(m_context->device, gpuBuffers.vertexMemory, nullptr);
        ...
        std::memcpy(indexMapped, indices.data(),
    ...
    bool VulkanRenderBackend::CreateOpaqueShaderPipelineScaffold() {
        const auto tryLoadShaderModule =
                [&](const std::array<std::string, 2> &candidatePaths,
                    VkShaderModule *outModule, const char *context) {
            for (const std::string &path: candidatePaths) {
                const std::vector<char> bytes = ReadBinaryFile(path);
                if (bytes.empty())
                    continue;
                if (TryCreateShaderModule(m_context->device, bytes, outModule,
                                          m_lastError, context))
                    return true;
                return false;
            }
        ...
        const bool vertexShaderLoaded = tryLoadShaderModule(
            {
                "assets/shaders/vulkan/opaque_scene.vert.spv",
                "renderer/shaders/vulkan/opaque_scene.vert.spv"
            },
            &m_context->opaqueVertexShaderModule, "vkCreateShaderModule(vertex)");
        const bool fragmentShaderLoaded = tryLoadShaderModule(
            {
                "assets/shaders/vulkan/opaque_scene.frag.spv",
                "renderer/shaders/vulkan/opaque_scene.frag.spv"
            },
            &m_context->opaqueFragmentShaderModule, "vkCreateShaderModule(fragment)");
        ...
        if ((!vertexShaderLoaded || !fragmentShaderLoaded) && m_lastError.empty()) {
            m_lastError = "Vulkan opaque shader modules are not available yet. "
                    "Expected precompiled SPIR-V files at "
    ...
    void VulkanRenderBackend::DestroyOpaqueShaderPipelineScaffold() {
        ...
        if (m_context->opaqueVertexShaderModule != VK_NULL_HANDLE) {
            vkDestroyShaderModule(m_context->device,
                                  m_context->opaqueVertexShaderModule, nullptr);
            m_context->opaqueVertexShaderModule = VK_NULL_HANDLE;
        }
        if (m_context->opaqueFragmentShaderModule != VK_NULL_HANDLE) {
            vkDestroyShaderModule(m_context->device,
                                  m_context->opaqueFragmentShaderModule, nullptr);
            m_context->opaqueFragmentShaderModule = VK_NULL_HANDLE;
        }
    ...
    bool VulkanRenderBackend::CreateOpaqueGraphicsPipelineScaffold() {
        ...
        for (auto &entry: m_context->opaquePipelineCacheByKey) {
            ...
            if (cachedPipeline != VK_NULL_HANDLE &&
                cachedPipeline != previousPrimaryPipeline)
        ...
        if (!m_context->opaqueGraphicsPipelineScaffoldReady) {
            m_lastError =
                    "Opaque graphics pipeline scaffold prerequisites are incomplete.";
            return false;
        }

        if (m_context->opaqueShaderStages[0].module == VK_NULL_HANDLE ||
            m_context->opaqueShaderStages[1].module == VK_NULL_HANDLE) {
            m_lastError = "Vulkan opaque graphics pipeline cannot execute draws "
                    "because shader modules are unavailable.";
            return true;
        }
        ...
        m_context->opaqueGraphicsPipeline =
                reinterpret_cast<VkPipeline>(pipelineHandle);
        m_context->opaqueGraphicsPipelineExecutable =
                m_context->opaqueGraphicsPipeline != VK_NULL_HANDLE;
    ...
    bool VulkanRenderBackend::GetOrCreateOpaquePipeline(
        const OpaquePipelineKey &key, void **outPipelineHandle) {
        ...
        *outPipelineHandle = nullptr;
        ...
        if (it != m_context->opaquePipelineCacheByKey.end() &&
        ...
        if (m_context->opaqueShaderStages[0].module == VK_NULL_HANDLE ||
            m_context->opaqueShaderStages[1].module == VK_NULL_HANDLE) {
            m_lastError = "Cannot build opaque Vulkan pipeline for key: shader modules "
                    "are unavailable.";
            return false;
        }
        ...
        if (!CheckVk(vkCreateGraphicsPipelines(m_context->device,
                                               m_context->opaquePipelineCache, 1,
                                               &pipelineInfo, nullptr, &newPipeline),
                     m_lastError, "vkCreateGraphicsPipelines(opaque-keyed)")) {
            return false;
        }
    ...
    bool VulkanRenderBackend::RecreateSwapchain() {
        ...
        if (!CheckVk(vkGetPhysicalDeviceSurfaceCapabilitiesKHR(
                         m_context->physicalDevice, m_context->surface,
                         &surfaceCapabilities),
                     m_lastError, "vkGetPhysicalDeviceSurfaceCapabilitiesKHR")) {
            return false;
        }

        uint32_t surfaceFormatCount = 0;
        vkGetPhysicalDeviceSurfaceFormatsKHR(m_context->physicalDevice,
                                             m_context->surface, &surfaceFormatCount,
                                             nullptr);
        ...
        vkGetPhysicalDeviceSurfacePresentModesKHR(
            m_context->physicalDevice, m_context->surface, &presentModeCount,
            presentModes.data());
        ...
        glfwGetFramebufferSize(m_context->window, &framebufferWidth,
                               &framebufferHeight);
        ...
        if (surfaceCapabilities.currentExtent.width !=
            ...
            m_context->swapchainExtent.width =
                    std::clamp(static_cast<uint32_t>(framebufferWidth),
                               surfaceCapabilities.minImageExtent.width,
                               surfaceCapabilities.maxImageExtent.width);
            m_context->swapchainExtent.height =
                    std::clamp(static_cast<uint32_t>(framebufferHeight),
        ...
        const VkSwapchainCreateInfoKHR swapchainInfo{
            ...
            .imageSharingMode = usesSeparatePresentQueue
                                    ? VK_SHARING_MODE_CONCURRENT
                                    : VK_SHARING_MODE_EXCLUSIVE,
            .queueFamilyIndexCount = usesSeparatePresentQueue ? 2u : 0u,
            .pQueueFamilyIndices =
            usesSeparatePresentQueue ? queueFamilyIndices.data() : nullptr,
        ...
        if (!CheckVk(vkCreateSwapchainKHR(m_context->device, &swapchainInfo, nullptr,
                                          &m_context->swapchain),
                     m_lastError, "vkCreateSwapchainKHR")) {
            return false;
        }

        uint32_t swapchainImageCount = 0;
        vkGetSwapchainImagesKHR(m_context->device, m_context->swapchain,
        ...
        if (!CheckVk(vkAllocateCommandBuffers(m_context->device, &commandBufferInfo,
                                              m_context->commandBuffers.data()),
                     m_lastError, "vkAllocateCommandBuffers")) {
            return false;
        }
        ...
        m_context->supportsColorReadback =
                (swapchainUsage & VK_IMAGE_USAGE_TRANSFER_SRC_BIT) != 0;
        m_context->supportsDepthReadback =
                m_context->depthFormat == VK_FORMAT_D32_SFLOAT &&
                HasFormatFeature(m_context->physicalDevice, m_context->depthFormat,
                                 VK_FORMAT_FEATURE_TRANSFER_SRC_BIT);
        ...
        if (m_context->opaqueShaderPipelineScaffoldReady &&
            !CreateOpaqueGraphicsPipelineScaffold())
            return false;
    ...
    bool VulkanRenderBackend::RecordFrameCommands(const RenderFrameConfig &frame) {
        VkCommandBuffer commandBuffer =
                m_context->commandBuffers[m_context->activeImageIndex];
        if (!CheckVk(vkResetCommandBuffer(commandBuffer, 0), m_lastError,
        ...
        if (hasQueuedOpaqueDraws && !hasExecutableOpaquePipeline) {
            m_lastError = "Opaque scene draw submissions are queued, but Vulkan draw "
                          "execution is not ready: "
                          "shader modules and graphics pipeline are still "
                          "scaffold-only. Pending draws: " +
        ...
        if (hasQueuedOpaqueDraws || hasQueuedOverlayRender) {
            ...
            const std::array<VkClearValue, 2> clearValues = {
                VkClearValue{
                    .color = {
                        {
                            frame.clearColor.x, frame.clearColor.y,
                            frame.clearColor.z, frame.clearColor.w
                        }
                    }
                },
            ...
            const VkRenderPassBeginInfo renderPassBeginInfo{
                ...
                .renderPass = m_context->opaqueRenderPass,
                .framebuffer =
                m_context->opaqueFramebuffers[m_context->activeImageIndex],
            ...
            if (hasQueuedOpaqueDraws && hasExecutableOpaquePipeline) {
                for (size_t drawIndex = 0; drawIndex < m_pendingOpaqueDraws.size();
                     ++drawIndex) {
                    ...
                    if (!EnsureOpaqueMeshGpuBuffers(*draw.mesh)) {
                        if (m_lastError.empty()) {
                            m_lastError = "Vulkan opaque draw execution skipped because mesh "
                                    "GPU buffers are unavailable.";
                        }
                        continue;
                    }
                    ...
                    if (meshGpu.indexBuffer == VK_NULL_HANDLE ||
                        meshGpu.vertexBuffer == VK_NULL_HANDLE || meshGpu.indexCount == 0)
                        continue;
                    ...
                    vkCmdBindVertexBuffers(commandBuffer, 0, 1, vertexBuffers,
                                           vertexOffsets);
                    vkCmdBindIndexBuffer(commandBuffer, meshGpu.indexBuffer, 0,
                                         VK_INDEX_TYPE_UINT32);

                    const uint32_t materialSlot = static_cast<uint32_t>(
                        drawIndex % m_context->opaqueMaterialDescriptorSets.size());
                    auto *materialStorage = reinterpret_cast<OpaqueMaterialGpuData *>(
                        m_context->opaqueMaterialBufferMapped);
                    ...
                    vkCmdBindDescriptorSets(
                        ...
                        m_context->opaquePipelineLayout, 0, 1,
                        &m_context->opaqueMaterialDescriptorSets[materialSlot], 0, nullptr);

                    const uint32_t submittedIndexCount =
                            static_cast<uint32_t>(draw.indexCount);
                    const uint32_t drawIndexCount =
                            std::min(submittedIndexCount, meshGpu.indexCount);
                    if (drawIndexCount == 0)
                        continue;
            ...
            auto viewportTargetIt =
                    m_context->offscreenTargets.find(kEditorViewportTargetKey);
            if (viewportTargetIt != m_context->offscreenTargets.end()) {
                Context::OffscreenRenderTarget &viewportTarget = viewportTargetIt->second;
                if (viewportTarget.readyForSampling &&
                    viewportTarget.image != VK_NULL_HANDLE) {
                    ...
                    vkCmdPipelineBarrier(commandBuffer,
                                         ...
                                         nullptr, 1, &sourceToTransfer);

                    const VkImageLayout destinationOldLayout =
                            viewportTarget.currentLayout == VK_IMAGE_LAYOUT_UNDEFINED
                                ? VK_IMAGE_LAYOUT_UNDEFINED
                    ...
                    vkCmdPipelineBarrier(commandBuffer,
                                         ...
                                         VK_PIPELINE_STAGE_TRANSFER_BIT, 0, 0, nullptr, 0,
                                         nullptr, 1, &targetToTransfer);

                    VkImageBlit blitRegion{};
                    blitRegion.srcSubresource = {VK_IMAGE_ASPECT_COLOR_BIT, 0, 0, 1};
                    blitRegion.srcOffsets[0] = {0, 0, 0};
                    blitRegion.srcOffsets[1] = {
                        static_cast<int32_t>(m_context->swapchainExtent.width),
                        static_cast<int32_t>(m_context->swapchainExtent.height), 1
                    };
                    blitRegion.dstSubresource = {VK_IMAGE_ASPECT_COLOR_BIT, 0, 0, 1};
                    blitRegion.dstOffsets[0] = {0, 0, 0};
                    blitRegion.dstOffsets[1] = {
                        static_cast<int32_t>(viewportTarget.width),
                    ...
                    vkCmdPipelineBarrier(commandBuffer, VK_PIPELINE_STAGE_TRANSFER_BIT,
                                         ...
                                         nullptr, 0, nullptr, 1, &targetToSample);
            ...
            if (m_context->supportsDepthReadback &&
                ...
                vkCmdCopyImageToBuffer(commandBuffer, m_context->depthImage,
                                       VK_IMAGE_LAYOUT_TRANSFER_SRC_OPTIMAL,
                                       m_context->depthReadbackBuffer, 1,
                                       &depthCopyRegion);
                ...
                vkCmdPipelineBarrier(commandBuffer, VK_PIPELINE_STAGE_TRANSFER_BIT,
                                     ...
                                     nullptr, 0, nullptr, 1, &depthToAttachment);
            ...
            const VkClearColorValue clearColor = {
                {
                    frame.clearColor.x, frame.clearColor.y, frame.clearColor.z,
                    frame.clearColor.w
                }
            };
            ...
            vkCmdClearColorImage(
                commandBuffer, m_context->swapchainImages[m_context->activeImageIndex],
        ...
        if (m_context->supportsColorReadback &&
            ...
            vkCmdCopyImageToBuffer(
                commandBuffer, m_context->swapchainImages[m_context->activeImageIndex],
                VK_IMAGE_LAYOUT_TRANSFER_SRC_OPTIMAL, m_context->colorReadbackBuffer, 1,
                &colorCopyRegion);
    ...
    void VulkanRenderBackend::BeginFrame(const RenderFrameConfig &frame) {
        ...
        if (!CheckVk(vkWaitForFences(m_context->device, 1, &m_context->inFlightFence,
                                     VK_TRUE, UINT64_MAX),
                     m_lastError, "vkWaitForFences")) {
            return;
        }

        VkResult acquireResult =
                vkAcquireNextImageKHR(m_context->device, m_context->swapchain, UINT64_MAX,
                                      m_context->imageAvailableSemaphore, VK_NULL_HANDLE,
        ...
        if (acquireResult == VK_ERROR_OUT_OF_DATE_KHR ||
            ...
            acquireResult =
                    vkAcquireNextImageKHR(m_context->device, m_context->swapchain,
                                          ...
                                          VK_NULL_HANDLE, &m_context->activeImageIndex);
        }
    ...
    void VulkanRenderBackend::EndFrame() {
        ...
        if (m_context->frameCommandsRecorded) {
            ...
            if (!CheckVk(vkQueueSubmit(m_context->graphicsQueue, 1, &submitInfo,
                                       m_context->inFlightFence),
                         m_lastError, "vkQueueSubmit")) {
                m_frameActive = false;
                return;
            }
    ...
    void VulkanRenderBackend::DrawMesh(const MeshDrawCommand &command) {
        if (!m_passActive || !IsSupportedVulkanScenePass(m_activePassId) ||
            !command.mesh || !command.material)
            return;
        ...
        m_pendingOpaqueDraws.back().pipelineKey =
                BuildOpaquePipelineKey(m_pendingOpaqueDraws.back().material);
        ++m_drawCalls;
    }
    ...
    VulkanRenderBackend::VulkanRenderBackend( // NOSONAR: cpp:S5008 native window handle type is interface-defined
    ...
    bool VulkanRenderBackend::IsInitialized() const { return false; }
    ...
    bool VulkanRenderBackend::TryGetImGuiVulkanInitData( // NOSONAR: cpp:S5008 Vulkan init API is pointer-handle based
        void **, void **, void **, uint32_t *, void **, // NOSONAR: cpp:S5008 pointer-handle output parameters are required by backend interface
        void **, uint32_t *) const { // NOSONAR: cpp:S5008 Vulkan API requires void* handles
        return false;
    }

    void *VulkanRenderBackend::GetActiveCommandBufferHandle() const { // NOSONAR: cpp:S5008 Vulkan API requires opaque pointer handles
        return nullptr;
    }

    void VulkanRenderBackend::QueueOverlayRenderCallback( // NOSONAR: cpp:S5817,cpp:S1238,cpp:S5213 interface-fixed callback signature
        OverlayRenderCallback, // NOSONAR: cpp:S1238,cpp:S5213 callback alias and parameter passing are fixed by the shared interface
        void *) { // NOSONAR: cpp:S5817,cpp:S1238,cpp:S5213,cpp:S5008 signature is fixed by shared backend interface
        // Not implemented: Vulkan backend is a stub
    }

    bool VulkanRenderBackend::ReadbackColorBgr8(int, int, std::vector<uint8_t> &,
                                                std::string *) {
        return false;
    }

    bool VulkanRenderBackend::ReadbackDepth32F(int, int, std::vector<float> &,
                                               std::string *) {
        return false;
    }

    bool VulkanRenderBackend::EnsureEditorViewportRenderTarget(uint32_t, uint32_t,
                                                               std::string *) {
        return false;
    }

    bool VulkanRenderBackend::TryGetEditorViewportRenderTargetHandle(
        RenderTargetHandle *, bool, std::string *) {
        return false;
    }

    bool VulkanRenderBackend::EnsureOffscreenRenderTarget( // NOSONAR: cpp:S5817 interface requires non-const mutating method
        const std::string &, uint32_t,
        uint32_t) { // NOSONAR: cpp:S5817 non-const to match mutable backend interface
        return false;
    }

    bool VulkanRenderBackend::TryGetOffscreenRenderTargetHandle( // NOSONAR: cpp:S5817 non-const to match mutable backend interface
        const std::string &, RenderTargetHandle *,
        bool) { // NOSONAR: cpp:S5817 non-const to match mutable backend interface
        return false;
    }

    bool VulkanRenderBackend::TryGetOffscreenRenderTargetMetadata(
        const std::string &, OffscreenTargetMetadata *) const {
        return false;
    }

    void VulkanRenderBackend::DestroyOffscreenRenderTarget( // NOSONAR: cpp:S5817 interface requires non-const mutating method
        const std::string &) { // NOSONAR: cpp:S5817 non-const to match mutable backend interface
        // Not implemented: Vulkan backend is a stub
    }

    void VulkanRenderBackend::DestroyAllOffscreenRenderTargets() { // NOSONAR: cpp:S5817 non-const to match mutable backend interface
        // Not implemented: Vulkan backend is a stub
    }

    bool VulkanRenderBackend::Initialize( // NOSONAR: cpp:S5008,cpp:S5817 init contract uses opaque handle and mutates backend
        void *) { // NOSONAR: cpp:S5008,cpp:S5817 native pointer + mutating init signature is fixed by interface
        return false;
    }

    void VulkanRenderBackend::Shutdown() { // NOSONAR: cpp:S5817 non-const to match mutable backend interface
        // Not implemented: Vulkan backend is a stub
    }

    bool VulkanRenderBackend::RecreateSwapchain() { // NOSONAR: cpp:S5817 non-const to match mutable backend interface
        return false;
    }

    void VulkanRenderBackend::DestroySwapchain() { // NOSONAR: cpp:S5817 non-const to match mutable backend interface
        // Not implemented: Vulkan backend is a stub
    }

    bool VulkanRenderBackend::CreateOpaqueRasterScaffold() { // NOSONAR: cpp:S5817 non-const to match mutable backend interface
        return false;
    }

    void VulkanRenderBackend::DestroyOpaqueRasterScaffold() { // NOSONAR: cpp:S5817 non-const to match mutable backend interface
        // Not implemented: Vulkan backend is a stub
    }

    bool VulkanRenderBackend::CreateOpaquePipelineCreationScaffold() { // NOSONAR: cpp:S5817 non-const to match mutable backend interface
        return false;
    }

    void VulkanRenderBackend::DestroyOpaquePipelineCreationScaffold() { // NOSONAR: cpp:S5817 non-const to match mutable backend interface
        // Not implemented: Vulkan backend is a stub
    }

    bool VulkanRenderBackend::CreateDepthResources() { // NOSONAR: cpp:S5817 non-const to match mutable backend interface
        return false;
    }

    void VulkanRenderBackend::DestroyDepthResources() { // NOSONAR: cpp:S5817 non-const to match mutable backend interface
        // Not implemented: Vulkan backend is a stub
    }

    bool VulkanRenderBackend::EnsureReadbackBuffers() { // NOSONAR: cpp:S5817 non-const to match mutable backend interface
        return false;
    }

    void VulkanRenderBackend::DestroyReadbackBuffers() { // NOSONAR: cpp:S5817 non-const to match mutable backend interface
        // Not implemented: Vulkan backend is a stub
    }

    bool VulkanRenderBackend::CreateOpaqueMaterialBindingScaffold() { // NOSONAR: cpp:S5817 non-const to match mutable backend interface
        return false;
    }

    void VulkanRenderBackend::DestroyOpaqueMaterialBindingScaffold() { // NOSONAR: cpp:S5817 non-const to match mutable backend interface
        // Not implemented: Vulkan backend is a stub
    }

    bool VulkanRenderBackend::CreateOpaqueDrawIndexBuffer() { // NOSONAR: cpp:S5817 non-const to match mutable backend interface
        return false;
    }

    void VulkanRenderBackend::DestroyOpaqueDrawIndexBuffer() { // NOSONAR: cpp:S5817 non-const to match mutable backend interface
        // Not implemented: Vulkan backend is a stub
    }

    bool VulkanRenderBackend::EnsureOpaqueMeshGpuBuffers( // NOSONAR: cpp:S5817 non-const to match mutable backend interface
        const Mesh &) { // NOSONAR: cpp:S5817 non-const to match mutable backend interface
        return false;
    }

    void VulkanRenderBackend::DestroyOpaqueMeshGpuBuffers() { // NOSONAR: cpp:S5817 non-const to match mutable backend interface
        // Not implemented: Vulkan backend is a stub
    }

    bool VulkanRenderBackend::CreateOpaqueShaderPipelineScaffold() { // NOSONAR: cpp:S5817 non-const to match mutable backend interface
        return false;
    }

    void VulkanRenderBackend::DestroyOpaqueShaderPipelineScaffold() { // NOSONAR: cpp:S5817 non-const to match mutable backend interface
        // Not implemented: Vulkan backend is a stub
    }

    bool VulkanRenderBackend::GetOrCreateOpaquePipeline( // NOSONAR: cpp:S5817,cpp:S5008 mutable cache + opaque handle output is interface-defined
        const OpaquePipelineKey &,
        void **) { // NOSONAR: cpp:S5817,cpp:S5008 mutating pipeline cache API uses opaque handle pointer by contract
        return false;
    }

    bool VulkanRenderBackend::CreateOpaqueGraphicsPipelineScaffold() { // NOSONAR: cpp:S5817 non-const to match mutable backend interface
        return false;
    }

    void VulkanRenderBackend::DestroyOpaqueGraphicsPipelineScaffold() { // NOSONAR: cpp:S5817 non-const to match mutable backend interface
        // Not implemented: Vulkan backend is a stub
    }

    bool VulkanRenderBackend::RecordFrameCommands( // NOSONAR: cpp:S5817 non-const to match mutable backend interface
        const RenderFrameConfig &) { // NOSONAR: cpp:S5817 non-const to match mutable backend interface
        return false;
    }

    bool VulkanRenderBackend::EnsureOffscreenRenderPass() { // NOSONAR: cpp:S5817 non-const to match mutable backend interface
        return false;
    }

    void VulkanRenderBackend::DestroyOffscreenRenderPass() { // NOSONAR: cpp:S5817 non-const to match mutable backend interface
        // Not implemented: Vulkan backend is a stub
    }

    bool VulkanRenderBackend::CreateOffscreenRenderTargetResources( // NOSONAR: cpp:S5817 non-const to match mutable backend interface
        const std::string &, uint32_t, uint32_t,
        uint64_t) { // NOSONAR: cpp:S5817 non-const to match mutable backend interface
        return false;
    }

    void VulkanRenderBackend::DestroyOffscreenRenderTargetResources( // NOSONAR: cpp:S5817 non-const to match mutable backend interface
        const std::string &) { // NOSONAR: cpp:S5817 non-const to match mutable backend interface
        // Not implemented: Vulkan backend is a stub
    }

    bool VulkanRenderBackend::TryRegisterOffscreenTargetImGuiDescriptor( // NOSONAR: cpp:S5817 non-const to match mutable backend interface
        const std::string &, RenderTargetHandle *,
        bool) { // NOSONAR: cpp:S5817 non-const to match mutable backend interface
        return false;
    }
