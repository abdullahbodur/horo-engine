...
Mesh Mesh::CreateSphere(float radius, int stacks, int slices) {
  ...
  for (int s = 0; s <= stacks; s++) {
    float phi = PI * (static_cast<float>(s) / stacks);
    for (int sl = 0; sl <= slices; sl++) {
      float theta = TWO_PI * (static_cast<float>(sl) / slices);
      Vec3 normal{Sin(phi) * Cos(theta), Cos(phi), Sin(phi) * Sin(theta)};
      verts.push_back({normal * radius,
                       normal,
                       {static_cast<float>(sl) / slices, static_cast<float>(s) / stacks}});
    }
  }

  for (int s = 0; s < stacks; s++) {
    for (int sl = 0; sl < slices; sl++) {
      unsigned int a = static_cast<unsigned int>(s * (slices + 1) + sl);
      unsigned int b = static_cast<unsigned int>((s + 1) * (slices + 1) + sl);
      unsigned int c = b + 1;
      unsigned int d = a + 1;
      inds.push_back(a);
      inds.push_back(b);
...
Mesh Mesh::CreateCylinder(float radius, float halfHeight, int slices) {
  ...
  const unsigned int topCenter = static_cast<unsigned int>(verts.size());
  verts.push_back({{0.0f, topY, 0.0f}, Vec3::Up(), {0.5f, 0.5f}});
  const unsigned int bottomCenter = static_cast<unsigned int>(verts.size());
  verts.push_back({{0.0f, bottomY, 0.0f}, Vec3{0.0f, -1.0f, 0.0f}, {0.5f, 0.5f}});
  ...
  for (int i = 0; i < slices; ++i) {
    ...
    const unsigned int ti0 = static_cast<unsigned int>(verts.size());
    verts.push_back({top0, Vec3::Up(), {(Cos(a0) + 1.0f) * 0.5f, (Sin(a0) + 1.0f) * 0.5f}});
    const unsigned int ti1 = static_cast<unsigned int>(verts.size());
    verts.push_back({top1, Vec3::Up(), {(Cos(a1) + 1.0f) * 0.5f, (Sin(a1) + 1.0f) * 0.5f}});

    inds.push_back(topCenter);
    inds.push_back(ti1);
    inds.push_back(ti0);

    const unsigned int bi0 = static_cast<unsigned int>(verts.size());
    verts.push_back({bot0, Vec3{0.0f, -1.0f, 0.0f}, {(Cos(a0) + 1.0f) * 0.5f, (Sin(a0) + 1.0f) * 0.5f}});
    const unsigned int bi1 = static_cast<unsigned int>(verts.size());
    verts.push_back({bot1, Vec3{0.0f, -1.0f, 0.0f}, {(Cos(a1) + 1.0f) * 0.5f, (Sin(a1) + 1.0f) * 0.5f}});
...
Mesh Mesh::CreatePyramid(float halfBase, float halfHeight) {
  ...
  auto addSide = [&](Vec3 a, Vec3 b, Vec3 c) {
    ...
    const unsigned int i = static_cast<unsigned int>(verts.size());
