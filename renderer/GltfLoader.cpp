...
namespace Monolith {
    ...
    // ---------------------------------------------------------------------------
    static bool LoadImageDataCallback( // NOSONAR: cpp:S107 callback signature required by tinygltf API
    ...
    static size_t GetAccessorStride(const tinygltf::Model &model, int accessorIdx) {
        const tinygltf::Accessor &acc = model.accessors[accessorIdx];
        if (const tinygltf::BufferView &bv = model.bufferViews[acc.bufferView]; bv.byteStride != 0)
    ...
    static int FindParentBoneIndex(const tinygltf::Model &model,
                                   ...
                                   int nodeIdx, int numProcessed) {
        for (int pi = 0; pi < numProcessed; ++pi) {
            const int parentNodeIdx =
                    skin.joints[sortedSlots[static_cast<size_t>(pi)]];
    ...
    static void LoadGltfSkeleton(const tinygltf::Model &model,
        ...
        for (int sortedPos = 0; sortedPos < numJoints; ++sortedPos) {
            ...
            bone.inverseBindMatrix = ibmBase
    ...
    static VertexAccessors BuildVertexAccessors(const tinygltf::Model &model,
                                                int posIdx, int normIdx,
                                                int uvIdx, int jointsIdx,
                                                int weightsIdx) {
        VertexAccessors acc{};
    ...
    static void LoadGltfMesh(const tinygltf::Model &model,
        ...
        for (const tinygltf::Primitive &prim: gltfMesh.primitives) {
            ...
            const VertexAccessors acc = BuildVertexAccessors(model, posIdx, normIdx,
                                                             uvIdx, jointsIdx, weightsIdx);
    ...
    static void ProcessAnimationChannel(
        ...
        const tinygltf::AnimationSampler &sampler, int boneIdx,
        float &clipDuration, std::unordered_map<int, BoneTrack> &trackMap) {
        ...
        auto &track =
                trackMap
                .try_emplace(boneIdx, BoneTrack{boneIdx, {}, {}, {}, {}, {}, {}})
                .first->second;
        ...
        for (const tinygltf::Animation &anim: model.animations) {
            ...
            for (const tinygltf::AnimationChannel &channel: anim.channels) {
                ...
                if (it == nodeIndexToBoneIndex.end())
                    continue;
                ProcessAnimationChannel(model, channel,
