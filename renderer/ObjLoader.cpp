...
static int ResolveIndex(int rawIdx, int size) {
  return rawIdx > 0 ? rawIdx - 1 : size + rawIdx;
}

static Vertex ParseFaceVertex(const std::string &token,
  ...
  // Split by '/'
  if (const size_t s1 = token.find('/'); s1 == std::string::npos) {
    pi = std::stoi(token);
  } else {
  ...
  if (pi != 0) {
    const int idx = ResolveIndex(pi, static_cast<int>(positions.size()));
    if (idx >= 0 && idx < static_cast<int>(positions.size()))
      v.position = positions[static_cast<size_t>(idx)];
  }
  if (ti != 0) {
    const int idx = ResolveIndex(ti, static_cast<int>(uvs.size()));
    if (idx >= 0 && idx < static_cast<int>(uvs.size()))
      v.uv = uvs[static_cast<size_t>(idx)];
  }
  if (ni != 0) {
    const int idx = ResolveIndex(ni, static_cast<int>(normals.size()));
...
Mesh Load(const std::string &path) {
  ...
  while (std::getline(file, line)) {
    ...
    if (keyword == "v") {
      ...
      Vertex v0 = ParseFaceVertex(tokens[0], positions, normals, uvs);
      for (size_t i = 1; i + 1 < tokens.size(); ++i) {
        Vertex v1 = ParseFaceVertex(tokens[i], positions, normals, uvs);
        Vertex v2 = ParseFaceVertex(tokens[i + 1], positions, normals, uvs);

        auto base = static_cast<unsigned int>(vertices.size());
