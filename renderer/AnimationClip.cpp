...
void AnimationClip::Sample(float time, std::vector<Mat4>& outLocalTransforms) const {
  ...
  for (const BoneTrack& track : m_tracks) {
    ...
    // Sample each channel — fall back to neutral values when the channel is absent.
    const Vec3 pos = track.positions.empty()
                         ? Vec3::Zero()
                         : SamplePositions(track, time);

    const Quaternion rot = track.rotations.empty()
                               ? Quaternion::Identity()
                               : SampleRotations(track, time);

    const Vec3 scale = track.scales.empty()
                           ? Vec3::One()
                           : SampleScales(track, time);
...
// so callers can safely access times[lo], times[lo+1], values[lo], values[lo+1].
// A return value of -1 signals "time is before the first key" (clamp to first).
// A return value of count-1 signals "time is at or beyond the last key".
...
Vec3 AnimationClip::SamplePositions(const BoneTrack& track, float time) {
  ...
  const float span = times[static_cast<size_t>(hi)] - times[static_cast<size_t>(lo)];
  const float raw  = (span > 0.0f)
                         ? (time - times[static_cast<size_t>(lo)]) / span
                         : 0.0f;
  const float t    = raw < 0.0f ? 0.0f : (raw > 1.0f ? 1.0f : raw);

  return Vec3::Lerp(values[static_cast<size_t>(lo)],
                    values[static_cast<size_t>(hi)],
                    t);
}
