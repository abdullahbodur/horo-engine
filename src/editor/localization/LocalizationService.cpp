...
namespace Horo::Editor
{
    namespace
    {
        ...
        [[nodiscard]] std::string NormalizeTag(std::string_view tag)
        {
            ...
            if (normalized.size() >= 2 && !IsAlpha(normalized[0])) return {};
            ...
            while (partStart < normalized.size())
            {
                ...
                if (const std::size_t length = partEnd == std::string::npos
                                                   ? normalized.size() - partStart
                                                   : partEnd - partStart;
    ...
    std::size_t MessageKeyHash::operator()(const MessageKey& key) const noexcept
    {
        ...
        return namespaceHash ^ (localKeyHash + static_cast<std::size_t>(0x9e3779b9) + (namespaceHash << 6U) + (
            namespaceHash >> 2U));
    }
    ...
    bool LocalizationService::LoadCatalogFile(const std::filesystem::path& path, LocalizationError* error)
    {
        ...
        if (const std::regex namespacePatternRe{R"re("namespace"\s*:\s*"([^"]+)")re"};
            !std::regex_search(json, localeMatch, localePattern) || !std::regex_search(
                json, namespaceMatch, namespacePatternRe))
        {
    ...
    {
        if (const auto parsed = LocaleTag::Parse(locale.value);
            !parsed.has_value() || parsed->value != locale.value)
        {
    ...
    const std::string& LocalizationService::Get(std::string_view namespaceId, std::string_view localKey) const
    {
        if (m_active)
        {
            ...
            if (it != m_active->messages.end()) return it->second;
        }
    ...
    {
        if (error) *error = LocalizationError{std::move(code), std::move(message)};
    }
} // namespace Horo::Editor
