...
[[nodiscard]] bool ParseHexColor(const std::string &hex, ImVec4 &color)
{
    ...
    color = ImVec4{static_cast<float>(red) / 255.0F, static_cast<float>(green) / 255.0F,
...
bool ReadJsonColors(const std::string &json,
                    std::unordered_map<std::string, ImVec4, ThemeStringHash, std::equal_to<>> &out)
{
