...
// ─────────────────────────────────────────────────────────────────────────
// Palet — HTML mockuplardaki CSS custom property'leriyle birebir eşleşir:
...
// ─────────────────────────────────────────────────────────────────────────
// Fontlar — uygulamada gerçekte yüklenen 3 font atlası
// ─────────────────────────────────────────────────────────────────────────
struct Fonts {
    ImFont* sans         = nullptr; // Inter,        yüklendiği boyut: 15px (--font-sans)
    ImFont* mono         = nullptr; // IBM Plex Mono yüklendiği boyut: 13px (--font-mono, regular)
    ImFont* monoSemiBold = nullptr; // IBM Plex Mono yüklendiği boyut: 15px (--font-mono, 600/700)
};
...
// HTML/CSS'teki keyfi font-size değerlerini, elimizdeki sabit atlaslarla
// yakalamak için gereken SetWindowFontScale() çarpanı.
...
// RAII: fontu push eder (nullptr ise no-op) ve pop'unu garanti eder.
struct ScopedFont {
    ImFont* font;
...
// RAII: pencereye özel font scale uygular, scope sonunda daima 1.0'a döner.
...
// Kısayol: `font`'u push edip HTML'deki hedef px değerine ölçekler,
// ikisini de scope sonunda otomatik geri alır.
//   Örn: ScopedTextStyle ts(f.mono, /*targetPx=*/11.0f, Theme::FontPx::Mono);
...
// ─────────────────────────────────────────────────────────────────────────
// Layout metrikleri — HTML mockuplardaki px değerlerinin karşılığı
// ─────────────────────────────────────────────────────────────────────────
namespace Layout {
...
// ─────────────────────────────────────────────────────────────────────────
// Global ImGui stilini uygular. ImGui::CreateContext()'ten sonra bir kez
// çağrılmalı. Renk/rounding/padding gibi TÜM görsel varsayılanlar buradan
// gelir; widget kodu bu fonksiyonun içeriğine bağımlı DEĞİLDİR.
// ─────────────────────────────────────────────────────────────────────────
