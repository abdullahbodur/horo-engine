...
Result<Math::Mat4> BuildRenderMvp(const Render::RenderCameraView &camera, const Math::Mat4 &localToWorld, const float aspect,
    ...
    const Result<Math::Mat4> projection = camera.projection.kind == Render::RenderProjectionKind::Perspective
                                              ? Math::TryPerspective(camera.projection.verticalFovRadians, aspect,
                                                                     camera.projection.nearPlane, camera.projection.farPlane, depthRange)
                                              : Math::TryOrthographic(camera.projection.orthographicHeight, aspect,
                                                                      camera.projection.nearPlane, camera.projection.farPlane, depthRange);
...
Result<Math::Ray> BuildEditorViewportRay(const EditorViewportCamera &camera, const float normalizedX, const float normalizedY,
    ...
    const Result<Math::Mat4> viewProjection = BuildEditorViewportViewProjection(camera, aspect, Math::ClipDepthRange::NegativeOneToOne);
