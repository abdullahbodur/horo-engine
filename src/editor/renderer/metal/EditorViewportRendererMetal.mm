                                       ...
                                       constant SelectionStyle& style [[buffer(0)]])
{
    ...
    if (impl_->gridPipeline == nil)
    {
        Shutdown();
        return Result<void>::Failure(
            MakeViewportError("editor.viewport.grid_pipeline_creation_failed",
    ...
    if (impl_->shadowPipeline == nil)
    {
        Shutdown();
        return Result<void>::Failure(
            MakeViewportError("editor.viewport.shadow_pipeline_creation_failed",
    ...
    MTLTextureDescriptor *shadowTextureDescriptor =
    ...
    if (impl_->depthState == nil || impl_->gridDepthState == nil || impl_->gridVertexBuffer == nil ||
        impl_->shadowSampler == nil || impl_->shadowDepthTexture == nil)
    {
        Shutdown();
...
Result<void> EditorViewportRendererMetal::ExecuteStaticMeshPass(const Render::StaticMeshPassDescriptor &descriptor)
{
    ...
    if (shadow.Value().has_value())
    {
        ...
        {
            return Result<void>::Failure(MakeViewportError(
                "editor.viewport.shadow_encoder_creation_failed",
        ...
        for (const Render::RenderStaticMeshInstance &instance : descriptor.scene.instances)
        {
            ...
            if (mesh == impl_->meshes.end())
            {
                ...
                return Result<void>::Failure(MakeViewportError(
                    "editor.viewport.stale_mesh_resource",
                    "Shadow pass instance references a stale mesh resource."));
            }
            const Math::Mat4 shadowMvp =
                Math::Multiply(shadow.Value()->viewProjection, instance.localToWorld);
    ...
    if (impl_->gridOptions.visible)
    {
        ...
        {
            [encoder endEncoding];
            return Result<void>::Failure(MakeViewportError("editor.viewport.invalid_grid",
                                                           "Failed to build finite viewport grid geometry."));
        }
        ...
        const auto encodeBatch = [&](const ViewportGridLineBatch batch) {
            ...
            [encoder setVertexBytes:viewProjection.Value().values.data()
                             length:sizeof(viewProjection.Value().values)
                            atIndex:1];
    ...
    sceneLighting.shadowLightIndex =
        shadow.Value().has_value() ? static_cast<std::uint32_t>(shadow.Value()->lightIndex) : 0U;
    ...
    if (impl_->lightVisualizerOptions.selectedLight.has_value())
    {
        ...
        {
            [encoder endEncoding];
            return Result<void>::Failure(MakeViewportError("editor.viewport.invalid_light_visualizer",
                                                           "Failed to build selected Light visualizer geometry."));
        }
        const Result<Math::Mat4> viewProjection =
            BuildRenderMvp(descriptor.scene.camera, Math::Mat4::Identity(), aspect, Math::ClipDepthRange::ZeroToOne);
        if (viewProjection.HasError())
        ...
        [encoder drawPrimitives:MTLPrimitiveTypeLine
                    vertexStart:0
                    vertexCount:static_cast<NSUInteger>(geometry.vertexCount)];
    }
