...
namespace Horo::Editor
{
    namespace
    {
        ...
        [[nodiscard]] Result<std::uint32_t> CompileShader(const std::uint32_t type, const char* source)
        {
            ...
            return Result<std::uint32_t>::Failure(
                MakeViewportError(RendererErrors::ViewportShaderCompileFailed,
                                  "Viewport shader compilation failed: " + log));
        }
    } // namespace
    ...
    Result<void> EditorViewportRendererOpenGL::ExecuteStaticMeshPass(const Render::StaticMeshPassDescriptor& descriptor)
    {
        ...
        {
            return Result<void>::Failure(
                MakeViewportError(RendererErrors::ViewportStaleTarget,
                                  "Viewport pass references a stale render target."));
        }
        ...
        if (const Result<void> synchronized = SynchronizeMeshes(descriptor.scene.meshResources); synchronized.
            HasError())
            return synchronized;
...
void main()
{
        ...
        const std::span<const EditorViewportMeshResourceView> resources)
    {
        ...
        for (const EditorViewportMeshResourceView& resource : resources)
        {
            ...
            GpuMesh mesh{
                .indexCount = static_cast<std::uint32_t>(resource.indices.size()),
            ...
            glBufferData(GL_ARRAY_BUFFER, static_cast<GLsizeiptr>(resource.vertices.size_bytes()),
    ...
    Result<void> EditorViewportRendererOpenGL::RecreateTarget(const EditorViewportExtent extent)
    {
        ...
        glTexImage2D(GL_TEXTURE_2D, 0, GL_RGBA8, static_cast<GLsizei>(extent.width),
                     static_cast<GLsizei>(extent.height), 0,
