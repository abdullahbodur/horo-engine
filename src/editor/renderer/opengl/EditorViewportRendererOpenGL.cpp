...
Result<void> EditorViewportRendererOpenGL::ExecuteStaticMeshPass(const Render::StaticMeshPassDescriptor &descriptor) {
    if (!initialized_) {
        return Result<void>::Failure(MakeViewportError(RendererErrors::ViewportNotInitialized, "Viewport renderer is not initialized."));
    }
    ...
    if (!descriptor.IsValid() || descriptor.extent.width != requestedExtent.width || descriptor.extent.height != requestedExtent.height) {
...
void main()
{
    ...
    if (linked != GL_TRUE) {
        ...
        return Result<void>::Failure(MakeViewportError(RendererErrors::ViewportShaderLinkFailed, "Viewport shader linking failed: " + log));
    }
...
Result<void> EditorViewportRendererOpenGL::SynchronizeMeshes(const std::span<const EditorViewportMeshResourceView> resources) {
    ...
    for (const EditorViewportMeshResourceView &resource : resources) {
        ...
        glBufferData(GL_ARRAY_BUFFER, static_cast<GLsizeiptr>(resource.vertices.size_bytes()), resource.vertices.data(), GL_STATIC_DRAW);
...
Result<void> EditorViewportRendererOpenGL::RecreateTarget(const EditorViewportExtent extent) {
    ...
    glRenderbufferStorage(GL_RENDERBUFFER, GL_DEPTH_COMPONENT24, static_cast<GLsizei>(extent.width), static_cast<GLsizei>(extent.height));
