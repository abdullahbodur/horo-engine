...
namespace {
    ...
    void DrawCardIconAndText(ImDrawList *drawList, const ActiveSnackbar &snackbar, const SeverityStyle &style,
                             const ImVec2 &pMin, const ImVec2 &iconCenter)
    {
    ...
    void DrawCardBackgroundAndProgress(ImDrawList *drawList, const ActiveSnackbar &snackbar,
                                       const SeverityStyle &style, const ImVec2 &pMin, const ImVec2 &pMax)
    {
...
void EditorSnackbarHost::OnNotificationEvent(const NotificationEvent &event)
{
    ...
    if (!event.deduplicationKey.empty()) {
        const auto it = std::find_if(activeSnackbars_.begin(), activeSnackbars_.end(),
                                     [&event](const ActiveSnackbar &item) {
...
std::optional<SnackbarActionInvokedEvent> EditorSnackbarHost::Draw(const EditorGuiContext &, const float elapsedSeconds)
{
    ...
    if (ImGui::IsKeyPressed(ImGuiKey_Escape, false) && !activeSnackbars_.empty() &&
        activeSnackbars_.front().event.dismissible) {
        DismissAt(0);
        if (activeSnackbars_.empty()) {
            return std::nullopt;
        }
