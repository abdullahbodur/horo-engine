...
void DrawGeneral(SettingsState &st, const EditorGuiContext &ctx)
{
    ...
    SettingRow(confirmLabel.c_str(), confirmDescription.c_str(), ctx.theme.fonts,
               [&st, &ctx]() { (void)ToggleControl("confirm-exit", &st.general.confirmExit, ctx.theme.fonts); });
...
void DrawAppearance(SettingsState &st, const EditorGuiContext &ctx)
{
    ...
    SettingRow(accentLabel.c_str(), accentDescription.c_str(), ctx.theme.fonts,
               [&st, &ctx]() { (void)ColorHexControl("accent-color", st.appearance.accentHex, 16, ctx.theme.fonts); });
    ...
    SettingRow(scaleLabel.c_str(), scaleDescription.c_str(), ctx.theme.fonts, [&st, &ctx]() {
        SliderIntControl("##ui-scale", &st.appearance.uiScale, 75, 200, SliderValueFormat::Percent, ctx.theme.fonts,
                         25);
    });
...
void DrawRendering(SettingsState &st, const EditorGuiContext &ctx)
{
    ...
    const std::array<const char *, 4> kViewport = {viewportText[0].c_str(), viewportText[1].c_str(),
    ...
    const std::array<const char *, 4> kTier = {tierText[0].c_str(), tierText[1].c_str(), tierText[2].c_str(),
                                               tierText[3].c_str()};
...
void DrawNetwork(SettingsState &st, const EditorGuiContext &ctx)
{
    ...
    SettingRow(clientsLabel.c_str(), clientsDescription.c_str(), ctx.theme.fonts,
               [&st, &ctx]() { InputIntControl("##max-clients", &st.network.maxPreviewClients, ctx.theme.fonts); });
...
void DrawDiagnostics(SettingsState &st, const EditorGuiContext &ctx)
{
    ...
    const std::array<const char *, 4> kLogLevels = {logLevelText[0].c_str(), logLevelText[1].c_str(),
    ...
    SettingRow(thresholdLabel.c_str(), thresholdDescription.c_str(), ctx.theme.fonts,
               [&st, &ctx]() { InputFloatControl("##stutter", &st.diagnostics.stutterThresholdMs, ctx.theme.fonts); });
}
...
void DrawBadge(const char *text, const ImVec4 colour, const EditorGuiContext &ctx)
{
    auto *dl = ImGui::GetWindowDrawList();
    ...
    const ImVec2 badgeMax{badgeMin.x + textSize.x + padX * 2.0F, badgeMin.y + textSize.y + padY * 2.0F};

    dl->AddRectFilled(badgeMin, badgeMax, ImColor{colour.x, colour.y, colour.z, 0.10F}, 999.0F);
    dl->AddRect(badgeMin, badgeMax, ImColor{colour.x, colour.y, colour.z, 0.28F}, 999.0F);
...
void DrawMcpDetailContent(SettingsState &st, const EditorGuiContext &ctx, const int activeTab)
{
    ...
    case 0:
        ...
        PluginSettingRow("Transport Mode", "Use stdio for local tools; HTTP is useful for explicit local integrations.",
                         ctx, [&st, &ctx]() {
        ...
        PluginSettingRow("Allowed Tool Groups", "Restrict what external tools can invoke.", ctx, [&st, &ctx]() {
            static constexpr std::array kScopes = {"Read + Safe Mutations", "Read Only", "Full Project Access",
                                                   "Custom Policy..."};
        ...
        PluginSettingRow("Asset Write Root", "All generated assets must stay under this folder.", ctx,
                         [&st, &ctx]() { (void)InputTextControl("##root", st.mcp.assetRoot, 64, ctx.theme.fonts); });
        break;
...
void DrawFmodDetailContent(SettingsState &st, const EditorGuiContext &ctx, const int activeTab)
{
    ...
    case 0:
        ...
        PluginSettingRow("Fail Build On Missing Banks", "Prevents shipping builds with unresolved audio events.", ctx,
...
void DrawSteamDetailContent(SettingsState &st, const EditorGuiContext &ctx, const int activeTab)
{
    ...
    case 0:
        ...
        PluginSettingRow("Networking Sockets", "Enable Steam networking transport for multiplayer preview.", ctx,
...
void DrawRuntimeDiscovery(SettingsState &st, const EditorGuiContext &ctx)
{
    ...
    PluginSettingRow(
        ...
        [&st, &ctx]() { (void)InputTextControl("##disc-path", st.runtime.discoveryPaths, 128, ctx.theme.fonts); });
    PluginSettingRow(
        "Load Order Policy", "Defines how editor, project, vendor, and local-development plugins are resolved.", ctx,
        [&st, &ctx]() {
    ...
    PluginSettingRow("Development Plugin Path",
                     ...
                     [&st, &ctx]() { (void)InputTextControl("##dev-path", st.runtime.devPath, 64, ctx.theme.fonts); });
    ...
    PluginSettingRow("Unsigned Plugin Policy",
                     ...
                     [&st, &ctx]() {
                         static constexpr std::array kPolicies = {"Block by default", "Allow after warning",
    ...
    PluginSettingRow("Network Access Policy",
                     "Default network behavior for plugins unless a plugin-specific permission overrides it.", ctx,
                     [&st, &ctx]() {
                         static constexpr std::array kNets = {"Deny by default", "Localhost only", "Prompt per plugin",
    ...
    PluginSettingRow("Compatibility Mode", "How strictly plugin API versions are validated when opening a project.",
                     ctx, [&st, &ctx]() {
                         static constexpr std::array kModes = {"Strict semantic versioning", "Allow compatible minors",
                                                               "Prompt on mismatch"};
                                                                 ...
                                                                 const EditorGuiContext &ctx, const ::ImTextureID logo)
{
    ...
    return (headerRequestedClose || footerRequestedClose) ? ModalFrameResult::RequestClose(ModalCloseReason::Cancelled)
