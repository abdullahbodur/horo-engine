...
namespace Horo::Editor {
    namespace {
        ...
        void DrawNetwork(SettingsState &st, const EditorGuiContext &ctx) {
            ...
            const std::string threadsDescription = ctx.localization.Get("editor", "settings.network.download_threads.description");
            ...
            SettingRow(latencyLabel.c_str(), latencyDescription.c_str(), ctx.theme.fonts, [&st, &ctx]() {
                SliderIntControl("##latency", &st.network.simulatedLatencyMs, 0, 500, SliderValueFormat::Milliseconds, ctx.theme.fonts);
            });
        ...
        void DrawPluginSectionTabs(SettingsState &st, const EditorGuiContext &ctx) {
            ...
            for (int i = 0; i < 2; ++i) {
                ...
                if (active) {
                    ...
                    dl->AddLine({bMin.x + 8.0F, bMax.y - 2.0F}, {bMax.x - 8.0F, bMax.y - 2.0F}, U32(Accent()), 2.0F);
                }
            }
        ...
        void DrawPluginList(SettingsState &st, const EditorGuiContext &ctx, float /*listW*/) {  // NOSONAR(cpp:S1144)
            ...
            for (const auto &p : kPlugins) {
                ...
                if (active) {
                    dl->AddRectFilled(cardPos, {cardPos.x + cardW, cardPos.y + cardH}, ImColor{Accent().x, Accent().y, Accent().z, 0.09F},
                                      Layout::Radius);
                    dl->AddRect(cardPos, {cardPos.x + cardW, cardPos.y + cardH}, U32(BorderStrong()), Layout::Radius);
                    ...
                    dl->AddRectFilled(cardPos, {cardPos.x + cardW, cardPos.y + cardH}, U32(Hover()), Layout::Radius);
        ...
        void DrawPluginDetailPanel(SettingsState &st, const EditorGuiContext &ctx, float w, bool embedded) {  // NOSONAR(cpp:S1144)
            ...
            const std::array<PluginDetailHeaderSpec, 3> kDetailHeaders = {{
                ...
                {"Vendor FMOD Integration", fmodDDesc.c_str(), fmodScope.c_str(), fmodSigned.c_str(), BadgeTone::Success,
        ...
        void DrawMcpDetailContent(SettingsState &st, const EditorGuiContext &ctx, const int activeTab) {
            switch (activeTab) {
                case 0:
                    ...
                    PluginSettingRow("Require Session Token", "Reject tool calls unless they include the generated editor session token.",
                                     ctx, [&st, &ctx]() {
        ...
        void DrawExtensionDetailSection(const char *label, const EditorGuiContext &ctx, const bool first) {
            if (!first) {
                ...
                ImGui::GetWindowDrawList()->AddLine(divider, {divider.x + ImGui::GetContentRegionAvail().x, divider.y}, U32(Border()),
                                                    1.0F);
        ...
        void DrawExtensionMarketplace(SettingsState &st, const EditorGuiContext &ctx) {
            ...
            const bool searchClicked =
                ImGui::Button(ctx.localization.Get("editor", "settings.extensions.marketplace.search.action").c_str(),
            ...
            for (const auto &entry : marketplace.entries) {
                ...
                const bool installing = marketplace.status == Extensions::ExtensionMarketplaceStatus::Installing &&
