...
namespace Horo::Editor
{
    SettingsModal::SettingsModal(const EditorGuiContext& context, EditorSettingsService& settings,
        ...
        : m_context(context), m_settings(settings), m_logo(logo),
          m_extensionInventory(extensionInventory),
    ...
    void SettingsModal::OnClose(const ModalCloseReason reason)
    {
        ...
        const char* reasonStr = (reason == ModalCloseReason::Completed)
                                    ...
                                    ? "cancelled"
                                    : "app_shutdown";
        LOG_INFO("editor.settings", "SettingsModal closed (reason=%s, dirty=%s).", reasonStr,
                 m_draft.dirty ? "yes" : "no");
        if (m_draft.dirty && !m_revertedPublished && m_events)
        {
            ...
            m_events->Publish(EditorSettingsChangedEvent{
                m_settings.Snapshot().revision, SettingsChangePhase::Reverted,
                SettingsDomain::All
            });
