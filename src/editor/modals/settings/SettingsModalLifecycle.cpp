...
namespace Horo::Editor
{
    ...
    Result<void> SettingsModal::OnOpen(EditorModalContext& context)
    {
        ...
        const auto rev = static_cast<unsigned long long>(m_settings.Snapshot().revision);
        m_logCtx = std::make_unique<Horo::Log::LogContext>(
            "modal",    "settings",
            "modal_id", std::to_string(kModalId),
    ...
    void SettingsModal::OnClose(ModalCloseReason reason)
    {
        m_draft.dirty = CollectDraftSettings(m_draft) != m_draft.committed;
        const char* reasonStr = (reason == ModalCloseReason::Completed)   ? "completed"
                              : (reason == ModalCloseReason::Cancelled)    ? "cancelled"
                                                                           : "app_shutdown";
        LOG_INFO("editor.settings", "SettingsModal closed (reason=%s, dirty=%s).",
                 reasonStr, m_draft.dirty ? "yes" : "no");
    ...
    bool SettingsModal::ApplyDraft()
    {
        ...
        LOG_INFO("editor.settings", "Settings applied (success=%s, revision=%llu).",
                 ok ? "yes" : "no",
                 static_cast<unsigned long long>(m_settings.Snapshot().revision));
        return ok;
    }
} // namespace Horo::Editor
