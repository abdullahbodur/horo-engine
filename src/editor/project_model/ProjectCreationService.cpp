...
void SetFailure(const std::shared_ptr<ProjectCreationServiceState> &state,
    ...
    LOG_ERROR("editor.project_creation", "Operation %llu failed [code=%d]: %s", static_cast<unsigned long long>(operation->snapshot.id),
              static_cast<int>(code), operation->snapshot.error->message.c_str());
  ...
  "settings": {{
    ...
    "requiredToolchain": {{
      ...
      "minimumCxxStandard": {}
                       ...
                       EscapeJson(request.defaultScene), EscapeJson(request.assetCompression), EscapeJson(request.textureCompression),
                       EscapeJson(request.buildProfile), EscapeJson(request.targetPlatform), EscapeJson(request.compilerFamily),
...
void RunCreate(const std::shared_ptr<ProjectCreationServiceState> &state,
    ...
    const std::filesystem::path parent = destination.has_parent_path() ? destination.parent_path() : std::filesystem::current_path();
    const std::filesystem::path staging =
    ...
    UpdateProgress(state, operation, ProjectCreationOperationState::Running, ProjectCreationOperationPhase::Validating, "validating",
                   0.02F);
    ...
    UpdateProgress(state, operation, ProjectCreationOperationState::Running, ProjectCreationOperationPhase::WritingMetadata, "metadata",
                   0.20F);
    std::this_thread::sleep_for(std::chrono::milliseconds(400));
    if (IsCancelled(cancellation, state, operation)) {
        cleanup();
    ...
    if (!PromoteWithoutReplace(staging, destination, error)) {
        ...
        const ProjectCreationErrorCode code =
            error == std::errc::file_exists ? ProjectCreationErrorCode::DestinationOccupied : ProjectCreationErrorCode::PromotionFailed;
...
[[nodiscard]] std::optional<Error> ValidateRequestSync(const ProjectCreationRequest &request) {
    if (Text::IsBlank(request.templateId) || Text::IsBlank(request.projectName) || request.projectName.find('/') != std::string::npos ||
        request.projectName.find('\\') != std::string::npos)
        return MakeFoundationError(ProjectCreationErrors::InvalidRequest, "Project name is required and must not contain path separators.");
    ...
    if (!IsSafeProjectRelativePath(request.defaultScene) || std::filesystem::path(request.defaultScene).filename().empty())
        return MakeFoundationError(ProjectCreationErrors::InvalidRequest, "Default scene must be a portable project-relative path.");
...
Result<ProjectCreationOperationHandle> ProjectCreationService::StartCreate(ProjectCreationRequest request) {
    LOG_DEBUG("editor.project_creation", "StartCreate requested for project '%s' at path '%s', template '%s'", request.projectName.c_str(),
              request.projectRoot.string().c_str(), request.templateId.c_str());
...
Result<void> ProjectCreationService::RequestCancel(const ProjectCreationOperationId id) {
    ...
    std::shared_ptr<ProjectCreationServiceState::Operation> operation;
    {
        ...
        if (found == state_->operations.end()) {
            ...
            return Result<void>::Failure(MakeFoundationError(ProjectCreationErrors::NotFound, "Project creation operation is not known."));
        }
