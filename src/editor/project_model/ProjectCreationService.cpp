               ...
               const ProjectCreationRequest request, const CancellationToken &cancellation)
{
    ...
    UpdateProgress(state, operation, ProjectCreationOperationState::Running, ProjectCreationOperationPhase::Staging,
                   "staging", 0.05F);
    if (IsCancelled(cancellation, state, operation))
        return;
    ...
    UpdateProgress(state, operation, ProjectCreationOperationState::Running, ProjectCreationOperationPhase::Promoting,
                   "promoting", 0.90F);
    if (IsCancelled(cancellation, state, operation))
    {
        cleanup();
...
{
    if (IsBlank(request.templateId) || IsBlank(request.projectName) ||
        request.projectName.find('/') != std::string::npos || request.projectName.find('\\') != std::string::npos)
        return MakeFoundationError("project_creation.invalid_request",
    ...
    if (std::filesystem::exists(request.projectRoot, error))
    {
        ...
        {
            return MakeFoundationError("project_creation.destination_occupied",
                                       "Project destination already exists and will not be overwritten.");
        }
    }
    if (error)
...
Result<ProjectCreationOperationHandle> ProjectCreationService::StartCreate(ProjectCreationRequest request)
{
    ...
    if (submitted.HasError())
    {
        ...
        return Result<ProjectCreationOperationHandle>::Failure(
            MakeFoundationError("project_creation.job_submission_failed", submitted.ErrorValue().message.c_str()));
    }
    {
        std::lock_guard lock(state_->synchronization.Mutex());
...
Result<void> ProjectCreationService::RequestCancel(const ProjectCreationOperationId id)
{
    ...
    std::shared_ptr<ProjectCreationServiceState::Operation> operation;
    {
        ...
        if (found == state_->operations.end())
        {
            ...
            return Result<void>::Failure(
                MakeFoundationError("project_creation.not_found", "Project creation operation is not known."));
        }
    ...
    if (jobId == 0)
        return Result<void>::Failure(MakeFoundationError("project_creation.not_ready",
