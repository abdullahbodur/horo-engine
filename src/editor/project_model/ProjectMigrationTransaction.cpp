...
[[nodiscard]] Result<std::vector<std::byte>> Read(const std::filesystem::path &path, const std::uint64_t limit)
{
    ...
    if (error || size > limit)
        return Result<std::vector<std::byte>>::Failure(Failure(
            ProjectErrors::MigrationRecoveryFailed, "Cannot read bounded migration file: " + path.generic_string()));
    std::vector<std::byte> result(static_cast<std::size_t>(size));
    std::ifstream stream(path, std::ios::binary);
    if (!stream || (size && !stream.read(reinterpret_cast<char *>(result.data()), static_cast<std::streamsize>(size))))
...
[[nodiscard]] Result<json> ParseCanonicalHistory(const std::span<const std::byte> bytes)
{
    try
    {
        ...
        json result = json::parse(reinterpret_cast<const char *>(bytes.data()),
                                  reinterpret_cast<const char *>(bytes.data() + bytes.size()), callback);
        const std::string canonical = result.dump() + "\n";
        if (duplicate || canonical.size() != bytes.size() ||
            std::memcmp(canonical.data(), bytes.data(), bytes.size()) != 0 || !result.is_object() ||
            !result.contains("receipts") || !result["receipts"].is_array())
            throw std::runtime_error("non-canonical");
...
[[nodiscard]] Result<MigrationContentHash> ParseHash(const std::string_view text)
{
    ...
    for (std::size_t i = 0; i < 32; ++i)
    {
        ...
        const int hi = nibble(text[7 + i * 2]), lo = nibble(text[8 + i * 2]);
        if (hi < 0 || lo < 0)
...
[[nodiscard]] json JournalJson(const Journal &journal)
{
    ...
    for (const auto &record : journal.records)
        records.push_back({{"path", record.path},
                           {"kind", record.kind == PreparedMigrationChangeKind::Add      ? "add"
                                    : record.kind == PreparedMigrationChangeKind::Remove ? "remove"
                                                                                         : "replace"},
...
[[nodiscard]] Result<Journal> LoadJournal(const std::filesystem::path &path)
{
    ...
    try
    {
        ...
        const auto root =
            json::parse(reinterpret_cast<const char *>(bytes.Value().data()),
                        reinterpret_cast<const char *>(bytes.Value().data() + bytes.Value().size()), callback);
        if (invalidStructure || !root.is_object())
            throw std::runtime_error("structure");
        ...
        if (!supportedRecoveryContract)
            throw std::runtime_error("recovery-contract");
        if (!root.at("records").is_array() || root.at("records").size() > 100'000)
            throw std::runtime_error("records");
        ...
        for (const auto &item : root.at("records"))
        {
            ...
            const auto normalized = std::filesystem::path(pathText).lexically_normal();
            if (normalized.is_absolute() || pathText.empty() || pathText.size() > 4096 ||
            ...
            journal.records.push_back({.path = pathText,
                                       .kind = kind == "add"      ? PreparedMigrationChangeKind::Add
                                               : kind == "remove" ? PreparedMigrationChangeKind::Remove
                                                                 ...
                                                                 const ProjectMigrationStoragePolicy &policy)
{
    ...
    for (const auto &entry : std::filesystem::directory_iterator(cleanupRoot, error))
    {
        ...
        for (std::filesystem::recursive_directory_iterator
                 it(entry.path(), std::filesystem::directory_options::skip_permission_denied, error),
             end;
             it != end && !error; it.increment(error))
        {
            ...
            const auto boundedSize =
                std::min<std::uint64_t>(static_cast<std::uint64_t>(size), policy.maximumCleanupBytes);
            if (error || bytes > policy.maximumCleanupBytes - boundedSize)
            {
...
[[nodiscard]] int PublishRank(const std::string &path)
{
...
{
    std::uint64_t bytes = 0, documents = 0;
    std::error_code error;
    for (std::filesystem::recursive_directory_iterator
         ...
         iterator != end; iterator.increment(error))
    {
        ...
        if (iterator->is_directory() && (relative == ".horo/local" || relative.starts_with(".horo/local/") ||
        ...
        if (error || bytes > limits.maxInputBytes - size)
            return Result<std::pair<std::uint64_t, std::uint64_t>>::Failure(
                                            ...
                                            const std::uint64_t input, const std::uint64_t documents)
{
    ...
    for (const auto &definition : request.plan.definitions)
    {
        ...
        const auto saturatedMultiply = [](const std::uint64_t left, const std::uint64_t right) {
            return right != 0 && left > std::numeric_limits<std::uint64_t>::max() / right
                       ? std::numeric_limits<std::uint64_t>::max()
                       : left * right;
        };
        const auto saturatedAdd = [](const std::uint64_t left, const std::uint64_t right) {
            return left > std::numeric_limits<std::uint64_t>::max() - right ? std::numeric_limits<std::uint64_t>::max()
                                                                            : left + right;
        };
        const std::uint64_t ratio = saturatedMultiply(output, estimate.maximumOutputRatioPermille) / 1000U;
        const std::uint64_t perDocument = saturatedMultiply(documents, estimate.maximumAddedBytesPerDocument);
        output = std::min(request.limits.maxOutputBytes,
                          saturatedAdd(saturatedAdd(ratio, perDocument), estimate.maximumFixedBytes));
    }
    return output;
}
...
[[nodiscard]] RecoveryEvidence InspectEvidence(const std::filesystem::path &projectRoot,
                                               const std::filesystem::path &operationRoot, const Journal &journal)
{
    ...
    const ProjectMigrationTransactionRequest &request) const
{
    ...
    ProjectMigrationStorageAdmission admission{
        ...
        .journalAndHistoryBytes = request.limits.maxJournalBytes + request.limits.maxHistoryBytes,
    ...
    admission.safetyMarginBytes =
        std::max(storagePolicy_.minimumSafetyMarginBytes,
                 withoutReserve > std::numeric_limits<std::uint64_t>::max() / storagePolicy_.safetyMarginPermille
                     ? std::numeric_limits<std::uint64_t>::max()
                     : (withoutReserve * storagePolicy_.safetyMarginPermille) / 1000U);
    ...
    const ProjectMigrationTransactionRequest &request)
{
    ...
    auto lease = mutations_.TryAcquire({request.projectRoot, ProjectMutationOwner::Migration, operationId});
    if (lease.HasError())
        return Result<ProjectMigrationTransactionResult>::Failure(lease.ErrorValue());
    ...
    if (admission.Value().RequiredBytes() > admission.Value().availableBytes)
        return Result<ProjectMigrationTransactionResult>::Failure(
            Failure(ProjectErrors::MigrationCapacityInsufficient,
                    "Conservative migration storage preflight failed: required=" +
                        std::to_string(admission.Value().RequiredBytes()) +
                        ", available=" + std::to_string(admission.Value().availableBytes) + "."));
    ...
    const auto historyPath = request.projectRoot / ".horo/migration_history.json";
    ...
    {
        metadata =
            json::parse(reinterpret_cast<const char *>(migratedProject.Value().data()),
                        reinterpret_cast<const char *>(migratedProject.Value().data() + migratedProject.Value().size()));
    }
    catch (...)
    {
        return Result<ProjectMigrationTransactionResult>::Failure(
    ...
    transactionDocuments.push_back({.path = ".horo/migration_history.json",
    ...
    if (candidate.OutputBytes() > estimatedOutput + request.limits.maxHistoryBytes + 64 * 1024)
        return Result<ProjectMigrationTransactionResult>::Failure(
            Failure(ProjectErrors::MigrationCapacityInsufficient,
    ...
    std::ranges::sort(journal.records, [](const Record &left, const Record &right) {
        const int lr = PublishRank(left.path), rr = PublishRank(right.path);
    ...
    for (auto &record : journal.records)
    {
        ...
        if (record.originalExists)
        {
            ...
            required += std::filesystem::file_size(authoritative, error);
        }
        if (record.kind != PreparedMigrationChangeKind::Remove)
        {
            ...
            required += std::filesystem::file_size(stagingRoot / record.path, error);
    ...
    const auto available = files_.AvailableBytes(request.projectRoot);
    if (available.HasError() || available.Value() < required)
        return Result<ProjectMigrationTransactionResult>::Failure(
    ...
    const std::filesystem::path &projectRoot) const
{
    ...
    MigrationRecoveryAction action = MigrationRecoveryAction::Unrecoverable;
    if (journal.Value().state == "Preparing")
        action = MigrationRecoveryAction::DiscardUnpublishedStaging;
    else if (journal.Value().state == "PublishReady" || journal.Value().state == "Publishing")
    {
        ...
        action = evidence.canResume    ? MigrationRecoveryAction::ResumePublish
                 : evidence.canRestore ? MigrationRecoveryAction::RestoreOriginals
                                       : MigrationRecoveryAction::Unrecoverable;
    }
    else if (journal.Value().state == "Committed")
    {
        ...
        action = evidence.allPublished ? MigrationRecoveryAction::FinalizeCommittedMigration
                 : evidence.canRestore ? MigrationRecoveryAction::RestoreOriginals
                                       : MigrationRecoveryAction::Unrecoverable;
    }
                                                         ...
                                                         const CancellationToken cancellation)
{
    ...
    if (snapshot.action == MigrationRecoveryAction::Unrecoverable)
        return Result<void>::Failure(*snapshot.diagnostic);
    auto lease = mutations_.TryAcquire(
        {projectRoot, ProjectMutationOwner::Migration, snapshot.operationId.value_or("recovery")});
    if (lease.HasError())
        ...
        snapshot.action == MigrationRecoveryAction::FinalizeCommittedMigration)
    {
        if (snapshot.action == MigrationRecoveryAction::FinalizeCommittedMigration)
            return MoveToCleanup(files_, projectRoot, operationRoot, journal.Value().operationId);
        std::filesystem::remove_all(operationRoot, error);
        return error ? Result<void>::Failure(Failure(ProjectErrors::MigrationRecoveryFailed, error.message()))
                     : Result<void>::Success();
    }
    if (snapshot.action == MigrationRecoveryAction::ResumePublish)
    {
        ...
        if (auto written = WriteJournal(files_, operationRoot, recovered); written.HasError())
            return written;
        auto result = Publish(files_, projectRoot, operationRoot, recovered);
        if (result.HasError())
            return result;
        return MoveToCleanup(files_, projectRoot, operationRoot, recovered.operationId);
    }
        ...
        if (iterator->originalExists)
        {
            if (!Matches(operationRoot / "rollback" / iterator->path, iterator->originalHash))
                return Result<void>::Failure(Failure(ProjectErrors::MigrationRecoveryFailed,
                                                     "Rollback original failed hash verification: " + iterator->path));
    ...
    const std::filesystem::path &projectRoot) const
{
    ...
    for (const auto &entry : std::filesystem::directory_iterator(cleanupRoot, error))
    {
        ...
        if (!entry.is_directory(error))
            continue;
        const auto journal = LoadJournal(entry.path() / "journal.json");
        if (journal.HasError() || journal.Value().state != "Committed")
            continue;
