...
namespace Horo::Editor
{
    ...
    const RendererBackendAvailability* RendererAvailabilitySnapshot::Find(
        const std::string_view backendId) const noexcept
    {
    ...
    {
        if (const RendererBackendAvailability* active = Find(activeBackendId_); active != nullptr && active->
            IsSelectable())
        {
