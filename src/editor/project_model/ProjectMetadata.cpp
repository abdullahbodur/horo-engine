...
ProjectOpenPreflight PreflightProjectOpen(const std::filesystem::path &projectRoot, const RendererAvailabilitySnapshot &availability) {
    const Application::ProjectCompatibilitySnapshot compatibility = Application::InspectProjectCompatibility(projectRoot);
    if (!IsCompatible(compatibility.status)) {
        ...
        return ProjectOpenPreflight{.status = unreadable ? ProjectOpenPreflightStatus::ProjectMetadataUnreadable
                                    ...
                                    .diagnostic = compatibility.diagnostic.has_value()
    ...
    if (backend->state == RendererAvailabilityState::RepairRequired || backend->state == RendererAvailabilityState::SignatureInvalid ||
    ...
    if (!backend->IsSelectable()) {
        const std::string diagnostic =
            !backend->diagnostic.empty() ? backend->diagnostic : "Project renderer is not available on this editor installation.";
