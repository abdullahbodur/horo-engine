...
namespace Horo::Editor
{
    ...
    ProjectSessionActivationLease& ProjectSessionActivationLease::operator=(
        ProjectSessionActivationLease&& other) noexcept
    {
        if (this != &other)
        {
            Release();
    ...
    {
        ProjectOpenPreflightSnapshot snapshot{
            .compatibility = InspectProjectCompatibility(projectRoot),
    ...
    ProjectOpenService::ProjectOpenService(JobSystem& jobs, DurableFileSystem& files,
                                           ProjectOpenPreflightService& preflight,
                                           ProjectMutationCoordinator& mutations,
                                           ProjectMigrationTransactionService& transactions,
                                           const RendererAvailabilitySnapshot& rendererAvailability,
                                           const std::span<IProjectOpenDerivedStateContributor*const> contributors)
    {
        state_ = std::make_unique<State>(State{
            jobs, files, preflight, mutations, transactions, rendererAvailability,
    ...
    Result<ProjectOpenOperationHandle> ProjectOpenService::Start(ProjectOpenRequest request)
    {
        ...
        auto submitted = state_->jobs.SubmitResult(
            ...
            [state = state_.get(), completion, requestCopy, id](
    ...
    {
        std::lock_guard lock(state_->sessions->mutex);
        if (state_->sessions->shutdown || state_->sessions->status !=
            ProjectSessionActivationLease::State::Status::Ready ||
            !state_->sessions->candidate.has_value() || state_->sessions->candidate->id != session ||
            SourceFingerprint(state_->sessions->candidate->projectRoot) != state_->sessions->candidate->
            sourceFingerprint)
    ...
    void ProjectOpenService::PumpOwnerThread()
    {
        ...
        if (!result.has_value())
        {
            snapshot.phase = operation.cancellation.Token().IsCancellationRequested()
                                 ? ProjectOpenPhase::Cancelled
                                 : ProjectOpenPhase::Failed;
            snapshot.outcome = operation.cancellation.Token().IsCancellationRequested()
                                   ? ProjectOpenOutcome::Cancelled
        ...
        {
            std::lock_guard lock(state_->sessions->mutex);
            state_->sessions->candidate = ProjectSessionCandidate{
