...
[[nodiscard]] Result<void> UpdatePatchMarker(DurableFileSystem &files, ProjectMutationCoordinator &mutations,
    ...
    auto lease = mutations.TryAcquire({projectRoot, ProjectMutationOwner::Migration, "project-open-" + std::to_string(operation.value)});
    if (lease.HasError())
...
ProjectOpenPreflightService::ProjectOpenPreflightService(ProjectMigrationTransactionService &transactions) : transactions_(transactions) {
...
ProjectOpenPreflightSnapshot ProjectOpenPreflightService::Inspect(const std::filesystem::path &projectRoot) const {
    ...
    if ((snapshot.compatibility.status == ProjectCompatibilityStatus::MigrationPathMissing ||
        ...
        auto plan = registry_->Plan(*snapshot.compatibility.sourceBaseline, snapshot.compatibility.metadata->persistentContract, *support_);
        if (plan.HasValue()) {
            snapshot.compatibility.status = ProjectCompatibilityStatus::AutomaticMigrationRequired;
...
Result<ProjectOpenOperationHandle> ProjectOpenService::Start(ProjectOpenRequest request) {
    ...
    auto submitted = state_->jobs.SubmitResult(JobDescriptor{.parentCancellation = cancellation},
        ...
        if (compatibility.status == ProjectCompatibilityStatus::CompatibleReleaseLine && compatibility.markerUpdateRequired) {
            ...
            if (auto updated = UpdatePatchMarker(state->files, state->mutations, requestCopy.projectRoot, *target, id); updated.HasError())
            ...
            if (!preflight.migrationPlan.has_value() || !compatibility.sourceBaseline.has_value() || !compatibility.metadata.has_value())
...
void ProjectOpenService::PumpOwnerThread() {
    ...
    if (!result.has_value()) {
        snapshot.phase = operation.cancellation.Token().IsCancellationRequested() ? ProjectOpenPhase::Cancelled : ProjectOpenPhase::Failed;
        snapshot.outcome =
