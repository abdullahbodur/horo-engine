...
[[nodiscard]] bool IsFinite(const EditorViewportNavigationDelta &delta) noexcept {
    return std::isfinite(delta.yawRadians) && std::isfinite(delta.pitchRadians) && std::isfinite(delta.moveRight) &&
           std::isfinite(delta.moveUp) && std::isfinite(delta.moveForward) && std::isfinite(delta.dollyScale) && delta.dollyScale > 0.0F;
}
...
[[nodiscard]] bool IsValid(const SceneObjectTransformPreview &preview) noexcept {
    ...
    return preview.object.IsValid() && Math::IsFinite(preview.localTransform.translation) && Math::IsFinite(preview.localTransform.scale) &&
...
Result<void> EditorViewportModel::Focus(const Math::Aabb &worldBounds, const float aspect) {
    if (!worldBounds.IsValid() || !std::isfinite(aspect) || aspect <= 0.0F)
        return Result<void>::Failure(MakeViewportError(ViewportModelErrors::InvalidFocusBounds, "Focus bounds and aspect must be valid."));
    ...
    if (!camera.IsValid())
        return Result<void>::Failure(MakeViewportError(ViewportModelErrors::InvalidCamera, "Focus operation produced an invalid camera."));
    current_.camera = camera;
    ++current_.revision.value;
...
Result<void> EditorViewportModel::SetTransformPreview(const SceneObjectTransformPreview &preview) {
    if (!IsValid(preview)) {
        return Result<void>::Failure(
            MakeViewportError(ViewportModelErrors::InvalidTransformPreview, "Viewport transform preview must be finite."));
    }
