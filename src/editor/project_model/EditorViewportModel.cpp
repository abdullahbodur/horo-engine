...
namespace Horo::Editor
{
    namespace
    {
        [[nodiscard]] Math::Vec3 RotateAroundAxis(const Math::Vec3 value, const Math::Vec3 axis,
        ...
        {
            return std::isfinite(delta.yawRadians) && std::isfinite(delta.pitchRadians) && std::isfinite(
                    delta.moveRight) &&
        ...
        {
            return delta.yawRadians == 0.0F && delta.pitchRadians == 0.0F && delta.moveRight == 0.0F && delta.moveUp ==
                0.0F &&
    ...
    Result<void> EditorViewportModel::Navigate(const EditorViewportNavigationDelta& delta)
    {
        ...
        {
            return Result<void>::Failure(
                MakeViewportError(ViewportModelErrors::InvalidCamera,
                                  "Viewport navigation produced an invalid camera."));
        }
    ...
    {
        if (projection != Runtime::CameraProjection::Perspective && projection !=
