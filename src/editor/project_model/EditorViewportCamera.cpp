...
namespace Horo::Editor {
    ...
    bool EditorViewportCamera::IsValid() const noexcept {
        ...
        const bool projectionValid =
            (projection == Runtime::CameraProjection::Perspective && verticalFovRadians > 0.0F && verticalFovRadians < Math::Pi) ||
            (projection == Runtime::CameraProjection::Orthographic && orthographicHeight > 0.0F);
        return Math::IsFinite(position) && Math::IsFinite(target) && Math::IsFinite(up) && std::isfinite(verticalFovRadians) &&
               std::isfinite(orthographicHeight) && std::isfinite(nearPlane) && std::isfinite(farPlane) && directionLengthSquared > 0.0F &&
               upLengthSquared > 0.0F && sideLengthSquared > 0.0F && verticalFovRadians > 0.0F && verticalFovRadians < Math::Pi &&
               projectionValid && nearPlane > 0.0F && farPlane > nearPlane;
    }
}  // namespace Horo::Editor
