...
[[nodiscard]] Error MakeExtractionError(std::string code, std::string message)
{
    return Error{ErrorCode{std::move(code)},
                 ErrorDomainId{"horo.editor.viewport_scene_extractor"},
                 ErrorSeverity::Error,
                 std::move(message),
                 {}};
}
                                                       ...
                                                       const std::size_t remainingDepth)
{
    if (remainingDepth == 0)
    {
        return Result<Math::Mat4>::Failure(
            MakeExtractionError("viewport_scene.hierarchy_cycle", "Scene object hierarchy contains a cycle."));
    }
    const auto found = std::ranges::find(objects, object, &SceneObjectSnapshot::id);
    if (found == objects.end())
    {
        return Result<Math::Mat4>::Failure(
            MakeExtractionError("viewport_scene.object_not_found", "Scene object or one of its parents is missing."));
    }
    ...
    Result<Math::Mat4> parentToWorld = ResolveWorldTransform(objects, *found->parent, preview, remainingDepth - 1);
                                                                     ...
                                                                     const SceneObjectId object)
{
    ...
    {
        if (!current->IsValid() ||
            std::ranges::any_of(chain, [current](const SceneObjectSnapshot *entry) { return entry->id == *current; }))
        {
            return Result<SceneObjectWorldTransforms>::Failure(
                MakeExtractionError("viewport_scene.hierarchy_cycle", "Scene snapshot hierarchy contains a cycle."));
        }
        ...
        if (found == document.objects.end())
        {
            return Result<SceneObjectWorldTransforms>::Failure(MakeExtractionError(
                "viewport_scene.object_not_found", "Scene object or one of its parents does not exist."));
        }
                                                               ...
                                                               Runtime::PrimitiveMeshCache &meshCache)
{
    if (!camera.IsValid())
    {
        return Result<EditorViewportSceneSnapshot>::Failure(
            MakeExtractionError("viewport_scene.invalid_camera", "Editor viewport camera is invalid."));
    }
    ...
    for (std::size_t index = 0; index < document.objects.size(); ++index)
    {
        ...
        {
            return Result<EditorViewportSceneSnapshot>::Failure(
                MakeExtractionError("viewport_scene.invalid_object_id", "Scene snapshot contains an invalid ID."));
        }
    }
    ...
    resolveWorldTransform = [&](const std::size_t index) -> Result<void> {
        ...
        else
        {
            ...
            {
                return Result<void>::Failure(MakeExtractionError("viewport_scene.parent_not_found",
    ...
    for (std::size_t index = 0; index < document.objects.size(); ++index)
    {
        ...
        if (acquired.HasError())
        {
            ...
            error.message = "Scene object " + std::to_string(document.objects[index].id.value) + ": " + error.message;
        ...
        extracted.instances.push_back(
            EditorViewportInstance{resourceHandle, worldTransforms[index], resource->localBounds,
    ...
    if (!extracted.View().IsValid())
    {
        return Result<EditorViewportSceneSnapshot>::Failure(MakeExtractionError(
            "viewport_scene.invalid_result", "Extracted viewport scene contains invalid numeric values."));
    }
                                                 ...
                                                 EditorViewportSceneSnapshot &scene)
{
    ...
    {
        return Result<void>::Failure(MakeExtractionError("viewport_scene.instance_identity_mismatch",
                                                         "Viewport instance identities are not aligned."));
    }
    if (preview.has_value() && std::ranges::find(objects, preview->object, &SceneObjectSnapshot::id) == objects.end())
    {
        return Result<void>::Failure(
            MakeExtractionError("viewport_scene.object_not_found", "Preview scene object does not exist."));
    }
    ...
    for (const SceneObjectId object : scene.instanceObjects)
    {
        ...
        if (!Math::IsFinite(transform.Value()))
        {
            return Result<void>::Failure(MakeExtractionError(
                "viewport_scene.invalid_result", "Viewport transform preview produced invalid numeric values."));
        }
    }
