...
namespace Horo::Editor {
    ...
    Result<EditorViewportPickResult> PickEditorViewportScene(const EditorViewportSceneSnapshot &scene,
        ...
        if (std::ranges::any_of(scene.instanceObjects, [](const SceneObjectId object) {
            return !object.IsValid();
        })) {
            return Result<EditorViewportPickResult>::Failure(
                MakePickingError(ViewportPickingErrors::InvalidIdentity,
        ...
        for (std::size_t index = 0; index < scene.instances.size(); ++index) {
            ...
            if (worldBounds.HasError())
                return Result<EditorViewportPickResult>::Failure(worldBounds.ErrorValue());
            const Result<std::optional<Math::RayHit>> hit = Math::IntersectRayAabb(ray.Value(), worldBounds.Value());
            if (hit.HasError())
                return Result<EditorViewportPickResult>::Failure(hit.ErrorValue());
        ...
        return Result<EditorViewportPickResult>::Success({scene.runtimeSceneId, nearestObject});
    }
}  // namespace Horo::Editor
