                                                 ...
                                                 const GuiContentRegion &contentRegion)
{
    ...
    // Footer Actions
    {
        constexpr auto kButtonSize = ImVec2(128.0f, 32.0f);
        ...
        const auto primaryLabel = ctx.localization.Get(
            "editor", state.hasFailed ? state.canRetry ? "project_loading.action.retry" : "project_loading.action.back"
        ...
        {
            ImGui::SameLine();
            const auto backLabel = ctx.localization.Get("editor", "project_loading.action.back");
