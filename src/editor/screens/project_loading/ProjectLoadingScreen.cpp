...
public:
    ...
    Result<void> OnEnter(const GuiRoute &route) override
    {
        ...
        logCtx_ = std::make_unique<Horo::Log::LogContext>(
            "project", state_.projectName,
            "path",    state_.projectRoot,
            "async",   operationId_.has_value() ? "yes" : "no");
        LOG_INFO("editor.loading", "ProjectLoadingScreen entered: project='%s' path='%s' (async=%s).",
                 state_.projectName.c_str(),
                 state_.projectRoot.c_str(),
    ...
    void Draw() override
    {
        const auto cmd = DrawProjectLoadingView(state_, context_);
    ...
    LeaveDecision CanLeave(const LeaveTarget &target) const override
    {
        ...
        if (std::holds_alternative<ApplicationCloseTarget>(target.value)) {
            LeaveRequirement req{.kind = LeaveRequirementKind::RunningOperation,
...
private:
    void UpdateAsyncCreation()
    {
        ...
        if (const auto snapshot = creationService_.Query(*operationId_)) {
            ...
            if (phaseStr && phaseStr != lastLoggedPhase_) {
                LOG_INFO("editor.loading", "[%s] %.0f%% — %s",
                         phaseStr, static_cast<double>(state_.progress), state_.statusText.c_str());
                lastLoggedPhase_ = phaseStr;
            }
            if (snapshot->state == ProjectCreationOperationState::Succeeded) {
                LOG_INFO("editor.loading", "Async project creation succeeded. Transitioning to EditorWorkspace.");
                host_.SetActiveCreationId(std::nullopt);
                static_cast<void>(
                    host_.Navigate(GuiRoute{GuiRouteKind::EditorWorkspace,
    ...
    void UpdateSimulatedLoading(float dt)
    {
        ...
        if (phaseStr && phaseStr != lastLoggedPhase_) {
            LOG_INFO("editor.loading", "[Simulated] %s (%.0f%%).",
                     phaseStr, static_cast<double>(state_.progress));
            lastLoggedPhase_ = phaseStr;
        }
        (void)prevProgress;
        ...
        if (state_.progress >= 100.0F) {
            state_.progress = 100.0F;
            LOG_INFO("editor.loading", "[Simulated] Project loading complete. Transitioning to EditorWorkspace.");
