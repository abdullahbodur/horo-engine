...
constexpr std::array<const char *, 6> kTemplateIds = {"empty",         "3d-starter", "first-person",
...
void DrawNewProjectBackdrop(const GuiContentRegion &contentRegion, const ImVec2 modalPos, const ImVec2 modalSize)
{
...
void DrawWizardHeader(const ProjectCreationController &controller, ProjectCreationViewState &st,
                      const EditorGuiContext &ctx, const ImTextureID logo, ProjectCreationViewCommand &outCommand)
{
    ...
    auto *dl = ImGui::GetWindowDrawList();
    dl->AddLine({headerPos.x, headerPos.y + HeaderH - 1.0F}, {headerPos.x + headerW, headerPos.y + HeaderH - 1.0F},
...
void DrawWizardSidebar(ProjectCreationViewState &st, const EditorGuiContext &ctx, const float sideH)
{
    ...
    const std::array<const char *, 4> kStepLabels = {stepLabelsStr[0].c_str(), stepLabelsStr[1].c_str(),
    ...
    const std::array<const char *, 4> kStepDescs = {stepDescsStr[0].c_str(), stepDescsStr[1].c_str(),
    ...
    for (int s = 1; s <= 4; ++s)
    {
        ...
        if (active)
        {
            dl->AddRectFilled(rowMin, {rowMin.x + rowSize.x, rowMin.y + rowSize.y}, Theme::U32(Theme::AccentSoft()),
                              Radius);
        }
...
void DrawTemplateIcon(ImDrawList *drawList, const int index, const ImVec2 iconPos, const ImU32 iconColor)
{
    ...
    if (index == 1)
    {
        ...
        drawList->AddRect({iconPos.x + 4.0F, iconPos.y + 6.0F}, {iconPos.x + 20.0F, iconPos.y + 18.0F}, iconColor, 2.0F,
                          0, 1.5F);
    ...
    if (index == 2)
    {
        ...
        drawList->AddRect({centerX - 7.0F, iconPos.y + 12.0F}, {centerX + 7.0F, iconPos.y + 20.0F}, iconColor, 4.0F, 0,
                          1.5F);
        return;
    }
    if (index == 3)
    {
        drawList->AddRect({iconPos.x + 4.0F, iconPos.y + 6.0F}, {iconPos.x + 20.0F, iconPos.y + 18.0F}, iconColor, 2.0F,
                          0, 1.5F);
        drawList->AddLine({iconPos.x + 4.0F, iconPos.y + 10.0F}, {iconPos.x + 20.0F, iconPos.y + 10.0F}, iconColor,
                          1.5F);
        drawList->AddLine({iconPos.x + 12.0F, iconPos.y + 10.0F}, {iconPos.x + 12.0F, iconPos.y + 18.0F}, iconColor,
                          1.5F);
        return;
    }
    if (index == 4)
    {
        drawList->AddRect({iconPos.x + 3.0F, iconPos.y + 5.0F}, {iconPos.x + 21.0F, iconPos.y + 17.0F}, iconColor, 2.0F,
                          0, 1.5F);
        drawList->AddLine({iconPos.x + 8.0F, iconPos.y + 20.0F}, {iconPos.x + 16.0F, iconPos.y + 20.0F}, iconColor,
                          1.5F);
        drawList->AddLine({iconPos.x + 12.0F, iconPos.y + 17.0F}, {iconPos.x + 12.0F, iconPos.y + 20.0F}, iconColor,
                          1.5F);
        return;
    }
...
void DrawTemplateCardContent(const EditorGuiContext &ctx, const int index, const bool selected, const float cardW,
                             const char *desc)
{
    using namespace Theme;
    using namespace WizardLayout;
...
void DrawProjectLocationField(ProjectCreationController &controller, ProjectCreationViewState &st,
                              const EditorGuiContext &ctx, const ProjectCreationDiagnostic *pathErr)
{
...
void DrawStepSettings(ProjectCreationController &controller, ProjectCreationViewState &st, const EditorGuiContext &ctx)
{
    ...
    static constexpr std::array<const char *, 3> kBuildProfile = {"desktop-debug", "desktop-profile",
                                                                  "desktop-release"};
    ...
    else if (templateId == "first-person")
    {
        ...
        ImGui::Dummy({0.0F, 8.0F});
        const float colW = (ImGui::GetContentRegionAvail().x - GridGap) * 0.5F;
        static constexpr std::array<const char *, 3> kInputMaps = {"QWERTY / Mouse", "AZERTY / Mouse",
                                                                   "Gamepad (XInput/SDL)"};
...

    {
        ...
        DrawComboField("PHYSICS", &st.physicsIndex, kPhysics.data(), static_cast<int>(kPhysics.size()), colW, ctx);
...

    {
        ...
        DrawComboField("TARGET PLATFORM", &st.targetPlatformIndex, kPlatform.data(), static_cast<int>(kPlatform.size()),
                       colW, ctx);
        controller.SetTargetPlatform(kPlatform[st.targetPlatformIndex]);

        ImGui::SameLine(0.0F, GridGap);
        DrawComboField("COMPILER FAMILY", &st.compilerFamilyIndex, kCompiler.data(), static_cast<int>(kCompiler.size()),
                       colW, ctx);
        controller.SetCompilerFamily(kCompiler[st.compilerFamilyIndex]);

        ImGui::Dummy({0.0F, GridGap});

        DrawComboField("MINIMUM C++ STANDARD", &st.cppStandardIndex, kCppStd.data(), static_cast<int>(kCppStd.size()),
                       colW, ctx);
                ...
                const bool last = false)
{
    ...
    if (!last)
    {
        ...
        for (int i = 0; i < steps; ++i)
        {
            ...
            dl->AddLine({x0, rowStart.y + rowH - 1.0F}, {x1, rowStart.y + rowH - 1.0F}, Theme::U32(Theme::Border()),
                        1.0F);
        }
    }
}
                    ...
                    const EditorGuiContext &ctx, const ProjectCreationViewState &st)
{
...

    {
        ...
        const std::string createTitle = ctx.localization.Get("editor", "project_creation.review.what_will_be_created");
        Ui::SectionTitle(createTitle.c_str(), ctx.theme.fonts);
        ImGui::Dummy({0.0F, 6.0F});
        SummaryRow("Portable metadata (commit)", ".horo/project.json, .horo/plugins.json, asset sidecars", ctx, false);
        SummaryRow("Local / derived (ignore)", ".horo/editor workspace.json, .horo/asset index.json, .horo/local/", ctx,
                   false);
        ...
        SummaryRow("Recommended .gitignore", ".horo/{editor workspace,asset index}.json .horo/local/ build/", ctx,
                   false, true);
    }
}
                                                   ...
                                                   const GuiContentRegion &contentRegion, const ImTextureID logo)
{
    ...
    const ImVec2 modalPos{contentRegion.x + (contentRegion.width - modalW) * 0.5F,
