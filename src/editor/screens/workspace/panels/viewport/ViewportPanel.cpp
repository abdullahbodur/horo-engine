...
namespace Horo::Editor {
    namespace {
        ...
        [[nodiscard]] std::optional<ImVec2> ProjectToViewport(const EditorViewportCamera &camera, const Math::Vec3 worldPosition,
                                                              const ImVec2 origin, const float width, const float height) noexcept {
        ...
        [[nodiscard]] std::optional<Math::Vec3> RotationPlaneVector(const EditorViewportCamera &camera, const Math::Vec3 center,
                                                                    const Math::Vec3 normal, const ImVec2 mouse, const ImVec2 origin,
    ...
    bool ViewportPanel::BeginCapture(const Input::PointerButton button) {
        if (inputRouter_ == nullptr || workspaceInputContext_ == nullptr || pointerCapture_.IsActive())
            return false;
        toolInputContext_ =
            inputRouter_->PushContext(Input::InputContextId{"editor.viewport.capture"}, Input::InputContextKind::EditorToolCapture);
    ...
    void ViewportPanel::DrawInteraction(ImDrawList *dl, const ImVec2 &orig, const float w, const float h, const bool hovered,
                                        const EditorWorkspaceViewModel &vm, EditorWorkspaceViewCommandData &cmd,
                                        const EditorGuiContext &ctx, const float deltaSeconds) {
        ...
        const auto selectedObject =
            vm.primarySelection.has_value() ? std::ranges::find(vm.objects, *vm.primarySelection, &SceneObject::id) : vm.objects.end();
        ...
        if (gizmoDrag_.has_value() && (selectedObject == vm.objects.end() || selectedObject->id != gizmoDrag_->object || !transformTool ||
        ...
        if (transformTool && selectedObject != vm.objects.end() && vm.primarySelectionWorldTransform.has_value() &&
            ...
            const Math::Mat4 &worldTransform = vm.primarySelectionPreviewWorldTransform.has_value()
                                                   ? *vm.primarySelectionPreviewWorldTransform
                                                   : *vm.primarySelectionWorldTransform;
            ...
            else if (const Result<Math::Transform> worldTrs = Math::TryDecomposeAffineTRS(worldTransform); worldTrs.HasValue())
                worldAxes = {
                ...
                if (vm.activeTransformTool == EditorTransformTool::Rotate) {
                    for (int axis = 0; axis < 3; ++axis) {
                        ...
                        for (int segment = 0; segment <= 64; ++segment) {
                            ...
                            const Math::Vec3 point = gizmoPosition + (basisU * std::cos(angle) + basisV * std::sin(angle)) * radiusWorld;
                    ...
                    for (int axis = 0; axis < 3; ++axis) {
                        const auto projected = ProjectToViewport(vm.viewportCamera, gizmoPosition + worldAxes[axis], orig, w, h);
                        if (!projected)
                            continue;
                    ...
                    if (vm.activeTransformTool == EditorTransformTool::Scale) {
                        ...
                        dl->AddRectFilled({gizmoCenter->x - 5.0F, gizmoCenter->y - 5.0F}, {gizmoCenter->x + 5.0F, gizmoCenter->y + 5.0F},
            ...
            if (!gizmoDrag_.has_value() && hoveredAxis.has_value() && primary.pressed && BeginCapture(Input::PointerButton::Primary)) {
                ...
                if (parentInverse.HasValue()) {
                    ...
                    gizmoDrag_ = TransformGizmoDrag{.object = selectedObject->id,
                                                    ...
                                                    .pixelsPerWorldUnit = axis < 3 ? std::max(pixelsPerWorldUnit[axis], 1.0F) : 1.0F};
                    navigationMode_ = NavigationMode::None;
                } else
                    FinishCapture();
            }
        ...
        if (gizmoDrag_.has_value()) {
            if (input.State(Input::Key::Escape).pressed) {
                ...
                if (drag.tool == EditorTransformTool::Move) {
                    ...
                    const auto current =
                        RotationPlaneVector(vm.viewportCamera, drag.initialWorldPosition, drag.worldAxis, mouse, orig, w, h);
                    if (current.has_value()) {
                        const float angle = std::atan2(Math::Dot(drag.worldAxis, Math::Cross(drag.startPlaneVector, *current)),
                    ...
                    if (drag.space == EditorTransformSpace::Local)
                        ...
                        Math::Mat4 scaleMatrix = drag.axis == 3 ? Math::Transform{.scale = {factor, factor, factor}}.ToMatrix()
        ...
        if (navigationMode_ != NavigationMode::None) {
            ...
            if (navigationMode_ == NavigationMode::Fly || navigationMode_ == NavigationMode::Orbit) {
                ...
                navigation.pitchRadians = -input.pointer.deltaY * lookSensitivity * (ctx.settings.settings.invertOrbitY ? -1.0F : 1.0F);
    ...
    auto ViewportPanel::DrawPanel(const ImVec2 &pos, const ImVec2 &size, const EditorWorkspaceViewModel &vm,
        ...
        if (hasRenderedViewport) {
            ...
            dl->AddRectFilledMultiColor(orig, ImVec2(orig.x + w, orig.y + h), ImGui::GetColorU32(ImVec4(0.05F, 0.06F, 0.09F, 1.0F)),
        ...
        const std::string objectCount =
            std::vformat(ctx.localization.Get("editor", "workspace.viewport.object_count"), std::make_format_args(objectCountValue));
