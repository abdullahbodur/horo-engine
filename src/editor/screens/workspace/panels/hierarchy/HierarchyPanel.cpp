    ...
    void HierarchyPanel::DrawPanel(const ImVec2 &pos, const ImVec2 &size, const EditorWorkspaceViewModel &vm,
        ...
        for (const HierarchyVisibleRow &row : visibleRows) {
            ...
            if (selected) {
                ...
                drawList->AddRectFilled(rowMin, ImVec2(rowMin.x + 2.0F, rowMax.y), Theme::U32(Theme::Accent()), Theme::Layout::Radius);
            } else if (hovered) {
            ...
            if (row.depth > 0) {
                ...
                drawList->AddLine(ImVec2(guideX, centerY), ImVec2(chevronCenterX - 3.0F, centerY), Theme::U32(Theme::Border()), 1.0F);
            }
            if (!node.children.empty()) {
                if (node.expanded || searchBuffer_[0] != '\0') {
                    drawList->AddTriangleFilled(ImVec2(chevronCenterX - 3.0F, centerY - 2.0F),
                                                ImVec2(chevronCenterX + 3.0F, centerY - 2.0F), ImVec2(chevronCenterX, centerY + 2.0F),
                                                Theme::U32(Theme::Dim()));
                } else {
                    drawList->AddTriangleFilled(ImVec2(chevronCenterX - 2.0F, centerY - 3.0F),
                                                ImVec2(chevronCenterX - 2.0F, centerY + 3.0F), ImVec2(chevronCenterX + 2.0F, centerY),
                                                Theme::U32(Theme::Dim()));
                }
            } else {
            ...
            if (Ui::BeginContextMenu("##HierarchyContext")) {
                ...
                if (workspaceEligible && Ui::ContextMenuItem(ctx.localization.Get("editor", "workspace.hierarchy.duplicate").c_str(),
                                                             nullptr, ctx.theme.fonts)) {
                    cmd = HierarchyEditSession::DuplicateCommand(node.id);
                }
                Ui::ContextMenuSeparator();
                if (workspaceEligible && Ui::ContextMenuItem(ctx.localization.Get("editor", "workspace.hierarchy.delete").c_str(), "Delete",
        ...
        if (workspaceEligible && panelFocused && !searchActive && !renamingId_.has_value() && editSession_.SelectedId().has_value()) {
            if (inputRouter_->ConsumeKey(*workspaceInputContext_, Input::Key::Delete) ||
                (inputRouter_->Snapshot().modifiers.command && inputRouter_->ConsumeKey(*workspaceInputContext_, Input::Key::Backspace))) {
                pendingDelete = *editSession_.SelectedId();
            }
        }
        if (pendingDelete.has_value()) {
            ...
            cmd = HierarchyEditSession::DeleteCommand(*pendingDelete);
        }
