...
namespace Horo::Editor {
    ActivityBarLayout::ActivityBarLayout() {
        for (auto &rail: m_groups) {
            rail.resize(kDefaultGroupCount);
        }
    }
    ...
    ActivityBarLayoutOperationResult
    ActivityBarLayout::Move(const std::string_view panelId, const ActivityBarSlot slot) {
        ...
        if (sourceGroup == mutableTargetGroup &&
            (requestedIndex == sourceIndex || requestedIndex == sourceIndex + 1)) {
        ...
        const std::size_t adjustedIndex = sourceGroup == mutableTargetGroup && requestedIndex > sourceIndex
                                              ? requestedIndex - 1
                                              : requestedIndex;
    ...
    std::optional<ActivityBarSlot> ActivityBarLayout::FindSlot(const std::string_view panelId) const {
        for (std::size_t railIndex = 0; railIndex < m_groups.size(); ++railIndex) {
            for (std::size_t groupIndex = 0; groupIndex < m_groups[railIndex].size(); ++groupIndex) {
                ...
                const auto item = std::find(items.begin(), items.end(), panelId);
                if (item != items.end()) {
