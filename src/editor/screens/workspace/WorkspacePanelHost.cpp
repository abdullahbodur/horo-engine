...
namespace Horo::Editor {
    namespace {
        LayoutNode MakeDefaultLayout() {
            auto left = std::make_unique<LayoutNode>(TabStackNode{
                .id = "workspace.left", .tabs = {"horo.hierarchy"}, .activeTab = "horo.hierarchy"
            });
            auto document = std::make_unique<LayoutNode>(TabStackNode{
                .id = "workspace.document", .tabs = {"horo.viewport"}, .activeTab = "horo.viewport"
            });
            auto right = std::make_unique<LayoutNode>(TabStackNode{
                .id = "workspace.right", .tabs = {"horo.inspector"}, .activeTab = "horo.inspector"
            });

            auto documentAndRight = std::make_unique<LayoutNode>(SplitNode{
                .id = "workspace.document_right", .axis = WorkspaceSplitAxis::Horizontal, .ratio = 0.72F,
                .first = std::move(document), .second = std::move(right)
            });

            return LayoutNode(SplitNode{
                .id = "workspace.root", .axis = WorkspaceSplitAxis::Horizontal, .ratio = 0.24F,
                .first = std::move(left), .second = std::move(documentAndRight)
            });
        }
        ...
        bool SplitAround(LayoutNode &node, const std::string_view targetNodeId,
                         const std::string_view panelId, const WorkspacePanelHost::DropKind kind) {
            const bool isTarget = std::visit(
                [&](const auto &value) { return value.id == targetNodeId; }, node.value);
            if (isTarget) {
                const bool horizontal = kind == WorkspacePanelHost::DropKind::SplitLeft ||
                                        kind == WorkspacePanelHost::DropKind::SplitRight;
                const bool panelFirst = kind == WorkspacePanelHost::DropKind::SplitLeft ||
                                        kind == WorkspacePanelHost::DropKind::SplitTop;
                auto original = std::make_unique<LayoutNode>(std::move(node));
                auto panel = std::make_unique<LayoutNode>(PanelNode{
                    .id = std::string(targetNodeId) + ".panel." + std::string(panelId),
                    .panel = std::string(panelId)
                });
                SplitNode split{
    ...
    WorkspaceLayoutOperationResult WorkspacePanelHost::OpenPanel(const std::string_view panelId,
        ...
        if (panelId.empty()) return {WorkspaceLayoutOperationCode::UnknownPanel};
        auto *stack = m_layout.FindTabStack(stackId);
        if (stack == nullptr) return {WorkspaceLayoutOperationCode::UnknownStack};
        if (ContainsPanel(m_layout.root, panelId)) return {WorkspaceLayoutOperationCode::UnknownPanel};
    ...
    WorkspaceLayoutOperationResult WorkspacePanelHost::DockPanel(const std::string_view panelId,
                                                                 ...
                                                                 const DropKind kind) {
        if (panelId.empty()) return {WorkspaceLayoutOperationCode::UnknownPanel};
        if (m_layout.FindNode(targetNodeId) == nullptr) return {WorkspaceLayoutOperationCode::UnknownStack};
        ...
        if (kind == DropKind::TabCenter) {
            ...
            if (alreadyPresent) {
                ...
                if (!result.Succeeded()) m_layout = backup;
                return result;
            }
            stack->tabs.emplace_back(panelId);
            stack->activeTab = stack->tabs.back();
