...
namespace Horo::Vfx {
    ...
    namespace {
        ...
        template <typename T> T *TakeStream(Detail::CpuParticleBufferState &state, std::size_t &offset) noexcept {
            ...
            T *const stream = reinterpret_cast<T *>(state.storage + offset);
        ...
        void BindStreams(Detail::CpuParticleBufferState &state) noexcept {
            std::size_t offset{};
            for (float *&stream : state.floats)
                stream = TakeStream<float>(state, offset);
            for (std::uint32_t index = 0; index < state.customFloatStreamCount; ++index)
                state.customFloats[index] = TakeStream<float>(state, offset);
            state.packedColor = TakeStream<std::uint32_t>(state, offset);
            state.customFlags = TakeStream<std::uint32_t>(state, offset);
            state.denseToSlot = TakeStream<std::uint32_t>(state, offset);
            state.slotToDense = TakeStream<std::uint32_t>(state, offset);
            state.slotGeneration = TakeStream<std::uint32_t>(state, offset);
            state.freeSlots = TakeStream<std::uint32_t>(state, offset);
            state.simulationBySlot = TakeStream<std::uint64_t>(state, offset);
        }

        void ZeroDenseParticle(Detail::CpuParticleBufferState &state, const std::uint32_t dense) noexcept {
            for (float *const stream : state.floats)
                stream[dense] = 0.0F;
            for (std::uint32_t index = 0; index < state.customFloatStreamCount; ++index)
                state.customFloats[index][dense] = 0.0F;
            state.packedColor[dense] = 0;
            state.customFlags[dense] = 0;
        }
        ...
        void MoveDenseParticle(Detail::CpuParticleBufferState &state, const std::uint32_t destination,
            ...
            for (std::uint32_t index = 0; index < state.customFloatStreamCount; ++index)
                state.customFloats[index][destination] = state.customFloats[index][source];
            state.packedColor[destination] = state.packedColor[source];
            state.customFlags[destination] = state.customFlags[source];
        }
        ...
        [[nodiscard]] Result<std::uint32_t> ResolveHandle(Detail::CpuParticleBufferState &state, const CpuParticleHandle &handle) {
            ...
            const std::uint32_t dense = state.slotToDense[handle.slot];
            if (dense == InvalidDenseIndex || dense >= state.active || state.slotGeneration[handle.slot] != handle.generation ||
                state.simulationBySlot[handle.slot] != handle.particle.Value()) {
                ++state.staleAccesses;
        ...
        void ClearLiveSlots(Detail::CpuParticleBufferState &state) noexcept {
            for (std::uint32_t dense = 0; dense < state.active; ++dense) {
                const std::uint32_t slot = state.denseToSlot[dense];
                state.slotToDense[slot] = InvalidDenseIndex;
                state.simulationBySlot[slot] = 0;
                if (state.slotGeneration[slot] == std::numeric_limits<std::uint32_t>::max())
                    ++state.retired;
                else
                    ++state.slotGeneration[slot];
            }
            state.active = 0;
            state.freeCount = 0;
            for (std::uint32_t slot = state.capacity; slot > 0; --slot) {
                const std::uint32_t index = slot - 1U;
                if (state.slotGeneration[index] != std::numeric_limits<std::uint32_t>::max())
                    state.freeSlots[state.freeCount++] = index;
            }
        }
    }  // namespace
    ...
    Result<CpuParticleBuffer> CpuParticleBuffer::Create(const CpuParticleBufferCreateInfo &info) {
        ...
        try {
            ...
            for (std::uint32_t index = 0; index < state->capacity; ++index) {
                state->slotToDense[index] = InvalidDenseIndex;
                state->slotGeneration[index] = 1;
                state->freeSlots[index] = state->capacity - index - 1U;
            }
    ...
    Result<CpuParticleHandle> CpuParticleBuffer::Spawn(const ParticleSimulationId particle) {
        ...
        const std::uint32_t slot = state_->freeSlots[--state_->freeCount];
        const std::uint32_t dense = state_->active++;
        state_->denseToSlot[dense] = slot;
        state_->slotToDense[slot] = dense;
        state_->simulationBySlot[slot] = particle.Value();
        state_->lastSimulationIdentity = particle.Value();
        ZeroDenseParticle(*state_, dense);
        state_->peakActive = std::max(state_->peakActive, state_->active);
        return Result<CpuParticleHandle>::Success(
            {.buffer = state_->buffer, .particle = particle, .slot = slot, .generation = state_->slotGeneration[slot]});
    }
    ...
    Result<void> CpuParticleBuffer::Kill(const CpuParticleHandle &handle) {
        ...
        const std::uint32_t last = state_->active - 1U;
        if (dense != last) {
            MoveDenseParticle(*state_, dense, last);
            const std::uint32_t movedSlot = state_->denseToSlot[last];
            state_->denseToSlot[dense] = movedSlot;
            state_->slotToDense[movedSlot] = dense;
        }
        --state_->active;
        state_->slotToDense[handle.slot] = InvalidDenseIndex;
        state_->simulationBySlot[handle.slot] = 0;
        if (state_->slotGeneration[handle.slot] == std::numeric_limits<std::uint32_t>::max())
            ++state_->retired;
        else {
            ++state_->slotGeneration[handle.slot];
            state_->freeSlots[state_->freeCount++] = handle.slot;
        }
        return Result<void>::Success();
    }
    ...
    Result<CpuParticleSoAView> CpuParticleBuffer::View() {
        ...
        CpuParticleSoAView view{
            ...
            .positionY = {state_->floats[1], count},
            .positionZ = {state_->floats[2], count},
            .velocityX = {state_->floats[3], count},
            .velocityY = {state_->floats[4], count},
            .velocityZ = {state_->floats[5], count},
            .sizeX = {state_->floats[6], count},
            .sizeY = {state_->floats[7], count},
            .rotation = {state_->floats[8], count},
            .angularVelocity = {state_->floats[9], count},
            .packedColor = {state_->packedColor, count},
            .age = {state_->floats[10], count},
            .maximumAge = {state_->floats[11], count},
            .customFlags = {state_->customFlags, count},
            .customFloatStreamCount = state_->customFloatStreamCount,
        };
        for (std::uint32_t index = 0; index < state_->customFloatStreamCount; ++index)
            view.customFloats[index] = {state_->customFloats[index], count};
    ...
    CpuParticleBufferStatistics CpuParticleBuffer::Statistics() const noexcept {
        ...
        return {.capacity = state_->capacity,
                ...
                .failedSpawns = state_->failedSpawns,
                .staleAccesses = state_->staleAccesses,
