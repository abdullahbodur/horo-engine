...
namespace Horo::Vfx {
    ...
    Result<CpuParticleSpawnStepResult> CpuParticleSpawnPipeline::Advance(const CpuParticleSpawnStep &step) {
        ...
        const double continuous = state_->spawnCarry + (state_->spawnRate * step.deltaSeconds);
        ...
        const std::uint32_t available = state_->buffer.Statistics().available;
        const std::uint32_t admitted = static_cast<std::uint32_t>(std::min<std::uint64_t>(requested, available));
        if (state_->nextSimulationIdentity > std::numeric_limits<std::uint64_t>::max() - admitted)
            return Failure<CpuParticleSpawnStepResult>(VfxErrors::ParticleSpawnOrdinalExhausted);

        state_->spawnCarry = continuous - integral;
        for (std::uint32_t birth = 0; birth < admitted; ++birth) {
            auto particle = ParticleSimulationId::Create(++state_->nextSimulationIdentity);
            if (particle.HasError())
                return Failure<CpuParticleSpawnStepResult>(VfxErrors::ParticleSpawnOrdinalExhausted);
            auto spawned = state_->buffer.Spawn(particle.Value());
            if (spawned.HasError())
                return Result<CpuParticleSpawnStepResult>::Failure(spawned.ErrorValue());
            const std::uint32_t dense = state_->buffer.Statistics().active - 1U;
            state_->handles[dense] = spawned.Value();
            ...
            InitializeParticle(*state_, spawned.Value(), mutableView, dense);
        }

        auto integrated = state_->buffer.View();
        if (integrated.HasError())
            return Result<CpuParticleSpawnStepResult>::Failure(integrated.ErrorValue());
        for (float &age : integrated.Value().age)
            age = std::min(age, std::numeric_limits<float>::max() - step.deltaSeconds) + step.deltaSeconds;

        std::uint32_t killed{};
        std::uint32_t dense{};
        std::uint32_t active = state_->buffer.Statistics().active;
        while (dense < active) {
            auto view = state_->buffer.View();
            if (view.HasError())
                return Result<CpuParticleSpawnStepResult>::Failure(view.ErrorValue());
            if (!ShouldKill(*state_, view.Value(), dense)) {
                ++dense;
                continue;
            }
            const std::uint32_t last = active - 1U;
            const CpuParticleHandle handle = state_->handles[dense];
            auto result = state_->buffer.Kill(handle);
            if (result.HasError())
                return Result<CpuParticleSpawnStepResult>::Failure(result.ErrorValue());
            state_->handles[dense] = state_->handles[last];
            state_->handles[last] = {};
            --active;
            ++killed;
        }

        const std::uint64_t dropped = requested - admitted;
        ++state_->statistics.steps;
        state_->statistics.spawned += admitted;
        state_->statistics.dropped += dropped;
        state_->statistics.killed += killed;
        state_->statistics.nextSpawnOrdinal += admitted;
        return Result<CpuParticleSpawnStepResult>::Success(
            {.requestedBirths = requested, .spawned = admitted, .dropped = dropped, .killed = killed, .active = active});
    }
    ...
    Result<CpuParticleHandle> CpuParticleSpawnPipeline::HandleAtDenseIndex(const std::uint32_t dense) {
        ...
        auto view = state_->buffer.View();
        if (view.HasError())
            return Result<CpuParticleHandle>::Failure(view.ErrorValue());
        if (dense >= view.Value().age.size())
