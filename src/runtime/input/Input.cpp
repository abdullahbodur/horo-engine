...
namespace Horo::Input
{
    namespace
    {
        ...
        Error InputError(const char* code, std::string message)
        {
            return Error{ErrorCode{code}, ErrorDomainId{"horo.input"}, ErrorSeverity::Error, std::move(message)};
        }
    } // namespace
                                                     ...
                                                     const InputBindingProfile& higher)
    {
        if (lower.schemaVersion != 1 || higher.schemaVersion != 1)
            return Result<InputBindingProfile>::Failure(
                InputError("input.profile.invalid_schema", "Cannot merge profiles with an unsupported schema."));
    ...
    namespace
    {
        ...
        InputBinding BindingFromJson(const nlohmann::json& json)
        {
            ...
            if (const auto chord = json.find("chord"); chord != json.end() && chord->is_array())
            {
                ...
                for (std::size_t i = 0; i < binding.chordSize; ++i) binding.chord[i] = static_cast<Key>((*chord)[i].get<
                    int>());
            }
    ...
    Result<InputBindingProfile> ParseBindingProfile(const std::string_view value)
    {
        ...
        if (value.empty() || value.size() > maximumProfileBytes)
            return Result<InputBindingProfile>::Failure(
                InputError("input.profile.malformed", "Input profile size is outside the supported bounds."));
        try
        {
            ...
            if (duplicateKey || !json.is_object())
                return Result<InputBindingProfile>::Failure(
                    InputError("input.profile.malformed",
            ...
            if (!overrides.is_array() || overrides.size() > maximumOverrides)
                return Result<InputBindingProfile>::Failure(
                    InputError("input.profile.malformed", "Input profile override count is invalid."));
            for (const auto& entry : overrides)
            {
                ...
                if (!bindings.is_array() || bindings.size() > maximumBindingsPerAction)
                    return Result<InputBindingProfile>::Failure(
                        InputError("input.profile.malformed", "Input profile binding count is invalid."));
            ...
            if (profile.schemaVersion != 1 || profile.profileId.empty() || profile.profileId.size() > 256)
                return Result<InputBindingProfile>::Failure(
                    InputError("input.profile.invalid_schema", "Invalid input profile schema."));
            return Result<InputBindingProfile>::Success(std::move(profile));
        }
        catch (const std::exception& exception)
        {
            return Result<InputBindingProfile>::Failure(InputError("input.profile.malformed", exception.what()));
        }
    }
    ...
    Result<std::string> SerializeBindingProfile(const InputBindingProfile& profile)
    {
        if (profile.schemaVersion != 1 || profile.profileId.empty())
            return Result<std::string>::Failure(InputError("input.profile.invalid_schema",
                                                           "Invalid input profile schema."));
        nlohmann::json json{
    ...
    Result<InputBindingProfile> LoadBindingProfile(const std::filesystem::path& path)
    {
        ...
        if (error || size == 0 || size > 1024U * 1024U)
            return Result<InputBindingProfile>::Failure(
                InputError("input.profile.read_failed", "Input profile is missing or exceeds the size limit."));
        std::ifstream input(path, std::ios::binary);
        if (!input) return Result<InputBindingProfile>::Failure(
            InputError("input.profile.read_failed", "Unable to read input profile."));
    ...
    Result<void> SaveBindingProfileAtomically(const std::filesystem::path& path, const InputBindingProfile& profile)
    {
        ...
        if (error) return Result<void>::Failure(InputError("input.profile.mkdir_failed", error.message()));
        ...
        };
        {
            ...
            if (!output) return Result<void>::Failure(
                InputError("input.profile.write_failed", "Unable to write input profile."));
        }
#if defined(_WIN32)
        if (!MoveFileExW(temporary.c_str(), path.c_str(), MOVEFILE_REPLACE_EXISTING | MOVEFILE_WRITE_THROUGH))
        {
            ...
            return Result<void>::Failure(
                InputError("input.profile.promote_failed", "Atomic profile replacement failed with Win32 error " +
        ...
        if (error)
        {
            ...
            std::filesystem::remove(temporary, error);
            return Result<void>::Failure(InputError("input.profile.promote_failed", message));
        }
#endif
                                                            ...
                                                            IInputCaptureOwner& owner)
    {
        if (!IsContextActive(context))
            return Result<PointerCaptureToken>::Failure(
                InputError("input.capture.inactive_context", "Input context is not active."));
        if (impl_->capture)
            return Result<PointerCaptureToken>::Failure(
                InputError("input.capture.busy", "Pointer is already captured."));
        const std::uint64_t token = impl_->nextToken++;
    ...
    Result<void> InputRouter::SetActionMap(std::vector<ActionDescriptor> actions, InputBindingProfile profile)
    {
        ...
        if (!validation.IsValid())
            return Result<void>::Failure(InputError("input.action_map.validation_failed",
    ...
    Result<void> InputRouter::SetProfile(InputBindingProfile profile)
    {
        ...
        if (!validation.IsValid())
            return Result<void>::Failure(InputError("input.profile.validation_failed",
                                                    validation.diagnostics.front().message));
        impl_->profile = std::move(profile);
        return Result<void>::Success();
    }
                                        ...
                                        const std::optional<PlayerId> player)
    {
        ...
        for (const InputBinding& binding : *bindings)
        {
            ...
            bool chord = true;
            for (std::size_t index = 0; index < binding.chordSize; ++index) chord = chord && snapshot.State(
            ...
            default:
                for (const GamepadState& pad : snapshot.gamepads)
                {
                    ...
                    else if (binding.kind == BindingControlKind::GamepadAxis)
                    {
                        ...
                        const float previousAxis = previous != nullptr
                                                       ? (binding.deadzoneKind == DeadzoneKind::Radial
                                                              ...
                                                              : ApplyDeadzone(
                                                                  previous->axes[Index(binding.gamepadAxis)], binding))
                                                       : 0.0F;
                    ...
                    else if (binding.kind == BindingControlKind::RawGamepadAxis && binding.rawControl < pad.rawAxes.
                        size())
                    {
                        axis = binding.deadzoneKind == DeadzoneKind::Radial
                        ...
                        const GamepadState* previous = impl_->previousSnapshot != nullptr
                                                           ? impl_->previousSnapshot->FindGamepad(pad.id)
                                                           : nullptr;
                        const float previousAxis = previous != nullptr && binding.rawControl < previous->rawAxes.size()
                                                       ? (binding.deadzoneKind == DeadzoneKind::Radial
                    ...
                    if (state.pressed)
                    {
                        const std::uint64_t control =
                            ...
                            (static_cast<std::uint64_t>(binding.kind == BindingControlKind::GamepadButton
                                                            ...
                                                            : binding.kind == BindingControlKind::RawGamepadButton ||
            ...
            axis *= binding.scale;
            radial2D = radial2D ||
                (descriptor->valueType == ActionValueType::Axis2D && binding.deadzoneKind == DeadzoneKind::Radial);
