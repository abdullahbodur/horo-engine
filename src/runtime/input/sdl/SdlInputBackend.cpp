...
namespace Horo::Input
{
    namespace
    {
        ...
        Error SdlError(const char* code, const char* fallback)
        {
            const char* native = SDL_GetError();
            return Error{
                ErrorCode{code}, ErrorDomainId{"horo.input.sdl"}, ErrorSeverity::Error,
                native != nullptr && *native != '\0' ? native : fallback
            };
        }
    } // namespace
    ...
    void SdlInputBackend::ProcessEvent(const SDL_Event& event)
    {
        ...
        case SDL_EVENT_JOYSTICK_BUTTON_UP:
            {
                const auto found = impl_->devices.find(event.jbutton.which);
                if (found != impl_->devices.end() && found->second.joystick)
        ...
        case SDL_EVENT_JOYSTICK_AXIS_MOTION:
            {
                const auto found = impl_->devices.find(event.jaxis.which);
                if (found != impl_->devices.end() && found->second.joystick)
                {
    ...
    Result<void> SdlInputBackend::PlayRumble(const GamepadDeviceId id, const RumbleEffect effect)
    {
        ...
        if (found == impl_->devices.end())
            return Result<void>::Failure(
                SdlError("input.haptics.stale_device", "Gamepad is unavailable."));
        if (!found->second.gamepad)
            return Result<void>::Failure(
                SdlError("input.haptics.unsupported", "Gamepad haptics are unsupported."));
        ...
        if (!SDL_RumbleGamepad(found->second.gamepad, amplitude(effect.lowFrequency), amplitude(effect.highFrequency),
            ...
            return Result<void>::Failure(SdlError("input.haptics.failed", "Unable to start gamepad rumble."));
