    ...
    namespace {
        ...
        [[nodiscard]] MeshData GenerateSphere(const SphereMeshParameters &parameters) {
            ...
            for (std::uint32_t ring = 0; ring + 1 < ringCount; ++ring) {
                ...
                for (std::uint32_t slice = 0; slice < parameters.slices; ++slice)
                    mesh.indices.insert(mesh.indices.end(), {a + slice, a + slice + 1, b + slice + 1, a + slice, b + slice + 1, b + slice});
            }
        ...
        [[nodiscard]] MeshData GenerateCone(const ConeMeshParameters &parameters) {
            ...
            for (std::uint32_t slice = 0; slice <= parameters.radialSegments; ++slice) {
                ...
                const Math::Vec3 normal =
                    Math::Normalize(Math::Vec3{parameters.height * std::cos(phi), parameters.radius, parameters.height * std::sin(phi)});
                mesh.vertices.push_back({{parameters.radius * std::cos(phi), 0, parameters.radius * std::sin(phi)}, normal, {u, 0}});
        ...
        [[nodiscard]] MeshData GenerateCapsule(const CapsuleMeshParameters &parameters) {
            ...
            for (std::uint32_t index = 1; index <= parameters.hemisphereRings; ++index) {
                ...
                rings.push_back({-bodyHalf + std::sin(angle) * parameters.radius, std::cos(angle) * parameters.radius, std::sin(angle),
                                 std::cos(angle), v});
            }
            const float midV = static_cast<float>(rings.size() + 1) / static_cast<float>(2 * parameters.hemisphereRings + 2);
            rings.push_back({bodyHalf, parameters.radius, 0, 1, midV});
            for (std::uint32_t index = 1; index < parameters.hemisphereRings; ++index) {
                const float t = static_cast<float>(index) / static_cast<float>(parameters.hemisphereRings);
            ...
            for (std::uint32_t ring = 0; ring + 1 < rings.size(); ++ring) {
                ...
                const std::uint32_t b = a + stride;
                for (std::uint32_t slice = 0; slice < parameters.radialSegments; ++slice)
                    mesh.indices.insert(mesh.indices.end(), {a + slice, b + slice, b + slice + 1, a + slice, b + slice + 1, a + slice + 1});
            }
        ...
        struct ParameterHashVisitor {
            ...
            template <typename T> void operator()(const T &parameters) const noexcept {
        ...
        [[nodiscard]] Result<MeshData> GenerateBoxMesh(const PrimitiveMeshDescriptor &descriptor) {
            ...
            if (!PositiveFinite(p->size.x) || !PositiveFinite(p->size.y) || !PositiveFinite(p->size.z))
                return Result<MeshData>::Failure(MakePrimitiveError(PrimitiveErrors::InvalidSize, "Box size must be finite and positive."));
        ...
        [[nodiscard]] Result<MeshData> GenerateSphereMesh(const PrimitiveMeshDescriptor &descriptor) {
            ...
            if (p->slices < 3 || p->stacks < 2)
                return Result<MeshData>::Failure(
                    MakePrimitiveError(PrimitiveErrors::InvalidTessellation, "Sphere tessellation is below the supported minimum."));
            if (p->slices > kMaximumSegments || p->stacks > kMaximumSegments)
                return Result<MeshData>::Failure(
                    MakePrimitiveError(PrimitiveErrors::CapacityExceeded, "Sphere tessellation exceeds the generation capacity."));
        ...
        [[nodiscard]] Result<MeshData> GenerateCapsuleMesh(const PrimitiveMeshDescriptor &descriptor) {
            ...
            if (p->totalHeight < 2.0F * p->radius)
                return Result<MeshData>::Failure(
                    MakePrimitiveError(PrimitiveErrors::InvalidCapsuleHeight, "Capsule total height cannot be smaller than its diameter."));
            if (p->radialSegments < 3 || p->hemisphereRings < 1)
                return Result<MeshData>::Failure(
                    MakePrimitiveError(PrimitiveErrors::InvalidTessellation, "Capsule tessellation is below the supported minimum."));
            if (p->radialSegments > kMaximumSegments || p->hemisphereRings > kMaximumSegments)
                return Result<MeshData>::Failure(
                    MakePrimitiveError(PrimitiveErrors::CapacityExceeded, "Capsule tessellation exceeds the generation capacity."));
        ...
        [[nodiscard]] Result<MeshData> GenerateCylinderMesh(const PrimitiveMeshDescriptor &descriptor) {
            ...
            if (p->radialSegments < 3)
                return Result<MeshData>::Failure(
                    MakePrimitiveError(PrimitiveErrors::InvalidTessellation, "Cylinder tessellation is below the supported minimum."));
            if (p->radialSegments > kMaximumSegments)
                return Result<MeshData>::Failure(
                    MakePrimitiveError(PrimitiveErrors::CapacityExceeded, "Cylinder tessellation exceeds the generation capacity."));
        ...
        [[nodiscard]] Result<MeshData> GenerateConeMesh(const PrimitiveMeshDescriptor &descriptor) {
            ...
            if (p->radialSegments < 3)
                return Result<MeshData>::Failure(
                    MakePrimitiveError(PrimitiveErrors::InvalidTessellation, "Cone tessellation is below the supported minimum."));
            if (p->radialSegments > kMaximumSegments)
                return Result<MeshData>::Failure(
                    MakePrimitiveError(PrimitiveErrors::CapacityExceeded, "Cone tessellation exceeds the generation capacity."));
        ...
        [[nodiscard]] Result<MeshData> GenerateMeshTypeChecked(const PrimitiveMeshDescriptor &descriptor) {
            ...
            switch (descriptor.type) {
                case Box:
                    return GenerateBoxMesh(descriptor);
                case Sphere:
                    return GenerateSphereMesh(descriptor);
                case Capsule:
                    return GenerateCapsuleMesh(descriptor);
                case Cylinder:
                    return GenerateCylinderMesh(descriptor);
                case Cone:
                    return GenerateConeMesh(descriptor);
                case Plane:
                    return GeneratePlaneMesh(descriptor);
                case Quad:
                    return GenerateQuadMesh(descriptor);
            }
            return Result<MeshData>::Failure(MakePrimitiveError(PrimitiveErrors::ParameterMismatch, "Unknown primitive mesh type."));
        }
    }  // namespace
    ...
    Result<PrimitiveMeshLease> PrimitiveMeshCache::Acquire(const PrimitiveMeshDescriptor &descriptor) {
        ...
        if (m_impl->limits.maximumItems == 0 || bytes > m_impl->limits.maximumBytes)
            return Result<PrimitiveMeshLease>::Failure(
                MakePrimitiveError(PrimitiveErrors::CacheCapacityExceeded, "Mesh cannot fit within the configured cache budget."));
        while (m_impl->entries.size() >= m_impl->limits.maximumItems || m_impl->bytes + bytes > m_impl->limits.maximumBytes) {
            ...
            for (auto candidate = m_impl->entries.begin(); candidate != m_impl->entries.end(); ++candidate)
                if (candidate->data.use_count() == 1 && (victim == m_impl->entries.end() || candidate->lastUse < victim->lastUse))
                    victim = candidate;
