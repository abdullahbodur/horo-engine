...
namespace Horo::Render {
    ...
    Result<void> RenderBackendRegistry::Register(RenderBackendDescriptor descriptor) {
        if (sealed_) {
            return Result<void>::Failure(MakeRegistryError(RegistryErrors::Sealed, "Renderer backend registry is already sealed."));
        }
    ...
    Result<std::unique_ptr<IRenderBackend>> RenderBackendRegistry::Create(const RenderBackendId &id) const {
        if (!sealed_) {
            return Result<std::unique_ptr<IRenderBackend>>::Failure(
                MakeRegistryError(RegistryErrors::NotSealed, "Renderer backend registry must be sealed before creation."));
        }

        const auto descriptor = std::ranges::find_if(descriptors_, [&](const RenderBackendDescriptor &registered) {
            return registered.id == id;
        });
        if (descriptor == descriptors_.end()) {
            return Result<std::unique_ptr<IRenderBackend>>::Failure(
                MakeRegistryError(RegistryErrors::BackendNotFound, "Renderer backend is not compiled into this host: " + id.Value()));
        }
        ...
        if (!backend.Value()) {
            return Result<std::unique_ptr<IRenderBackend>>::Failure(
                MakeRegistryError(RegistryErrors::ProviderReturnedNull, "The renderer backend provider returned no backend instance."));
        }
        return backend;
    }
