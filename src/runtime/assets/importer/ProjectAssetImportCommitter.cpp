...
namespace Horo::Assets {
    namespace {
        Result<void> WriteBytes(const std::filesystem::path &path, const std::span<const std::uint8_t> bytes) {
            ...
            if (!output)
                return Result<void>::Failure(MakeError(AssetErrors::IndexIo, std::format("Unable to open {}.", path.string())));
        ...
        Result<void> WriteIdentitySidecar(const std::filesystem::path &path, const AssetImportMetadata &metadata) {
    ...
    Result<void> ProjectAssetImportCommitter::Commit(PreparedAssetImportBatch batch, IAssetIdGenerator &idGenerator,
        ...
        for (const auto &item : batch.items) {
            ...
            LOG_INFO("editor.asset_import", "Committed asset → %s (%zu bytes)", outputPath.string().c_str(), prepared.editorPayload.size());
            ...
            if (item.createMetaSidecar && item.supportsMetaSidecar) {
                ...
                const AssetImportMetadata metadata{
                    ...
                    .lastImportReasons = item.importReasons.empty() ? std::vector{AssetImportReason::InitialImport} : item.importReasons,
                    .importedAtUtc = CurrentImportTimestampUtc(),
                };
                if (const Result<void> written = WriteIdentitySidecar(sidecarPath, metadata); written.HasError())
                    return written;
        ...
        if (registry_ != nullptr) {
            ...
            LOG_INFO("editor.asset_import", "Published asset registry revision %llu with %zu assets.",
                     static_cast<unsigned long long>(rebuilt.Value().publishedRevision.value), rebuilt.Value().registeredAssets);
        }
