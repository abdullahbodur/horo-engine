...
namespace Horo::Assets
{
    namespace
    {
        ...
        [[nodiscard]] std::optional<ImportSettingValue> ParseSettingValue(
            const ImportSettingDescriptor& descriptor, const std::string& serialized)
        {
            try
            {
    ...
    Result<AssetImportMetadata> ReadAssetImportMetadata(
        const std::filesystem::path& absoluteMetadataPath)
    {
        if (!absoluteMetadataPath.is_absolute())
            return Result<AssetImportMetadata>::Failure(
                MetadataError("Asset import metadata path must be absolute."));
        ...
        if (error || size == 0 || size > kMaximumMetadataBytes)
            return Result<AssetImportMetadata>::Failure(
                MetadataError("Asset import metadata is missing, empty, or too large."));

        std::ifstream input(absoluteMetadataPath, std::ios::binary);
        if (!input)
            return Result<AssetImportMetadata>::Failure(
                MetadataError("Asset import metadata could not be opened."));
        std::string contents(static_cast<std::size_t>(size), '\0');
        input.read(contents.data(), static_cast<std::streamsize>(contents.size()));
        if (input.gcount() != static_cast<std::streamsize>(contents.size()))
            return Result<AssetImportMetadata>::Failure(
                MetadataError("Asset import metadata could not be read completely."));

        const nlohmann::json json = nlohmann::json::parse(contents, nullptr, false, true);
        if (!json.is_object())
            return Result<AssetImportMetadata>::Failure(
                MetadataError("Asset import metadata schema is missing or unsupported."));
        ...
        if (schema == json.end() || !schema->is_number_unsigned() ||
            schema->get<std::uint32_t>() != AssetImportMetadata::kSchemaVersion)
        {
            return Result<AssetImportMetadata>::Failure(
                MetadataError("Asset import metadata schema is missing or unsupported."));
        }
        ...
        if (id.HasError() || type.HasError())
            return Result<AssetImportMetadata>::Failure(
                MetadataError("Asset import metadata identity is invalid."));
        ...
        if (const auto settings = json.find("importSettings");
            settings != json.end() && settings->is_object())
        {
        ...
        if (const auto dependencies = json.find("dependencies");
            dependencies != json.end() && dependencies->is_array())
        {
            for (const auto& value : *dependencies)
            {
                if (!value.is_string())
                    return Result<AssetImportMetadata>::Failure(
                        MetadataError("Asset import dependency identity is invalid."));
        ...
        if (const auto reasons = json.find("lastImportReasons");
            reasons != json.end() && reasons->is_array())
        {
    ...
    {
        if (!metadata.assetId.IsValid() || metadata.assetType.Value().empty() ||
            !metadata.absoluteSourcePath.is_absolute() ||
            metadata.importerContributionId.empty() || metadata.importerVersion.empty() ||
            metadata.importerModuleId.empty() || metadata.importerModuleVersion.empty())
        {
            return Result<std::string>::Failure(
                MetadataError("Asset import provenance is incomplete."));
        }
    ...
    Result<std::vector<ImportSettingValue>> ResolveImportSettings(
    ...
    Result<std::vector<std::uint8_t>> ReadAssetImportSource(
    ...
    {
        return FormatSha256(ComputeSha256(std::span<const std::byte>{
