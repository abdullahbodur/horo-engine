...
namespace Horo::Assets::CookErrors
{
    ...
    const ErrorCodeDescriptor InvalidTarget{
        kCookDomain, ErrorCode{"asset.cook.invalid_target"}, kCookError,
        "Cook target ID is not canonical.",
        "Use a lowercase hyphen-separated identifier such as headless-null."
    };
    const ErrorCodeDescriptor UnsupportedFormat{
        kCookDomain, ErrorCode{"asset.cook.unsupported_format"}, kCookError,
        "Cooked artifact format version is unsupported.",
        "Use a compatible engine version or re-cook the asset."
    };
    const ErrorCodeDescriptor MalformedArtifact{
        kCookDomain, ErrorCode{"asset.cook.malformed_artifact"}, kCookError,
        "Cooked artifact is malformed.",
        "Re-cook the asset from its authoritative source."
    };
    const ErrorCodeDescriptor TooLarge{
        kCookDomain, ErrorCode{"asset.cook.too_large"}, kCookError,
        "Cooked artifact exceeds the configured size limit.",
        "Increase the limit or reduce the source asset."
    };
    const ErrorCodeDescriptor HashMismatch{
        kCookDomain, ErrorCode{"asset.cook.hash_mismatch"}, kCookError,
        "Cooked artifact payload digest does not match.",
        "Re-cook the asset from its authoritative source."
    };
    const ErrorCodeDescriptor CookerMissing{
        kCookDomain, ErrorCode{"asset.cook.cooker_missing"}, kCookError,
        "No cooker contribution is registered for this asset type and target.",
        "Install or enable a matching cooker extension."
    };
    const ErrorCodeDescriptor DuplicateCooker{
        kCookDomain, ErrorCode{"asset.cook.duplicate_cooker"}, kCookError,
    ...
    const ErrorCodeDescriptor CatalogSealed{
        kCookDomain, ErrorCode{"asset.cook.catalog_sealed"}, kCookError,
        "The cooker catalog is already sealed.",
        "Register cookers before sealing the catalog."
    };
    const ErrorCodeDescriptor Cancelled{
        kCookDomain, ErrorCode{"asset.cook.cancelled"}, kCookError,
        "The cook operation was cancelled.",
        "Retry if the owning operation is still active."
    };
    const ErrorCodeDescriptor DependencyUnsupported{
        kCookDomain, ErrorCode{"asset.cook.dependency_unsupported"}, kCookError,
    ...
    const ErrorCodeDescriptor SourceReadFailed{
        kCookDomain, ErrorCode{"asset.cook.source_read_failed"}, kCookError,
...
namespace Horo::Assets::ImportErrors
{
    ...
    const ErrorCodeDescriptor NoImporter{
        kImportDomain, ErrorCode{"asset.import.no_importer"}, kImportError,
        "No importer registered for this file extension.",
        "Install or enable a matching importer extension."
    };
    const ErrorCodeDescriptor ObjNoVertices{
        kImportDomain, ErrorCode{"asset.import.obj.no_vertices"}, kImportError,
    ...
    const ErrorCodeDescriptor ObjParseWarning{
        kImportDomain, ErrorCode{"asset.import.obj.warning"}, ErrorSeverity::Warning,
