...
namespace Horo
{
    ...
    Error ConfigurationSchema::ErrorFor(const char* code)
    {
        return Error{ErrorCode{code}, ErrorDomainId{"horo.configuration"}, ErrorSeverity::Error, code};
    }
    ...
    Result<void> ConfigurationSchema::Register(const SettingDescriptor& descriptor)
    {
        ...
        {
            return Result<void>::Failure(ErrorFor("configuration.schema_invalid"));
        }
    ...
    Result<void> ConfigurationSchema::Seal()
    {
        if (m_sealed)
        {
            return Result<void>::Failure(ErrorFor("configuration.schema_sealed"));
        }
        m_sealed = true;
        return Result<void>::Success();
    }
    ...
    Result<void> ConfigurationService::Validate(const ConfigurationDraft& draft) const
    {
        ...
        {
            return Result<void>::Failure(ConfigurationSchema::ErrorFor("configuration.draft_stale"));
        }
        ...
        for (auto it = begin; it != end; ++it)
        {
            ...
            if (type == SettingValueType::Boolean)
            {
                if (rawValue == "true")
                    ...
                    return Result<void>::Failure(ConfigurationSchema::ErrorFor("configuration.json_parse_error"));
            }
    ...
    Result<void> ConfigurationService::LoadFile(const std::string& path)
    {
        ...
        if (!input)
        {
            return Result<void>::Failure(ConfigurationSchema::ErrorFor("configuration.file_not_found"));
        }
    ...
    Result<void> ConfigurationService::SaveFile(const std::string& path) const
    {
        ...
        if (!output)
        {
            return Result<void>::Failure(ConfigurationSchema::ErrorFor("configuration.file_write_error"));
        }
