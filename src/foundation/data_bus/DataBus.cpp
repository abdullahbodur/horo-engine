...
namespace Horo
{
    ...
    Subscription EngineDataBus::SubscribeErased(const EventTypeId type, const std::string_view name, Handler handler)
    {
        ...
        LOG_TRACE(state->config.logCategory, "subscribe event=%s handler=%llu", name.data(),
                  static_cast<unsigned long long>(id));
        return Subscription([weak = std::weak_ptr<State>(state), type, id, category = state->config.logCategory,
            eventName = std::string(name)]
        {
            if (const auto locked = weak.lock())
            {
                ...
                if (it != locked->handlers.end())
                {
                    ...
                    records.erase(
                        std::ranges::remove_if(records, [id](const State::Record& record) { return record.id == id; })
                        .begin(),
                        records.end());
                }
                LOG_TRACE(category, "unsubscribe event=%s handler=%llu", eventName.c_str(),
                          static_cast<unsigned long long>(id));
            }
        });
    }
    ...
    void EngineDataBus::QueueErased(const EventTypeId type, const std::string_view name,
