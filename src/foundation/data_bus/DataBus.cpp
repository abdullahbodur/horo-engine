...
namespace Horo
{
    struct EngineDataBus::State
    {
        ...
        struct Queued { EventTypeId type; std::string name; std::shared_ptr<std::any> payload; QueuedPublisher publish; };
    ...
    void EngineDataBus::PublishErased(const EventTypeId type, const std::string_view name, const std::any &raw, DeferredPublisher retry)
    {
        const auto state = m_state;
    ...
    void EngineDataBus::QueueErased(const EventTypeId type, const std::string_view name, std::shared_ptr<std::any> payload, QueuedPublisher publish)
    {
