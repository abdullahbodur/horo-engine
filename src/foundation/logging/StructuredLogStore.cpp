...
namespace Horo::Log {
    ...
    StructuredLogStore::StructuredLogStore(const std::size_t capacity) : capacity_(std::max<std::size_t>(capacity, 1U)) {}
    ...
    std::optional<StructuredLogSnapshot> StructuredLogStore::SnapshotIfChanged(const std::uint64_t knownRevision) const {
        std::lock_guard lock(mutex_);
