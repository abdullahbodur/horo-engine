...
namespace Horo::Physics {
    namespace {
        ...
        Result<void> ValidateSettings(const PhysicsWorldSettingsDescriptor &descriptor) {
            const auto world = ValidatePhysicsWorldDescriptor(descriptor.world);
            if (world.HasError())
                return world;
            const auto step = ValidatePhysicsStepPolicy(descriptor.step);
            if (step.HasError())
                return step;
            const auto budgets = ValidatePhysicsWorldBudgets(descriptor.budgets);
            if (budgets.HasError())
                return budgets;
    ...
    Result<PhysicsWorldSettings> PhysicsWorldSettings::Capture(const PhysicsWorldSettingsDescriptor &descriptor) {
        const auto valid = ValidateSettings(descriptor);
        if (valid.HasError())
