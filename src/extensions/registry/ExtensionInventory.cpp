...
namespace Horo::Extensions
{
    namespace
    {
        ...
        [[nodiscard]] std::string BuildCompositionVersion(
                ...
                fs::is_symlink(fs::symlink_status(absoluteManifestPath, error)) || error)
            {
                return Result<ExtensionManifest>::Failure(
                    MakeError(ExtensionErrors::InvalidManifest,
                              "Extension manifest must be a regular non-symlink file."));
            }
        ...
        [[nodiscard]] Result<void> CopyPackageTree(const fs::path& source, const fs::path& destination)
        {
            ...
            if (error)
                return Result<void>::Failure(
                    MakeError(ExtensionErrors::LoadFailed,
                              "Unable to create extension installation staging directory."));
                 ...
                 it != end; it.increment(error))
            {
                if (error || ++entryCount > kMaximumPackageEntries)
                    return Result<void>::Failure(
                        MakeError(ExtensionErrors::LoadFailed,
                                  "Extension package traversal failed or exceeded entry limits."));
                ...
                if (error || relative.empty() || relative.is_absolute() ||
                    std::ranges::any_of(relative, [](const fs::path& component) { return component == ".."; }))
                {
                    return Result<void>::Failure(
                ...
                else
                {
                    return Result<void>::Failure(
                        MakeError(ExtensionErrors::LoadFailed,
                                  "Extension package contains an unsupported filesystem entry."));
                }
                if (error)
                    return Result<void>::Failure(
                        MakeError(ExtensionErrors::LoadFailed, "Unable to stage extension package contents."));
            }
    ...
    Result<void> ExtensionInventory::Refresh()
    {
        ...
        {
            runtimeStates.emplace(
                entry.packageId,
                RuntimeState{
                    .active = entry.runtimeActive,
        ...
        if (error)
            return Result<void>::Failure(
                MakeError(ExtensionErrors::LoadFailed, "Unable to create the user extension directory."));
        for (const auto& directory : fs::directory_iterator(installRoot_, error))
        {
            if (error)
                return Result<void>::Failure(
                    MakeError(ExtensionErrors::LoadFailed, "Unable to enumerate installed extensions."));
            ...
            if (!IsSafePackageId(manifest.id) ||
                directory.path().filename() != fs::path{manifest.id} ||
                std::ranges::any_of(entries_, [&manifest](const ExtensionInventoryEntry& entry)
                {
                    return entry.packageId == manifest.id;
                }))
                continue;
            const std::string compositionVersion =
                BuildCompositionVersion(manifest.version, manifest.modules);
    ...
    Result<std::string> ExtensionInventory::InstallFromDirectory(const fs::path& absoluteSourceDirectory)
    {
        ...
        if (error || !fs::is_directory(source, error) ||
            fs::is_symlink(fs::symlink_status(source, error)) || error)
        {
            return Result<std::string>::Failure(
                MakeError(ExtensionErrors::InvalidManifest,
                          "Extension source must be a regular non-symlink directory."));
        }
        ...
        if (!error && IsPathContainedBy(canonicalInstallRoot, source))
        {
            return Result<std::string>::Failure(
                MakeError(ExtensionErrors::InvalidManifest,
                          "Extension source must be outside the managed installation directory."));
        }
        error.clear();
        ...
        const fs::path staging = installRoot_ /
            (".install-" + manifest.id + "-" + std::to_string(nonce));
        ...
        if (error)
        {
            cleanup();
            return Result<std::string>::Failure(
                MakeError(ExtensionErrors::LoadFailed, "Unable to publish the staged extension package."));
        }
    ...
    Result<void> ExtensionInventory::SetEnabled(const std::string_view packageId, const bool enabled)
    {
        ...
        if (entry == entries_.end())
            return Result<void>::Failure(
                MakeError(ExtensionErrors::InvalidManifest, "Unknown extension package ID."));
    ...
    {
        if (auto entry = std::ranges::find(entries_, packageId, &ExtensionInventoryEntry::packageId);
            entry != entries_.end())
        {
    ...
    std::vector<fs::path> ExtensionInventory::EnabledUserPackageRoots() const
    {
        ...
        for (const auto& entry : entries_)
        {
            if (entry.origin == ExtensionOrigin::UserInstalled &&
                entry.enabled && entry.locallyTrusted && entry.absoluteRootPath.is_absolute())
    ...
    Result<void> ExtensionInventory::LoadState()
    {
        ...
        try
        {
            ...
            for (const auto& id : state.value("enabled", Json::array()))
                if (id.is_string()) enabled_.insert(id.get<std::string>());
            for (const auto& id : state.value("trusted", Json::array()))
                if (id.is_string()) trusted_.insert(id.get<std::string>());
        }
        catch (const Json::exception&)
        {
            return Result<void>::Failure(
                MakeError(ExtensionErrors::InvalidManifest, "Extension activation state is malformed."));
        }
        return Result<void>::Success();
    }
    ...
    Result<void> ExtensionInventory::SaveState() const
    {
        ...
        if (error)
            return Result<void>::Failure(
                MakeError(ExtensionErrors::LoadFailed, "Unable to create extension state directory."));
    ...
    void ExtensionInventory::AddBuiltInPackages()
    {
        ExtensionInventoryEntry builtIn{
            ...
            .contributions = {
                {
                    .type = "asset.importer", .id = "horo.asset-importer.obj-mesh",
                    .module = "horo.builtin.assets.importer.obj"
                },
                {
                    .type = "asset.importer", .id = "horo.asset-importer.fbx-mesh",
                    .module = "horo.builtin.assets.importer.fbx"
                },
            },
