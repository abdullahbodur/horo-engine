...
namespace Horo::Extensions
{
    ...
    Result<ExtensionManifest> ParseExtensionManifest(const std::string& jsonContent)
    {
        try
        {
            ...
            const Json& package =
                json.contains("package") && json["package"].is_object()
                    ? json["package"]
                    : json;
            ...
            if (package.contains("id") && package["id"].is_string())
                ...
                return Result<ExtensionManifest>::Failure(
                    MakeError(ExtensionErrors::InvalidManifest, "Missing extension package 'id'."));
            ...
            if (json.contains("modules"))
            {
                if (!json["modules"].is_array())
                    return Result<ExtensionManifest>::Failure(
                        MakeError(ExtensionErrors::InvalidManifest, "'modules' must be an array."));
                for (const auto& module : json["modules"])
                {
                    if (!module.is_object() || !module.contains("id") || !module["id"].is_string())
                        return Result<ExtensionManifest>::Failure(
                            MakeError(ExtensionErrors::InvalidManifest,
                                      "Every extension module requires a stable 'id'."));
                    ...
                    }))
                    {
                        return Result<ExtensionManifest>::Failure(
                            MakeError(ExtensionErrors::InvalidManifest,
                                      "Extension module identities must be unique within a package."));
                    }
            ...
            if (json.contains("contributions"))
            {
                ...
                for (const auto& contribution : json["contributions"])
                {
                    if (!contribution.is_object() ||
                        !contribution.contains("type") || !contribution["type"].is_string() ||
                        !contribution.contains("id") || !contribution["id"].is_string() ||
                        !contribution.contains("module") || !contribution["module"].is_string())
                    {
                        return Result<ExtensionManifest>::Failure(
                            MakeError(ExtensionErrors::InvalidManifest,
                                      "Every contribution requires string type, id, and module fields."));
                    }
