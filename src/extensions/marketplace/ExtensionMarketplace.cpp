...
#else
[[nodiscard]] Result<std::vector<std::byte>> Download(
    const std::string&, std::size_t, const CancellationToken&)
{
    return Result<std::vector<std::byte>>::Failure(
        MarketplaceError("Extension Marketplace networking is not installed in this build."));
}
#endif
...
#else
[[nodiscard]] Result<fs::path> ExtractPackage(
    std::span<const std::byte>, const CancellationToken&)
{
    return Result<fs::path>::Failure(
        MarketplaceError("Extension Marketplace archive support is not installed in this build."));
}
#endif
} // namespace
