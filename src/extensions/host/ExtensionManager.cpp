...
[[nodiscard]] bool IsValidBoundedText(const HoroExtensionStringView value) noexcept
{
    return (value.data != nullptr || value.length == 0) &&
        value.length <= kMaximumModuleIdentityBytes;
}

[[nodiscard]] fs::path NativeLibraryPath(const ExtensionManifest& manifest,
...
ExtensionManager::ExtensionManager(Assets::AssetImporterCatalog* importerCatalog)
...
Result<std::string> ExtensionManager::LoadExtension(const std::string& extensionDir)
{
    ...
    if (!fileStream.is_open())
        return Result<std::string>::Failure(
            MakeError(ExtensionErrors::InvalidManifest, "Could not open extension.json"));
    ...
    if (manifest.modules.size() != 1)
        return Result<std::string>::Failure(
            MakeError(ExtensionErrors::InvalidManifest,
                      "The current native loader requires exactly one module per package."));
    ...
    if (!HasSafeModuleEntry(manifest.rootPath, moduleEntry) ||
        !IsContainedLibraryPath(manifest.rootPath, libraryPath))
        return Result<std::string>::Failure(
            MakeError(ExtensionErrors::InvalidManifest,
                      "Native module entry must resolve inside its absolute package root."));
    ...
    const auto loadFunc = reinterpret_cast<HoroExtensionLoadFunc>(
        library->GetSymbol("horo_extension_load"));
    if (loadFunc == nullptr)
        return Result<std::string>::Failure(
    ...
    lifetime->unload = reinterpret_cast<HoroExtensionUnloadFunc>(
        library->GetSymbol("horo_extension_unload"));
    ...
    if (!IsValidBoundedText(moduleApi.moduleId) ||
        !IsValidBoundedText(moduleApi.moduleVersion) ||
        ...
        (!View(moduleApi.moduleVersion).empty() && View(moduleApi.moduleVersion) != declaredModuleVersion))
    {
        ...
        return Result<std::string>::Failure(
            MakeError(ExtensionErrors::InvalidManifest,
                      "Loaded module identity/version does not match extension.json."));
    }
        ...
        if (m_importerCatalog == nullptr)
            return Result<std::string>::Failure(
                MakeError(ExtensionErrors::ContributionRejected,
                          "The host did not provide an asset importer catalog."));
        if (auto registered = m_importerCatalog->RegisterBatch(
                std::move(registration.contributions)); registered.HasError())
        {
            return Result<std::string>::Failure(MakeError(
                ExtensionErrors::ContributionRejected,
