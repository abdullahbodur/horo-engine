...
[[nodiscard]] Result<std::shared_ptr<ProjectMigrationContext::State>> BuildInventory(const std::filesystem::path &root,
    ...
    std::filesystem::recursive_directory_iterator iterator(canonicalRoot, std::filesystem::directory_options::skip_permission_denied,
                                                           error);
    ...
    while (iterator != end) {
        ...
        if (const std::string generic = relative.generic_string(); !portablePaths.emplace(PortablePathKey(generic)).second)
            return Result<std::shared_ptr<ProjectMigrationContext::State>>::Failure(
                MigrationError(ProjectErrors::MigrationInventoryInvalid, "Portable case collision in migration inventory: " + generic));
...
[[nodiscard]] Result<void> MergeExecutionResults(std::vector<std::optional<MigrationDocumentChange>> &results,
    ...
    for (auto &result : results) {
        ...
        if (const std::uint64_t key = (static_cast<std::uint64_t>(handle.generation) << 32U) | handle.index; !writes.emplace(key).second)
            return Result<void>::Failure(
...
[[nodiscard]] Result<void> ExecutePlan(const ProjectMigrationPlan &plan, ProjectMigrationContext &context, JobSystem &jobs,
    ...
    for (const ProjectMigrationDefinition &definition : plan.definitions) {
        for (const ProjectMigrationNode &node : definition.pipeline->Nodes()) {
            if (const Result<void> executed = ExecuteNode(definition, node, context, jobs, limits, cancellation); executed.HasError())
                return executed;
        }
    }
    return Result<void>::Success();
...
Result<std::vector<std::byte>> PreparedProjectMigration::ReadCandidateDocument(const std::string_view projectRelativePath) const {
    const auto document =
        std::ranges::find_if(state_->context->documents, [projectRelativePath](const ProjectMigrationContext::State::Document &candidate) {
...
Result<void> ProjectMigrationContext::ReplaceDocument(const MigrationDocumentHandle document, std::vector<std::byte> replacement) {
    if (document.generation != state_->generation || document.index >= state_->documents.size() || !state_->documents[document.index].alive)
...
Result<void> ProjectMigrationContext::AddDocument(const std::string &projectRelativePath, const MigrationDocumentKind kind,
    ...
    if (normalized.is_absolute() || normalized.empty() || generic == ".." || generic.starts_with("../"))
        return Result<void>::Failure(MigrationError(ProjectErrors::MigrationInventoryInvalid, "Added document path escapes the project."));
    if (const std::string key = PortablePathKey(generic); std::ranges::any_of(state_->documents, [&key](const State::Document &existing) {
    ...
    if (state_->documents.size() >= state_->limits.maxDocuments || state_->outputBytes + document.size() > state_->limits.maxOutputBytes)
        return Result<void>::Failure(MigrationError(ProjectErrors::MigrationInventoryLimit, "Added document exceeds migration limits."));
...
Result<PreparedProjectMigration> ProjectMigrationExecutor::Prepare(const std::filesystem::path &projectRoot,
    ...
    LOG_INFO("application.project_migration.execute", "Preparing migration source=%s target=%s.", sourceText.c_str(), targetText.c_str());
...
Result<void> ProjectMigrationExecutor::Finalize(PreparedProjectMigration &prepared,
    ...
    for (const PreparedMigrationDocument &document : documents) {
        ...
        if (normalized.is_absolute() || normalized.empty() || path == ".." || path.starts_with("../"))
            return Result<void>::Failure(
                MigrationError(ProjectErrors::MigrationInventoryInvalid, "Transaction-owned candidate document escapes the project root."));

        const auto existing =
            std::ranges::find_if(prepared.state_->context->documents, [&path](const ProjectMigrationContext::State::Document &candidate) {
