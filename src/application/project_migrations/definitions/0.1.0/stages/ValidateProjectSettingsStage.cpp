...
namespace Horo::ProjectMigrations::R0_1_0 {
    namespace {
        ...
        constexpr std::string_view TargetContract = "sha256:997e790fc23515b362847c755006156aa35353ce7f2624518acf7ed1214ddb03";
        ...
        [[nodiscard]] Result<Json> ProjectRoot(const Application::ProjectMigrationContext &context) {
            ...
            try {
                return Result<Json>::Success(
                    Json::parse(reinterpret_cast<const char *>(document.Value().bytes.data()),
                                reinterpret_cast<const char *>(document.Value().bytes.data() + document.Value().bytes.size())));
            } catch (...) {
                return Result<Json>::Failure(
                    MakeError(Application::ProjectErrors::MigrationStageFailed, "Candidate project metadata is not valid JSON."));
            }
        }

        template <std::size_t Size>
        [[nodiscard]] bool IsAllowed(const Json &settings, const char *key, const std::array<std::string_view, Size> &allowed) {
        ...
        [[nodiscard]] Result<void> ValidateCompression(const Json &root) {
            if (!root.is_object() || !root.contains("settings") || !root["settings"].is_object())
                return Result<void>::Failure(
                    MakeError(Application::ProjectErrors::MigrationStageFailed, "Project settings must be an object."));
            const Json &settings = root["settings"];
            if (!IsAllowed(settings, "assetCompression", AssetCompression))
                return Result<void>::Failure(
                    MakeError(Application::ProjectErrors::MigrationStageFailed, "assetCompression is missing or unsupported."));
            if (!IsAllowed(settings, "textureCompression", TextureCompression))
                return Result<void>::Failure(
                    MakeError(Application::ProjectErrors::MigrationStageFailed, "textureCompression is missing or unsupported."));
        ...
        public:
            ...
            [[nodiscard]] Result<void> Validate(const Application::ProjectMigrationContext &context,
                ...
                constexpr std::array required{"horoVersion",    "persistentContract", "projectId", "name",
                                              "projectVersion", "createdAt",          "settings",  "migrationHistoryHead"};
                if (!root.is_object() || std::ranges::any_of(required, [&root](const char *key) {
                    return !root.contains(key);
                }))
                    return Result<void>::Failure(
                        MakeError(Application::ProjectErrors::MigrationStageFailed, "Target project metadata lacks required fields."));
                if (!root["horoVersion"].is_string() || root["horoVersion"] != "0.1.0" || !root["persistentContract"].is_string() ||
