...
struct Transform {
  ...
  Transform(const Vec3 &pos, const Quaternion &rot = Quaternion::Identity(),
...
// World-space axis-aligned bounds for a mesh-local AABB (center ± half) after
// Transform.
inline void WorldAabbFromLocalBox(const Vec3 &localCenter,
                                  const Vec3 &localHalf,
                                  const Transform &worldFromLocal,
                                  Vec3 &outWorldCenter, Vec3 &outWorldHalf) {
